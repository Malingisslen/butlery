...
| ARCH-011 | Codebase-wide (400-800 locations) | Default Value Extensions adoption ~16-32% (Phase 1: 129 migrations across 10 models) - verbose null coalescing (80-250 lines remaining waste) | Low | S | P3 | ✅ PHASE 1 COMPLETE | **Nov 2025 - Phase 1 Complete**: 10 models migrated (recipe_unified, shared_shopping_list, activity_feed_item, unified_shopping_list, shared_menu, shared_content, unified_shopping_item, recipe_serialization, user_profile, realtime_invitation), 129 migrations, ~120-150 lines improved. **Extension methods**: .orEmpty(), .orZero(), .orTrue()/.orFalse(), .orNow(), .orDefault(). **Learning**: Method chaining requires parentheses: (field?.toString()).orEmpty(). **Phase 2 Target**: Remaining 270-670 locations in ViewModels, Services, Repositories (1-2 weeks). |
