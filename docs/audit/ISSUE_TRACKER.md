...
**Recent Progress**: Nov 2025 - Architecture violations corrected, SerializationUtils Phase 1-3 complete (7 models, ~144-182 lines saved including 33-line helper deletion)
...
| ARCH-009 | Codebase-wide (~20-30 model files) | SerializationUtils adoption 70% in models (Phase 1-3 complete) - duplicate Firestore parsing (340-527 lines remaining) | Medium | M | P2 | ✅ PHASE 3 COMPLETE | **Nov 2025 - Phase 3 Complete**: 7 models migrated (UserProfile, FriendRequest, RecipeComment, RecipeCore, SharedShoppingList, SharedRecipe, SharedMenu), ~144-182 lines saved, 242+ tests passing. **Phase 1** (60 lines): UserProfile, FriendRequest, RecipeComment. **Phase 2** (11-14 lines): RecipeCore, SharedShoppingList with safeObjectList pattern. **Phase 3** (73-108 lines): SharedRecipe (20 lines fromMap reduction), SharedMenu (53-88 lines including 33-line _parseTimestamp helper deletion). **Bonus**: Eliminated timestamp parsing warnings in SharedMenu tests. **Remaining**: Phase 4 (3 models - 15-24 lines). Total opportunity: 15-24 lines remaining. Incremental migration (2-3 days remaining). |
...
| ARCH-012 | Overall codebase | 1,176-2,148 lines realistic duplicate code (revised from 5,400-7,900, Phase 1-3: -144-182 lines) | Medium | L | P2 | Open | **REVISED**: Incremental infrastructure adoption program (8-12 weeks). Much infrastructure already adopted. Many "violations" were false positives. **Nov 2025 Progress**: SerializationUtils Phase 1-3 complete (-144-182 lines including 33-line helper deletion). **Remaining**: SerializationUtils Phase 4 (340-527 lines) + ValidationUtils expansion (800-1,200 lines) + incremental cleanup. |
