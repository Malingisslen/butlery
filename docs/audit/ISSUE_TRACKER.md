...
| ARCH-009 | Codebase-wide (~20-30 model files) | SerializationUtils adoption 0% in models - duplicate Firestore parsing (600-800 lines waste) | Medium | M | P2 | Open | **REAL OPPORTUNITY**: Adopt SerializationUtils in 20-30 model fromFirestore methods. Incremental migration as models are touched (2-3 weeks). |
...
| ARCH-012 | Overall codebase | 1,500-2,500 lines realistic duplicate code (revised from 5,400-7,900) | Medium | L | P2 | Open | **REVISED**: Incremental infrastructure adoption program (8-12 weeks). Much infrastructure already adopted. Many "violations" were false positives (ChangeNotifier services, well-architected ViewModels). Focus: SerializationUtils (600-800 lines) + ValidationUtils expansion (800-1,200 lines) + incremental cleanup. |
