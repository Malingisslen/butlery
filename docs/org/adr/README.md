...
- [ADR-0021](ADR-0021-the-erasure-handle-stays-out-of-the-bundle.md) — Four Art. 15 withholdings that had been chosen without asking Malin were put to her; she answered KEEP on the group-menu erasure handle, then REVERSED herself when the panel measured that the field is client-written and unverified, that any signed-in account resolves a uid through `public_profiles`, and that a minor's uid can be in it — so it stays stripped and no production behaviour changed. Row 4 (shared-list item rows) deferred to BUT-1747 as a pre-launch gate once it was measured that no client may read that path (2026-09-17).
- [ADR-0020](ADR-0020-rate-limit-stamp-and-message-exemption.md) — Rate limits are stamped in the same request and bound to the guarded document id; messages carry no burst guard because queued offline sends replay back to back; sharing and new-DM windows drop to 3 s — escalated to Malin (2026-09-14).
- [ADR-0019](ADR-0019-a-failed-moderation-read-is-not-a-clean-bill-of-health.md) — The pre-deletion moderation warning reads a tri-state, not a `bool`: Trust & Safety required a failed read to surface no UI, Security required it not to read as "never reported" — one value was answering two questions, and the fail-closed `hasOnly` limb means a future field added elsewhere would silently switch the warning off for exactly the people who have an open case. Also carries the shared-device escalation and Malin's answer to it (2026-09-12).
- [ADR-0018](ADR-0018-self-removal-uses-the-one-named-membership-write.md) — Leaving a shared shopping list goes through the ONE named membership write with a required `MembershipWriteIntent`, not a second repository method; the panel split on layering and ADR-002's zero-callers incident decided it (2026-09-12).
- [ADR-0017](ADR-0017-withholding-a-subjects-own-moderation-count.md) — **SUPERSEDED the same day: Malin reversed it and the count IS exported** (2026-09-08), after a search found that withholding a subject's OWN data to protect an ongoing process is an Art. 23 restriction requiring a legislative measure, and none exists. Original entry: **Malin: withhold the moderation COUNT too** (2026-09-08), after the DPO seat showed her single "whole collection" reason covered only half of it: the reporters' identities are third-party data (Art. 15(4), strong), but `totalReports` is the requester's OWN data and 15(4) does not reach it. Recorded as the weaker of the two positions, with the alternative (export the count alone) named. The two grounds must not be merged back into one sentence.
- [ADR-0016](ADR-0016-the-reporters-derived-moderation-row.md) — **Malin: DELETE the row** (2026-09-08), against T&S's alternative, with their condition riding along (the runbook must say a report's trace disappears entirely when its reporter erases). When the person who FILED a report erases their account, does the derived `system_events/content_report_*` row get deleted (consistent with `deleteUserReports`, which already hard-deletes the source `reports` row) or anonymized (T&S: keep the row, null `reporterId`, so a moderator can still see that a report existed). Both stakes are top-tier, so the panel did not resolve it (2026-09-08).
- [ADR-0015](ADR-0015-user-moderation-stays-out-of-BUT-2032.md) — `user_moderation` is kept OUT of BUT-2032 despite being the bigger Art. 17 gap: `reportHistory[].reporterId` is the only data surviving the reporter's own erasure that can answer whether one account repeatedly reports the same target (DSA art. 23), so stripping it must be a Trust & Safety decision, not a side effect of tidying two collections that share a trigger. User-safety over shared-truth consistency; own ticket, filed before BUT-2032 closes (2026-09-08).
- [ADR-0014](ADR-0014-the-new-system-events-cascade-steps-keep-their-audit-rows.md) — The new `system_events` cascade steps stage their own `stageCascadeAuditEntry` rows rather than inheriting the cascade's zero-audit posture. The plan had argued the steps "mirror BUT-781"; Security measured that BUT-781's anonymization runs in the TRIGGER, where it DOES audit per row — the analogy carries the action, not the record. Data-integrity over cost, ~1 write per mutated row (2026-09-08).
...
- [ADR-0012](ADR-0012-real-recipe-pages-as-test-fixtures-in-a-public-repo.md) — **SUPERSEDED 2026-09-05 by its own terms review** (koket.se forbids copying outright and ends its terms with "Do not mine us"; recept.se blocks ClaudeBot with an express art. 4 reservation; and full capture buys nothing technically because the parser reads JSON-LD and DOM, not prose). Malin withdrew the override and chose structure + placeholder text, keeping ingredient lines verbatim because they are data and the parser's hardest logic. Original entry: BUT-1490's fabricated site fixtures get replaced with real captured pages; Legal Counsel recommended DOM structure with placeholder text instead, because `Malingisslen/butlery` is PUBLIC and the DSM art. 4 TDM exception covers the fetch but not indefinite redistribution, with the sui generis database right (URL 49 §) running separately from copyright — Malin overrode it and chose full capture, keeping every condition: clause text shown before any fetch, restrictive-or-silent sites keep fabricated HTML, provenance is hygiene not permission (2026-09-03).
