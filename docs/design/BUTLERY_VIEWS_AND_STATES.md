...
8. [All Dialogs & Overlays](#all-dialogs--overlays)
9. [Catalogs moved to their source of truth](#catalogs-moved-to-their-source-of-truth)
...
## Catalogs moved to their source of truth

These sections were verbatim duplicates and drifted stale; read the authoritative source instead:

- **Snackbar messages** → `lib/l10n/app_sv.arb` / `app_en.arb` (butler-voice rules in `docs/design/butler-voice-guide.md`).
- **User flows** → the interactive [`docs/onboarding/workflow-map.html`](../onboarding/workflow-map.html) (JSON-driven, CI-linted).
- **Colour palette** → `lib/theme/app_colors.dart` (tokens also in [`butlery-mockup-reference.md`](butlery-mockup-reference.md) §1).
- **Spacing / border-radius** → `lib/theme/app_dimensions.dart` (square design language — see `lib/widgets/CLAUDE.md`).
