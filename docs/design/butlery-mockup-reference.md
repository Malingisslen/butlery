...
Per-screen layout inventories moved out — they duplicated `lib/views/` and
[`BUTLERY_VIEWS_AND_STATES.md`](BUTLERY_VIEWS_AND_STATES.md) (route → screen → states),
which are the source of truth. This file keeps only what `/preview` consumes: the design
tokens (§1), typography (§2), and the reusable component specs (§4) below.
