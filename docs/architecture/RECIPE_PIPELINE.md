...
- **Allergen safety spine:** global `ingredients` collection (client-write-denied) → `IngredientLookupService` → Phase 1 TriState. Three writers mutate that collection: `sync-ingredients.ts` (Sheet CSV), a *duplicate* standalone `tools/sync_ingredients.dart`, and alias learning. The `toAdd` path is a full-overwrite `batch.set` that wipes learned aliases; no validation gate between a Google-Sheet cell edit and production allergen verdicts.
