...
6. **M6: Insecure Authorization**
   ...
   ```
   - Review Firebase Security Rules coverage (firestore.rules - 1337 lines)
   ...
   **Butlery-Specific Authorization Checks:**
   - **PermissionValidationMixin on all repository classes** - Verify all Firebase repositories use this mixin
   - **Collection group rule coverage** - Butlery uses `collectionGroup('members')` queries requiring specific rules
   - **Subcollection-based sharing security** - Issue #014 moved from array-based to subcollection sharing
   - **Cloud Functions security** - Functions use Admin SDK; verify no sensitive operations exposed to clients
   - **Audit logging for GDPR Article 30** - Check FirebaseAuditRepository usage for compliance
...
**Butlery-Specific Security Checks (Additional):**

1. **FCM Token Security**
   - Verify FCM tokens are user-scoped only
   - Check token refresh handling
   - Audit token storage location (should use secure storage)

2. **GDPR Consent Tracking (Article 7)**
   - Verify FirebaseConsentRepository implementation
   - Check granular consent controls
   - Audit consent version tracking

3. **Real-time Collaboration Security**
   - Verify presence system security (who can see active editors)
   - Check collaborative editing permissions
   - Audit copy-on-write pattern for shared content
