...
- **FirebaseServiceMixin**: `lib/core/mixins/firebase_service_mixin.dart` (820 lines)
- **Firebase config**: `lib/core/config/firebase_config.dart`
- **Cloud Functions**: `functions/src/index.ts`

### Butlery-Specific Firebase Checks (CRITICAL)

1. **FirebaseServiceMixin Adoption**
   - This 820-line mixin centralizes Firebase operations, eliminating 300+ duplicate lines
   - Check: All Firebase-accessing services should use this mixin
...
2. **Multi-Platform Firebase Configuration**
   - Butlery supports 5 platforms: Android, iOS, Web, macOS, Windows
   - Verify: `firebase_config.dart` handles all platforms
   - Check: Emulator support enabled for local development

3. **Cloud Functions Analysis**
   - `structureRecipe` / `ocrRecipeImage` - LLM integration (Mistral AI)
   - `updateRecipeRatingStats` - Denormalized rating aggregation triggers
   - `sendNotification` / `sendNotificationBatch` - FCM delivery
   - Check: Triggers properly configured (onCreate, onUpdate, onDelete)

4. **Subcollection-Based Sharing Pattern**
   - Issue #014 addressed unlimited sharing via subcollections instead of array limits
   - Collections: shared_recipes/members, shared_menus/members, etc.
   - Verify: Security rules cover collection group queries
