...
## Butlery-Specific Edge Case Checks

### Circuit Breaker Usage
1. **CircuitBreaker Class Adoption**
   - `lib/core/circuit_breaker.dart` exists
   - Check: Adoption for external service calls
   - Verify: Circuit opens after repeated failures

### DNS Error Handling
2. **DNS-Aware Error Handling**
   - `FirebaseServiceMixin` handles DNS errors specifically
   - Check: `executeFirebaseOperationWithDNSResilience()` usage
   - Verify: Multiple DNS server testing (Google, Cloudflare, Quad9)

### Offline Sync Conflict Detection
3. **Sync Conflict Handling**
   - `SyncStatus` enum includes 'conflict' state
   - Check: Conflict state properly surfaced to UI
   - Verify: User can resolve conflicts

### Collaborative Editing Conflicts
4. **Real-time Collaboration Conflicts**
   - Multiple editors simultaneously on same recipe
   - Check: Last-write-wins or merge strategy implemented
   - Verify: Presence tracking shows active editors

### Business Logic Edge Cases
5. **Friend Request Expiration**
   - Friend requests expire after 7 days (`isExpired` getter)
   - Check: Expired requests handled gracefully
   - Verify: UI shows appropriate message

6. **FCM Token Refresh**
   - Token refresh handling in notification service
   - Check: Stale tokens cleaned up
   - Verify: Token refresh doesn't interrupt notifications
