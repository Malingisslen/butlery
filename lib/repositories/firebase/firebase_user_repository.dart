...
class FirebaseUserRepository extends BaseFirebaseRepository<UserProfile>
  ...
  Future<({WriteBatch batch, int count})> _commitIfNeeded(
    ...
    if (count >= _batchLimit) {
      await batch.commit();
      return (batch: firestore.batch(), count: 0);
    }
  ...
  Future<void> propagateProfileChanges({
    ...
    await validateSelfOperation(
      currentUserId: currentUser,
      targetUserId: userId,
      operation: 'propagate profile changes',
    );

    final futures = <Future<void>>[
      _propagateToComments(userId, newDisplayName, newAvatarUrl),
      _propagateToRealtimeResources(userId, newDisplayName),
      _propagateToSharedRecipes(userId, newDisplayName),
      _propagateToShoppingLists(userId, newDisplayName),
      _propagateToGroupInvitations(userId, newDisplayName),
      _propagateToConversations(userId, newDisplayName, newAvatarUrl),
      _propagateToMessages(userId, newDisplayName, newAvatarUrl),
    ];
    ...
    // All individual methods log their own errors; nothing else to do here.
    // This line just forces the linter to see `results` as used.
    assert(results.length == futures.length);

    logPermissionCheck(
      userId: currentUser,
      resource: 'user_profile',
      operation: 'propagate_profile_changes',
      granted: true,
      details: 'Propagated displayName to denormalized locations',
    );
  }

  Future<void> _propagateToComments(
    ...
    try {
      ...
      AppLogger.warning('⚠️ Failed to propagate displayName to comments: $e');
    }
  }

  Future<void> _propagateToRealtimeResources(
    ...
    for (final col in [
      ...
      try {
        ...
        for (final doc in ownerDocs.docs) {
          final updates = <String, dynamic>{
            'ownerDisplayName': displayName,
          };
          // Also update lastEditedByDisplayName if this user was the last editor
          if (doc.data()['lastEditedBy'] == userId) {
            updates['lastEditedByDisplayName'] = displayName;
          }
        ...
        for (final doc in editorDocs.docs) {
          // Skip docs already updated above (where user is also owner)
          if (doc.data()['ownerId'] == userId) continue;
          batch.update(doc.reference, {
        ...
        AppLogger.warning('⚠️ Failed to propagate displayName to $col: $e');
      }
    }
  }

  Future<void> _propagateToSharedRecipes(
      String userId, String displayName) async {
    try {
      final snapshot = await firestore
          .collection(FirestoreCollections.sharedRecipes)
          .where('socialData.ownerId', isEqualTo: userId)
          .get();
      ...
      for (final doc in snapshot.docs) {
        batch.update(doc.reference, {
          'socialData.ownerDisplayName': displayName,
        });
        opCount++;
        final state = await _commitIfNeeded(batch, opCount);
        batch = state.batch;
      ...
      AppLogger.warning(
          '⚠️ Failed to propagate displayName to shared recipes: $e');
    }
  }

  Future<void> _propagateToShoppingLists(
      String userId, String displayName) async {
    for (final col in [
      FirestoreCollections.unifiedShoppingLists,
      ...
      try {
        ...
        for (final doc in ownerDocs.docs) {
          ...
          if (doc.data()['lastActivityByUserId'] == userId) {
            updates['lastActivityByDisplayName'] = displayName;
          }
          batch.update(doc.reference, updates);
          opCount++;
          final state = await _commitIfNeeded(batch, opCount);
        ...
        for (final doc in activityDocs.docs) {
          if (doc.data()['ownerId'] == userId) continue;
          batch.update(doc.reference, {
            'lastActivityByDisplayName': displayName,
          });
          opCount++;
          final state = await _commitIfNeeded(batch, opCount);
          batch = state.batch;
  ...
  Future<void> _propagateToGroupInvitations(
      String userId, String displayName) async {
    try {
      final snapshot = await firestore
      ...
      AppLogger.warning(
          '⚠️ Failed to propagate displayName to group invitations: $e');
    }
  }

  Future<void> _propagateToConversations(
      String userId, String displayName, String? avatarUrl) async {
    try {
      final snapshot = await firestore
      ...
      for (final doc in snapshot.docs) {
        batch.update(doc.reference, {
          ...
          'participantAvatarUrls.$userId': avatarUrl,
        });
        opCount++;
        final state = await _commitIfNeeded(batch, opCount);
        batch = state.batch;
      ...
      AppLogger.warning(
          '⚠️ Failed to propagate displayName to conversations: $e');
    }
  }

  Future<void> _propagateToMessages(
      String userId, String displayName, String? avatarUrl) async {
    try {
      // Only update recent messages (last 30 days) to avoid massive queries
      ...
      AppLogger.warning('⚠️ Failed to propagate displayName to messages: $e');
    }
  }
