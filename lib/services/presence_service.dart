...
class PresenceService extends BaseService {
  ...
  Future<void> startTyping(String conversationId) async {
    try {
      ...
      // Set typing status
      await _firestore.collection(FirestoreCollections.presence).doc(currentUser.uid).set({
  ...
  Future<bool> isUserOnline(String userId) async {
    try {
      final snapshot =
          await _firestore.collection(FirestoreCollections.presence).doc(userId).get();
  ...
  Future<void> _cleanupStaleTypingIndicators() async {
    try {
      ...
      final snapshot =
          await _firestore.collection(FirestoreCollections.presence).doc(currentUser.uid).get();
