...
class RealtimeCacheManager {
  ...
  static Future<Recipe?> loadFromCache({
    ...
    try {
      ...
      if (cachedData != null) {
        ...
        AppLogger.debug('✅ Real-time recipe loaded from cache: ${recipe.title}');
        return recipe;
      }
      return null;
    } catch (e) {
  ...
  static Future<void> updateCacheWithEdit({
    ...
    try {
      ...
      if (existingData == null) {
        AppLogger.warning('No cached recipe found for real-time update: $recipeId');
        return;
      }
      ...
      AppLogger.debug('✅ Cache updated with real-time edit for recipe $recipeId');
    } catch (e) {
  ...
  static void _applyEditToData(Map<String, dynamic> recipeData, Map<String, dynamic> editData) {
    // Update basic fields
  ...
  static Future<void> preloadRecipesForRealtimeEditing({
    ...
    try {
      AppLogger.info('🔄 Preloading ${recipeIds.length} recipes for real-time editing');
  ...
  static Future<void> _cleanupConflictTimers(
    ...
    AppLogger.info('⏰ Cancelling ${conflictResolutionTimers.length} conflict timers');
  ...
  static Future<void> cleanupExcessMemory({
    ...
    try {
      ...
      if (estimatedMemory > 1024) { // If over 1MB
        AppLogger.warning('⚠️ High memory usage detected: ${estimatedMemory}KB');
  ...
  static Future<void> _cleanupOldestPendingEdits(
    ...
    for (final entry in pendingRealtimeEdits.entries) {
      ...
      if (recipeEdits.length > 10) { // Keep only 10 most recent edits per recipe
