...
class UnifiedRecipeService
  ...
  /// BUT-989 / BUT-1048: symmetric link/unlink delegated to
  /// [RecipeRelationsService]. Facade methods retained for backwards
  /// compatibility with existing callers; new tests should target the
  /// extracted service directly (mockable without the
  /// `implements Interface` noSuchMethod-on-concrete-method trap).
  late final RecipeRelationsService _relations = RecipeRelationsService(
    getRecipe: getRecipeById,
    updateRecipe: (recipe) async =>
        (await personal.updateUnifiedRecipe(recipe)).isSuccess,
  );

  Future<bool> linkRecipes(String aId, String bId) => _relations.link(aId, bId);

  Future<bool> unlinkRecipes(String aId, String bId) =>
      _relations.unlink(aId, bId);
