...
class PerformanceReport {
  ...
  Map<String, dynamic> toJson() => {
    ...
    'metrics': metrics.map((k, v) => MapEntry(k.name, v.map((m) => m.toJson()).toList())),
    'summary': summary,
    'warnings': warnings,
  };
}
...
class PerformanceMonitoringService {
  ...
  void stopTimer(String name, {MetricType type = MetricType.custom, Map<String, dynamic>? metadata}) {
  ...
  void recordNetworkRequest(String url, Duration duration, {int? statusCode, int? byteSize}) {
  ...
  void recordUserInteraction(String interaction, Duration responseTime) {
    ...
    if (responseTime > _thresholds.maxInteractionTime) {
      _addWarning('Slow interaction response: $interaction (${responseTime.inMilliseconds}ms)');
    }
  }
  ...
  PerformanceReport generateReport() {
    ...
    if (cacheHitRate < _thresholds.minCacheHitRate) {
      _addWarning('Low cache hit rate: ${(cacheHitRate * 100).toStringAsFixed(1)}%');
    }
