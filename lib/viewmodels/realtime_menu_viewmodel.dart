// lib/viewmodels/realtime_menu_viewmodel_new.dart
...
/// - RealtimeMenuState: Core state management and UI state
/// - RealtimeStreamManager: Real-time streaming and connection management
/// - RealtimeMenuOperations: Recipe operations and category management
/// - RealtimeParticipantManager: Participant management and permissions
///
...
class RealtimeMenuViewModel extends ChangeNotifier {
  ...
  // Legacy managers (still needed for compatibility)
  ...
  RealtimeMenuViewModel({
    ...
    // Forward state notifications
    _state.addListener(() => notifyListeners());

    _initialize();
  }

  // ===== STATE GETTERS (DELEGATE TO STATE) =====
  ...
  // Connection status (delegate to connection monitor)
  ...
  // Permission getters (delegate to participant manager)
  ...
  // Participant getters (delegate to participant manager)
