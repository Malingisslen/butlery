...
enum ImageUploadState { pending, uploading, completed, failed, cancelled, retrying }
...
class ImageUploadStatus {
  ...
  bool get canRetry => hasError && retryAttempts < maxRetryAttempts && state != ImageUploadState.cancelled;
  bool get isRetrying => state == ImageUploadState.retrying;
  bool get isActive => state == ImageUploadState.uploading || state == ImageUploadState.retrying;
  ...
  double? get fileSizeMB => totalBytes != null ? totalBytes! / (1024 * 1024) : null;
  
  /// Upload speed in MB/s for display
  double? get uploadSpeedMBPerSecond => uploadSpeedBytesPerSecond != null ? 
    uploadSpeedBytesPerSecond! / (1024 * 1024) : null;
...
class UploadSafetyResult {
  ...
  int get totalProblemsCount => pendingImagePaths.length + failedImagePaths.length;
}
...
class CircuitBreakerState {
  ...
  CircuitBreakerState withFailure() {
    return CircuitBreakerState(
      ...
      isOpen: failureCount + 1 >= RecipeImageManager._circuitBreakerFailureThreshold,
    );
  }
...
class RecipeImageManager extends ChangeNotifier with StreamManagementMixin {
  ...
  final Set<_CancellableUploadOperation> _activeUploads = <_CancellableUploadOperation>{};
  ...
  final Map<String, ImageUploadStatus> _imageStates = {}; // Complete state tracking system
  ...
  static final Random _random = Random(); // Shared Random instance for jitter calculations
  ...
  static const int _circuitBreakerFailureThreshold = 10; // Open circuit after 10 failures
  
  // Background notification system
  final Set<UploadNotificationTrigger> _notificationTriggers = {};
  DateTime? _lastNotificationTime;
  static const Duration _notificationCooldown = Duration(seconds: 3); // Prevent notification spam
  ...
  bool get isUploadingImage {
    return _isUploadingImage || _imageStates.values.any((status) => status.state == ImageUploadState.uploading);
  }
  ...
  double getUploadProgress(String filePath) => _imageStates[filePath]?.progress ?? 0.0;
  
  /// Check if a file is currently uploading
  bool isFileUploading(String filePath) => _imageStates[filePath]?.state == ImageUploadState.uploading;
  ...
  Map<String, ImageUploadStatus> get imageUploadStatuses => Map.unmodifiable(_imageStates);
  ...
  void reorderImages(int oldIndex, int newIndex) {
    ...
    if (oldIndex >= 0 && oldIndex < imageKeys.length && 
        newIndex >= 0 && newIndex < imageKeys.length && 
        oldIndex != newIndex) {
  ...
  Future<void> showImagePickerDialog(BuildContext context, {String? recipeId}) async {
    AppLogger.info('🎯 IMAGE_PICKER_FLOW: Starting showImagePickerDialog');
    
    if (!canAddMoreImages) {
      AppLogger.warning('🎯 IMAGE_PICKER_FLOW: Cannot add more images ($totalImageCount/$maxImages)');
    ...
    try {
      ...
      AppLogger.info('🎯 IMAGE_PICKER_FLOW: Showing ImagePickerDialogs.showImageSourceDialog');
      final imageSource = await ImagePickerDialogs.showImageSourceDialog(context);
      AppLogger.info('🎯 IMAGE_PICKER_FLOW: Dialog returned imageSource: $imageSource');

      if (imageSource != null) {
        // Step 2: Use ImagePickerService to actually pick image from selected source
        AppLogger.info('🎯 IMAGE_PICKER_FLOW: Using ImagePickerService to pick image from: ${imageSource.name}');
        final pickedFile = await _imagePickerService.pickImage(imageSource);
        AppLogger.info('🎯 IMAGE_PICKER_FLOW: ImagePickerService returned: $pickedFile');
        ...
        if (pickedFile != null) {
          ...
          AppLogger.info('🎯 IMAGE_PICKER_FLOW: Created XFile, calling _processImagePickerResult');
          await _processImagePickerResult(xFile, recipeId: recipeId);
          AppLogger.info('🎯 IMAGE_PICKER_FLOW: _processImagePickerResult completed');
        } else {
          AppLogger.warning('🎯 IMAGE_PICKER_FLOW: No image selected from ${imageSource.name}');
  ...
  Future<void> pickImageFromCamera(BuildContext context, {String? recipeId}) async {
    ...
    try {
      ...
      AppLogger.info('🎯 IMAGE_PICKER_FLOW: Using ImagePickerService to pick image from camera');
      ...
      if (pickedFile != null) {
        ...
        AppLogger.warning('🎯 IMAGE_PICKER_FLOW: No image selected from camera');
        _setImageUploadError('Ingen bild valdes');
      }
    } catch (e) {
      AppLogger.error('🎯 IMAGE_PICKER_FLOW: Exception occurred: $e');
  ...
  Future<void> pickImageFromGallery(BuildContext context, {String? recipeId}) async {
    ...
    try {
      _clearImageUploadError();
      _setUploadingImage(true); // Add loading state
      AppLogger.info('🎯 IMAGE_PICKER_FLOW: Using ImagePickerService to pick image from gallery');
      
      final pickedFile = await _imagePickerService.pickImage(ImageSource.gallery);
      AppLogger.info('🎯 IMAGE_PICKER_FLOW: ImagePickerService returned: $pickedFile');
      ...
      if (pickedFile != null) {
        ...
        AppLogger.warning('🎯 IMAGE_PICKER_FLOW: No image selected from gallery');
        _setImageUploadError('Ingen bild valdes');
      }
    } catch (e) {
      AppLogger.error('🎯 IMAGE_PICKER_FLOW: Exception occurred: $e');
  ...
  Future<void> pickMultipleImagesFromGallery(BuildContext context, {String? recipeId}) async {
    AppLogger.info('🎯 IMAGE_PICKER_FLOW: Starting pickMultipleImagesFromGallery');
    ...
    try {
      ...
      AppLogger.info('🔍 Using ImagePickerService to pick multiple images from gallery');
  ...
  Future<void> showMultipleImagePickerDialog(BuildContext context, {String? recipeId}) async {
  ...
  Future<void> uploadMultipleImages(List<XFile> imageFiles, String recipeId) async {
    ...
    if (filesToUpload.length < imageFiles.length) {
      limitationMessage = 'Bara ${filesToUpload.length} av ${imageFiles.length} bilder kunde laddas upp (max $maxImages bilder)';
    }
    ...
    try {
      ...
      AppLogger.info('Started background uploads for ${filesToUpload.length} images');
  ...
  Future<void> removeImageAndCleanup(String pathOrUrl) async {
    try {
      ...
      if (imageStatus?.url != null && _isUploadedUrl(imageStatus!.url!)) {
        ...
        AppLogger.info('☁️ 🗑️ Bild borttagen från Firebase storage: ${imageStatus.url}');
      } else {
        AppLogger.info('📁 🗑️ Pending file removed (no storage cleanup needed): $pathOrUrl');
      }
    } catch (e) {
  ...
  Future<void> _processImagePickerResult(dynamic result, {String? recipeId}) async {
    AppLogger.info('🚀 INSTANT_IMAGES: Processing image picker result (no upload yet)');
    
    try {
      if (result is XFile) {
        // Single image - add to pending list for immediate display
        final file = File(result.path);
        await _addPendingImageFromFile(file);
      } else if (result is List<XFile>) {
        // Multiple images - add all to pending list
  ...
  Future<void> _addPendingImageFromFile(File imageFile) async {
    ...
    AppLogger.info('⚡ INSTANT_FEEDBACK: Image added with immediate UI update: $filePath');
  ...
  String _getTemporaryRecipeId() {
    ...
    _temporaryRecipeId ??= 'temp_${DateTime.now().millisecondsSinceEpoch}_${_random.nextInt(10000)}';
  ...
  void _startBackgroundUploadForFile(File imageFile, String recipeId) {
    ...
    _imageStates[filePath] = ImageUploadStatus(
      ...
      maxRetryAttempts: currentState?.maxRetryAttempts ?? 3, // Will be updated based on error type
    );
    
    notifyListeners();
    
    // Start upload with enhanced error handling
    _uploadSingleFileWithEnhancedTracking(imageFile, recipeId).then((uploadedUrl) {
      if (_uploadsCanceled || _disposed) {
        AppLogger.info('📦 Upload completed but manager disposed/cancelled: $filePath');
        return;
      }
      
      if (uploadedUrl != null) {
        AppLogger.info('✅ Background upload completed: $filePath -> $uploadedUrl');
        ...
        final wasRetry = currentState?.retryAttempts != null && currentState!.retryAttempts > 0;
        if (wasRetry) {
      ...
      if (_uploadsCanceled || _disposed) {
        AppLogger.info('📦 Upload error but manager disposed/cancelled: $filePath');
        return;
      }
  ...
  void _handleUploadFailure(String filePath, File imageFile, String errorMessage, dynamic error) {
    ...
    AppLogger.info('🔄 UPLOAD_FAILURE: $filePath failed with ${errorType.name} (attempt ${currentAttempts + 1}/$maxAttempts)');
    ...
    if (failedStatus.canRetry && _shouldAutoRetry(errorType)) {
      ...
      AppLogger.info('🔄 RETRY_SCHEDULED: Auto-retrying $filePath in ${retryDelay.inSeconds}s (${currentAttempts + 1}/$maxAttempts)');
      ...
      Future.delayed(retryDelay).then((_) {
        if (!_uploadsCanceled && !_disposed) {
          ...
          if (latestStatus?.canRetry == true && latestStatus?.errorType == errorType) {
            retryFailedUpload(filePath);
          } else {
            AppLogger.info('🔄 RETRY_CANCELLED: Conditions changed for $filePath');
          }
        }
      });
    } else {
      final strategy = _retryStrategies[errorType];
      AppLogger.info('🔄 NO_AUTO_RETRY: $filePath - ${strategy?.description ?? "No retry strategy"}');
    }
  }
  ...
  bool _shouldAutoRetry(ImageUploadErrorType errorType) {
    ...
    if (circuitBreakerState.shouldReset(_circuitBreakerResetTime)) {
      ...
      AppLogger.info('🔄 CIRCUIT_BREAKER: Reset ${errorType.name} circuit breaker after timeout');
    }
    
    // Check if circuit is open (too many failures)
    if (_circuitBreakers[errorType]?.isOpen == true) {
      AppLogger.warning('🔄 CIRCUIT_BREAKER: ${errorType.name} circuit is OPEN - blocking auto-retry');
      return false;
    }
    
    AppLogger.info('🔄 RETRY_STRATEGY: ${errorType.name} -> ${strategy.description} (auto: ${strategy.autoRetry}, circuit: CLOSED)');
    return true;
  }
  ...
  CircuitBreakerState _getCircuitBreakerState(ImageUploadErrorType errorType) {
    return _circuitBreakers[errorType] ?? const CircuitBreakerState(
      failureCount: 0,
      isOpen: false,
    );
  }
  ...
  void _updateCircuitBreakerOnFailure(ImageUploadErrorType errorType) {
    ...
    if (newState.isOpen && !currentState.isOpen) {
      AppLogger.warning('🔄 CIRCUIT_BREAKER: ${errorType.name} circuit OPENED after ${newState.failureCount} failures');
    }
  }
  
  /// Reset circuit breakers on successful upload (recovery)
  void _resetCircuitBreakerOnSuccess() {
    final hadOpenCircuits = _circuitBreakers.values.any((state) => state.isOpen);
    
    if (hadOpenCircuits) {
      // Reset all circuit breakers on success to allow normal operation
      _circuitBreakers.clear();
      AppLogger.info('🔄 CIRCUIT_BREAKER: All circuits RESET on successful upload');
    }
  }
  ...
  Future<void> retryAllFailedUploads() async {
    ...
    AppLogger.info('🔄 BULK_RETRY: Starting bulk retry for ${failedItems.length} failed uploads');
  ...
  void cancelAllActiveUploads() {
    ...
    AppLogger.info('🔄 BULK_CANCEL: Cancelling ${activeItems.length} active uploads');
  ...
  void clearAllFailedUploads() {
    ...
    AppLogger.info('🔄 BULK_CLEAR: Clearing ${failedItems.length} failed uploads');
  ...
  Map<String, dynamic> get uploadManagementSummary {
    ...
    final completed = _imageStates.values.where((s) => s.state == ImageUploadState.completed).length;
  ...
  static Stream<UploadNotificationEvent> get notificationStream => _notificationController.stream;
  ...
  void _sendNotificationEvent(UploadNotificationEvent event) {
    if (!_canSendNotification()) {
      AppLogger.debug('🔔 NOTIFICATION: Skipping notification due to cooldown: ${event.trigger.name}');
      return;
    }
    ...
    AppLogger.info('🔔 NOTIFICATION: Sent ${event.trigger.name} notification: ${event.message}');
  }
  
  /// Check and trigger progress milestone notifications
  void _checkProgressMilestones(double previousProgress, double currentProgress) {
  ...
  void _triggerMajorFailureNotification(int failedCount, int totalCount) {
    ...
    if (failedCount >= 2 || (totalCount > 1 && failedCount >= totalCount / 2)) {
      _sendNotificationEvent(UploadNotificationEvent(
        ...
        message: '$failedCount av $totalCount bilder misslyckades - kontrollera din internetanslutning',
  ...
  Future<String?> _uploadSingleFileWithEnhancedTracking(File imageFile, String recipeId) async {
    final filePath = imageFile.path;
    final fileSize = imageFile.lengthSync();
      ...
      final uploadedUrl = await _storageService.uploadRecipeImage(
        ...
        onProgress: (progress) {
          ...
          if (lastProgressUpdate != null && lastBytesTransferred != null) {
            final timeDiff = now.difference(lastProgressUpdate!).inMilliseconds / 1000.0;
  ...
  Future<bool> _validateImageFile(File imageFile) async {
    try {
      ...
      const maxSizeInBytes = 20 * 1024 * 1024; // 20MB (generous limit before compression)
  ...
  void _doSafeNotification() {
    if (!_disposed && !_isNotifying) {
      ...
      try {
        notifyListeners();
        AppLogger.debug('🔔 UI notification sent - images: ${imageUrls.length}, pending: ${pendingImages.length}');
      } catch (e) {
  ...
  UploadSafetyResult ensureUploadSafety() {
    ...
    AppLogger.info('Upload safety check: Safe=$isSafe, Pending=${pendingImagePaths.length}, Failed=${failedImagePaths.length}');
  ...
  Future<UploadChoice?> showUploadChoiceDialog(BuildContext context, UploadSafetyResult safetyResult) async {
    ...
    if (hasFailedUploads && hasPendingUploads) {
      ...
      dialogContent = 'Några bilder kunde inte laddas upp (${safetyResult.failedImagePaths.length}) och andra laddas fortfarande upp (${safetyResult.pendingImagePaths.length}).\n\nVad vill du göra?';
    } else if (hasFailedUploads) {
      dialogTitle = 'Bilduppladdning misslyckades';
      dialogContent = '${safetyResult.failedImagePaths.length} bilder kunde inte laddas upp.\n\nVad vill du göra?';
    } else {
      dialogTitle = 'Bilduppladdning pågår';
      dialogContent = '${safetyResult.pendingImagePaths.length} bilder laddas fortfarande upp.\n\nVad vill du göra?';
    }
    ...
    actions.addAll([
      ...
      TextButton(
        onPressed: () => Navigator.of(context).pop(UploadChoice.saveWithoutPending),
        style: TextButton.styleFrom(foregroundColor: Colors.orange),
        child: Text(hasFailedUploads ? 'Spara utan misslyckade bilder' : 'Spara utan väntande bilder'),
      ),
    ]);
  ...
  Future<bool> waitForUploadsToComplete({Duration? timeout}) async {
    ...
    AppLogger.info('Waiting for uploads to complete (timeout: ${timeoutDuration.inSeconds}s)');
    ...
    try {
      ...
      final uploadOperations = pendingImages.map((file) {
        final operation = _CancellableUploadOperation(
          ...
          uploadFunction: (file, recipeId) => _uploadSingleImageWithTracking(file, recipeId, () {}),
        );
      ...
      final successfulUrls = results.where((url) => url != null).cast<String>().toList();
  ...
  Future<String?> _uploadSingleImageWithTracking(File file, String recipeId, VoidCallback? onComplete) async {
    ...
    try {
      ...
      if (_imageStates.containsKey(filePath)) {
        _imageStates[filePath] = _imageStates[filePath]!.copyWith(
          state: ImageUploadState.uploading
        );
      ...
      if (_disposed || _uploadsCanceled) {
        AppLogger.info('☁️ Upload canceled after completion for ${file.path} - ignoring result');
        return null;
      }
      ...
      if (_imageStates.containsKey(filePath) && !_disposed && !_uploadsCanceled) {
        _imageStates[filePath] = _imageStates[filePath]!.copyWith(
          state: ImageUploadState.failed,
          error: e.toString()
        );
  ...
  Map<String, dynamic> get uploadProgress {
    final total = _imageStates.length;
    final completed = _imageStates.values.where((s) => s.state == ImageUploadState.completed).length;
    final uploading = _imageStates.values.where((s) => s.state == ImageUploadState.uploading).length;
    final pending = _imageStates.values.where((s) => s.state == ImageUploadState.pending).length;
  ...
  Duration _calculateRetryDelay(int attemptNumber) {
    ...
    final finalDelay = Duration(seconds: max(jitteredDelay, 1)); // Minimum 1 second
    
    AppLogger.info('🔄 RETRY_BACKOFF: Attempt $attemptNumber -> ${finalDelay.inSeconds}s delay (base: ${maxDelaySeconds}s, jitter: ${jitterFactor.toStringAsFixed(2)})');
  ...
  Future<void> retryFailedUpload(String filePath) async {
    ...
    if (status == null || !status.canRetry) {
      AppLogger.warning('Cannot retry upload for $filePath - status: ${status?.state}');
      return;
    }
    
    AppLogger.info('🔄 Retrying upload for: $filePath (attempt ${status.retryAttempts + 1}/${status.maxRetryAttempts})');
    ...
    _imageStates[filePath] = status.copyWith(
      ...
      nextRetryAt: DateTime.now().add(_calculateRetryDelay(status.retryAttempts)),
    );
  ...
  Map<String, dynamic> get uploadQueueSummary {
    ...
    for (final status in _imageStates.values) {
      ...
      if (status.uploadStartTime != null) {
        if (earliestStartTime == null || status.uploadStartTime!.isBefore(earliestStartTime)) {
    ...
    final progressPercentage = total > 0 ? ((overallProgress) * 100).round() : 100;
    ...
    return {
      ...
      'statusText': _getEnhancedQueueStatusText(pending, active, completed, failed, total, overallProgress),
      'progressText': _getProgressDisplayText(overallProgress, estimatedTimeRemaining),
      'speedText': _getSpeedDisplayText(averageSpeedBytesPerSecond),
      'summaryText': _getQueueSummaryText(completed, failed, total, totalElapsedTime),
    };
  }
  
  /// Enhanced queue status text with progress information
  String _getEnhancedQueueStatusText(int pending, int active, int completed, int failed, int total, double overallProgress) {
    if (total == 0) return '';
  ...
  String _getQueueSummaryText(int completed, int failed, int total, Duration? elapsedTime) {
    if (total == 0) return '';
    
    final successRate = total > 0 ? ((completed / total) * 100).round() : 0;
    
    if (completed == total && failed == 0) {
      final timeText = elapsedTime != null ? ' på ${_formatDuration(elapsedTime)}' : '';
  ...
  void cancelAllUploads() {
    ...
    AppLogger.info('🛑 Thread-safe canceling ${_activeUploads.length} active uploads');
...
class _UploadOperationGroup {
  ...
  int get activeCount => _operations.where((op) => !op.isCompleted && !op.isCancelled).length;
