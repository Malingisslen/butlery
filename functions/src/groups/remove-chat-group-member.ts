...
/**
 * BUT-2005 moved these to `group-menu-access.ts` with the function that owns
 * them. Re-exported because `deleteGroupMenuPlans` below still bounds its own
 * scan by `MAX_GROUP_MENU_PLANS`, and because `weekly-menu-plans-rules.test.ts`
 * imports through this module to compare the rules-backed caps against the
 * rules text — without that comparison a drift in the TypeScript copy alone is
 * invisible, since the Dart-vs-rules guard cannot see a TypeScript literal.
 */
export {
  MAX_GROUP_MENU_PLANS,
  MAX_TRAIL_ROWS,
