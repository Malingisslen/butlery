...
### 2026-09-05 — BUT-1917 step 2+3 re-review: the block mirror's reconciliation inherits none of the trigger's guards
Second pass after three blocking findings were applied (reconciliation wired as a `WEEKLY_REPORT_TASKS` task rather than its own `onSchedule`; the `block_mirror/blockedByUserIds` COLLECTION+COLLECTION_GROUP `arrayConfig: CONTAINS` fieldOverride added; `rebuildMirrorFor` now reads `users/{uid}` inside the transaction and DELETES an orphaned mirror when the owner is gone). All three verified applied and correct.

One new blocking finding, and it is the same shape as the one the file already documents about itself. `blockedIdFromEvent` refuses `"."`, `".."`, anything containing `/`, `__x__` and >1500 bytes, with a comment stating that one client write of `blocks/{myUid}_.` would otherwise make `mirrorRef` throw INVALID_ARGUMENT forever under `retry: true`. `collectUidsToReconcile` reads `blockedId` straight off every `blocks` document with NO such check, and `reconcileMirrors` calls `mirrorRef(db, uid)` on it with no per-uid `try`. Reachability confirmed against `firestore.rules:2474` — the create rule pins `blockerId == auth.uid` and `blockId == blockerId + '_' + blockedId` and says nothing about the SHAPE of `blockedId`, and `uid_.` is a legal document id — so any authenticated account can plant the row. Effect: the weekly reconciliation throws on that uid and never reaches the rest of the `Set`, every Monday, indefinitely. It is loud in the sense that the chain records the task failed; it is silent in the sense that nothing repairs the mirrors, and the mirror is the safety net for a control whose failure shows on no screen. Fix: hoist the segment predicate out of `blockedIdFromEvent` and apply it in `collectUidsToReconcile`, AND wrap each uid's work in the loop so one bad or contending row cannot abandon the pass.

Verified clean this round: the residual probe's cross-user `collectionGroup(block_mirror).where("blockedByUserIds","array-contains",uid).count()` leg sits beside the poll-vote one with its own `try`, fails CLOSED on a missing index (FAILED_PRECONDITION -> `residual += 1`), and is pinned by `scenario_probeSeesLeftoverBlockMirrors` with a clean control; `deleteBlockMirrors` uses `arrayRemove` per document rather than rebuilding another user's document, stamps `sourceRev: Date.now()` so an in-flight trigger rebuild loses the compare, DECLINES above `MAX_MIRROR_SWEEP_ROWS` rather than truncating, and runs sequentially AFTER tier 1 so `deleteBlocks` has committed first; the own-mirror half is covered by `deleteUserSubcollections`' `block_mirror` entry plus the `listCollections()` enumeration; `EXPORT_EXEMPT.block_mirror` is bound to the export by `scenario_blockMirrorExemptionRestsOnIncomingBlocks`; the index entry is surgical (19 TTL fieldOverrides and 53 `indexes` untouched).

Two questions answered rather than filed. (1) The explicit `match /users/{userId}/block_mirror/{doc} { allow read, write: if false; }` may wait for step 4: measured that `match /users/{userId}` at line 329 carries no nested `{document=**}`, no `match /{path=**}/block_mirror/...` collection-group wildcard exists, and the terminal `match /{document=**} { allow read, write: if false; }` catches the path — so no client can read or write the mirror today and the unforgeability claim holds. The comments saying "the rules deny every client write" are therefore TRUE as written and must not be reworded. The load-bearing note for step 4: the block must NOT grant the OWNER read either, because the mirror is the list of people who blocked them and a real-time read of it is a safety disclosure the Art. 15 route (BUT-2018) at least gets to decide deliberately. (2) The exemption test's `manager.includes("'incoming_blocks'")` asserts the leg's own call-site key, so "tighten it to assert the leg call" would be the same assertion; the real gap is that BUT-2018 redacting the uids while keeping the key leaves the test green and the exemption's premise ("the bundle reproduces the SAME facts") false. That pin is a content-level one and belongs to BUT-2018 as an acceptance criterion, not to this commit.

### 2026-09-05 — BUT-1917 round 2: the fix for the poison-pill finding introduced a raw-uid log and a false coverage sentence
Round 1's blocking finding is properly closed: the segment predicate is hoisted to an exported `isUsableUidSegment`, both call sites go through it, `collectUidsToReconcile` skips an unusable id instead of letting `mirrorRef` throw, and `reconcileMirrors` wraps each uid in its own `try` with `checked/repaired/skipped/failed`. The `skipped` counter is a genuine improvement over what I asked for — `rebuildMirrorFor` returning false also covers the orphan-delete and dead-owner paths, so folding them into `repaired: 0` had been logging "found no drift" on a run that changed something. The isolation test stages the throw with a `Proxy` over the store and says in its own comment that the fake cannot raise the SDK's INVALID_ARGUMENT, so the segment rejection is proven separately against the predicate — the right split rather than a weakened assertion.

Two new findings, BOTH inside the repair, which is the pattern this repo keeps paying for. (1) The new skip branch logs `doc_id: doc.id`, and a `blocks` document id is `{blockerId}_{blockedId}` — a raw uid in Cloud Logging, on a path a client plants. `functions/src/shared/hash-uid.ts` states the prohibition in its own header ("Raw userIds must not appear in logs — use this hash for correlation") and every other log in the same file uses `hashUid`/`uid_prefix`. (2) `collectUidsToReconcile`'s docstring says "its behaviour is asserted on the emulator lane instead". Measured: `grep -rln` for `collectUidsToReconcile|reconcileMirrors|runReconcileBlockMirrors` across the repo returns only the two production files, the dispatcher registry and the unit test; no `*.integration.test.ts` and no npm script names `block_mirror`. So the wiring line that closes round 1's finding is executed by nothing while a sentence claims a lane that does not exist — strike it rather than re-point it, and note the sibling sentence in the test file ("That belongs on the emulator lane") is PRESCRIPTIVE and stays.

Also seen and non-blocking: the docstring-stranding defect the round claimed to fix recurs twice in the same file (`blockedIdFromEvent`'s docstring now floats above `isUsableUidSegment`'s; `RECONCILE_REV_SKEW_MS`' floats above `MAX_RECONCILE_SOURCE_ROWS`'), so both of those symbols now carry none — a second JSDoc block before one declaration silently orphans the first. `MAX_RECONCILE_SOURCE_ROWS` is declared 160 lines below its use (legal, module init precedes the call). And the partial-pass ERROR from `collectUidsToReconcile` can sit in the same run's logs as `reconciliation found no drift` from `reconcileMirrors`, with `runReconcileBlockMirrors` discarding both — no single verdict for the run.

### 2026-09-05 — BUT-1917 round 3: PASS. Both blocking findings closed, and the strike was a strike
`doc_id: doc.id` -> `doc_id_hash: hashUid(doc.id)`. Verified independently rather than from the brief: all nine `logger` payloads in `sync-block-mirror.ts` now carry only hashes, counts, caps, a boolean and an error NAME — no raw uid, no composite id (the brief said "three logger calls", which is the count of calls carrying an identifier, not of calls; nothing false shipped in the code). The coverage sentence is struck and replaced by its opposite: the function has no unit case "including the `isUsableUidSegment` skip below, which is therefore UNPINNED: reverting it reddens nothing" — confirmed true, since no test imports `collectUidsToReconcile` and reverting the call site compiles. That is the honest state of the line closing round 1's finding, and it is now visible instead of described as covered.

The three Lows also landed: both stranded docstrings reunited (`blockedIdFromEvent` and `RECONCILE_REV_SKEW_MS` carry their own blocks, no orphan floats), `MAX_RECONCILE_SOURCE_ROWS` moved beside `MAX_MIRROR_ENTRIES` above its two uses, and `collectUidsToReconcile` now returns `{ uids, partial }` so `runReconcileBlockMirrors` emits ONE verdict (`weekly pass: clean` / `NOT clean` with `checked/repaired/skipped/failed/partial`).
