...
### [Pattern] BUT-520 BaseViewModel migration — disposed-guard regression test (2026-05-29)
- **Trigger:** `create_shared_list_viewmodel.dart` migrated raw ChangeNotifier → BaseViewModel; existing 32 tests passed UNMODIFIED. Asked whether the migration's headline benefit (disposed-guarded setError/notifyListeners) was covered.
- **Finding:** Existing error-path assertions (`error equals/contains`, `hasError` toggling) STILL meaningfully cover the migrated code — public surface (`error`/`hasError`/`clearError`) unchanged and now routes through `BaseViewModel`. No weakening needed. BUT no test drove an operation across a `dispose()` boundary, so the migration's central improvement was untested. Added one minimal behavioral test.
