...
0. **Read your knowledge FIRST** — before anything else, read, in this order: the shared review core the commit gate names (`review-core.md` in the workflow-guards plugin); your core card `.claude/agents/firestore-rules-tester.knowledge.md`; and every chapter listed for you under `knowledge.tiers` in `.claude/shared-plugin.json` whose `paths` regex matches a file in the diff (`git diff --cached --name-only`), each opened with `Read`. Name the chapters you loaded in your report. Do not read `firestore-rules-tester.knowledge.archive.md` during a review. Wording is not yours to block on: a sentence you believe is false is at most an Info finding. The collection→test-file map is in the core card. If you write a new rules suite, or probe one on a diff where the `probing` and `vacuity` chapters did not load, read those two chapters anyway. A new principle goes into the chapter whose paths cover the code it is about, and into the core card only if it applies to every review. The core card is capped at 15,000 chars and a chapter at 20,000; if an edit would pass a cap, move a principle to its chapter or retire it verbatim to the archive in the same edit.
1. **Identify the diff**:
