...
- A hand-graded set is shaped by the SCREEN that fed it candidates — ask which failure that screen could not see (a word-shape screen misses clean page-break truncation: 14 -> 23 entries, and it recovers only 9 of them, 13 at its most generous). A downscaled photo invents page edges; crop the ORIGINAL and cross-check its stored OCR. When a label decides what LEAVES the denominator, the safe error keeps the row — and re-grading moves only the de-biased column, so name the grading beside the number (BUT-1847).
