...
- A source edit made THROUGH another language inherits that language's escape rules: `\b` in a Python heredoc string writes a literal BACKSPACE into your Dart comment, compiler-invisible, and it recurred three times in one commit because each CORRECTION went through another heredoc. Use `chr(92)+'b'`, then sweep `grep -rlP '[\x00-\x08]' --include='*.dart' --include='*.ts'` — the baseline is ONE known file (`ocr_extraction_service_test.dart`, deliberate fixtures), so anything else is yours, and an earlier clean sweep proves nothing about the next edit (2026-08-19).
...
`, which makes `comm` see no overlap. Do a set comparison in ONE language, `tr -d '
'` before any `comm`/`diff`, and use `mktemp -d`. When the two `comm` directions don't add up, the INPUTS are wrong — stop refining the regex (2026-08-17).
