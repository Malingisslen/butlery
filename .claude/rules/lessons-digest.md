...
- A comment that QUOTES another comment's wording or POSITION ("the header says three", "the two just above") is a dependency on both — fixing the cited text makes the citation false in the SAME commit, which is how one correction spawned two. State the fact; don't point at where it is written. And scope a count to what it counts (2026-08-19).
...
- A source edit made THROUGH another language inherits ITS escape rules: `\b` in a Python heredoc writes a literal BACKSPACE into a Dart comment, compiler-invisible — 4x in one commit: 3 source files AND the commit message, which no file sweep reaches. Use `chr(92)+'b'`, then from the REPO ROOT `git ls-files -z --cached --others --exclude-standard -- '*.dart' '*.ts' '*.md' | xargs -0 -r grep -laP '[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]'` — expect exactly ONE known file (the OCR fixture; `text-integrity.yml` EXCLUDES it and scans nine more extensions, so it expects zero — a clean sweep here does not predict a green guard there); a SECOND is yours. It was two until BUT-1902 repaired the archive the same day — a stale baseline here fails OPEN (2026-08-19).
- A hand-graded set is shaped by the SCREEN that fed it candidates — ask which failure that screen could not see (a word-shape screen misses clean page-break truncation: 14 -> 23 entries; a DIFFERENT, terminal-punctuation screen run over the finished set recovers only 9 of them, 13 at its most generous, while the word-shape one carries 89 false positives and cannot score itself). A downscaled photo invents page edges; crop the ORIGINAL and cross-check its stored OCR. A borderline label that decides what LEAVES the denominator gets argued from a WRITTEN criterion, never from how the transcription reads (one cut landing after a full stop is an accident of the frame) and never from the number it produces — publish the sensitivity instead. Re-grading moves only the de-biased column, so name the grading beside the number, and never edit the measured input underneath a review that is reading it (BUT-1847).
...
- A rule bounding a field's VALUE does not bound a denormalised COPY of it on another collection (a deny-list there won't name it) — and "how long does the bad value persist" is a claim about EVERY writer, server trigger AND client repo: two drafts of one comment said "up to an hour" then "PERMANENT", both wrong, because the client merge-sets the field with no comparison. Your own guard can falsify the paragraph beside it in the same commit, and never cite a rules LINE NUMBER — this edit moved everything below it by 55 lines. And a CORRECTION's supporting detail is as falsifiable as the claim it supports — one paragraph shipped FIVE false ones in a row, each introduced by the fix to the previous; grep every noun in a sentence you just rewrote (BUT-1903).
