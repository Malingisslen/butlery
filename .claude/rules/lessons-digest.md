...
- A source edit made THROUGH another language inherits ITS escape rules: `\b` in a Python heredoc writes a literal BACKSPACE into a Dart comment, compiler-invisible — 4x in one commit: 3 source files AND the commit message, which no file sweep reaches. Use `chr(92)+'b'`, then from the REPO ROOT `git ls-files -z --cached --others --exclude-standard -- '*.dart' '*.ts' '*.md' | xargs -0 -r grep -laP '[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]'` — expect exactly TWO known files (the OCR fixture, and the archive corruption on BUT-1900); a third is yours (2026-08-19).
