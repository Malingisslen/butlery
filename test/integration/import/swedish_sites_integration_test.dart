  ...
  group('Swedish Recipe Sites - Integration Tests', () {
    ...
    group('ICA.se - Complete Recipe with Enhancements', () {
      test('should extract complete ICA recipe with all site-specific fields', () async {
        ...
        expect(result.recipe!.ingredients, hasLength(13),
            reason: 'Should extract all 13 ingredients');
        expect(result.recipe!.ingredients, contains('500 g nötfärs'),
        ...
        expect(result.recipe!.instructions, hasLength(6),
            reason: 'Should extract all 6 instruction steps');
        expect(result.recipe!.instructions.first, contains('Blanda köttfärs'),
        ...
        expect(result.metadata!['extraction_method'], equals('site_specific'),
            reason: 'Should use ICA site parser, not generic RecipeScraper');
        expect(result.metadata!['site_parser'], equals('ica.se'),
      ...
      test('should extract ICA cooking tips', () async {
        ...
        expect((result.metadata!['cookingTips'] as List), isNotEmpty,
            reason: 'Should have at least one cooking tip');

        final tips = result.metadata!['cookingTips'] as List;
        expect(tips.first.toString(), contains('Låt smeten svälla'),
    ...
    group('ICA.se - Swedish Text Handling', () {
      test('should preserve Swedish characters (Å, Ä, Ö) in recipe data', () async {
        ...
        expect(result.recipe!.description, contains('torsdagsmiddagen'),
    ...
    group('ICA.se - Quality Validation', () {
      ...
      test('should accept complete recipe with high quality score', () async {
        ...
        expect(result.recipe!.instructions, hasLength(greaterThan(2)),
            reason: 'Should have 2+ instructions');
        expect(result.recipe!.portions, isNotNull,
            reason: 'Should have portions');
        expect(result.recipe!.timeMinutes, isNotNull,
            reason: 'Should have time');
        expect(result.recipe!.imageUrls, isNotEmpty,
    ...
    group('ICA.se - CSS Fallback When JSON-LD Missing', () {
      test('should extract recipe using CSS selectors when JSON-LD is missing', () async {
        ...
        expect(result.recipe, isNotNull);
        expect(result.recipe!.title, equals('Klassisk kladdkaka'),
        ...
        expect(result.recipe!.instructions, hasLength(greaterThan(3)),
    ...
    group('ICA.se - Formatting Cleanup', () {
      test('should clean ICA-specific formatting quirks', () async {
        ...
        for (final ingredient in ingredients) {
          expect(ingredient, equals(ingredient.trim()),
              reason: 'Ingredient should not have leading/trailing whitespace');
        }

        // Assert - Specific cleaned ingredients
        expect(ingredients.any((ing) => ing.contains('500 g nötfärs')), isTrue,
            reason: 'Should have cleaned ingredient "500 g nötfärs"');
        expect(ingredients.any((ing) => ing.contains('8 tortillabröd')), isTrue,
    ...
    group('Arla.se - Complete Recipe with Enhancements', () {
      test('should extract complete Arla recipe with all site-specific fields', () async {
        ...
        // Assert - Recipe extracted successfully
        expect(result.isSuccess, isTrue, reason: 'Arla import should succeed');
        expect(result.recipe, isNotNull, reason: 'Recipe should be extracted');

        // Assert - Standard recipe fields
        expect(result.recipe!.title, equals('Chokladbollar'),
            reason: 'Title should match JSON-LD data');
        expect(result.recipe!.description, contains('Klassiska chokladbollar'),
            reason: 'Description should be extracted');
        ...
        expect(result.recipe!.timeMinutes, equals(15),
        ...
        expect(result.metadata!['site_parser'], equals('arla.se'),
            reason: 'Should track which site parser was used');
      ...
      test('should extract Arla cooking tips', () async {
        ...
        final tips = result.metadata!['cookingTips'] as List;
        expect(tips.first.toString(), contains('Låt smeten kallna'),
      ...
      test('should extract nutritional information from Arla recipe', () async {
        ...
        expect(result.isSuccess, isTrue);
        expect(result.metadata!['nutrition'], isA<Map>(),
    ...
    group('Arla.se - Swedish Text Handling', () {
      test('should preserve Swedish characters (Å, Ä, Ö) in recipe data', () async {
        ...
        // Assert - Swedish characters preserved
        expect(result.isSuccess, isTrue);
        expect(result.recipe!.title, equals('Äppelpaj med vaniljsås'),
            reason: 'Title should preserve Ä character');
    ...
    group('Arla.se - CSS Fallback', () {
      test('should extract recipe using CSS selectors when JSON-LD is missing', () async {
        ...
        expect(result.isSuccess, isTrue,
            reason: 'Should succeed via CSS selector fallback');
        expect(result.recipe, isNotNull);
        expect(result.recipe!.title, equals('Fluffiga pannkakor'),
            reason: 'Should extract title from CSS selectors');
    ...
    group('Köket.se - Formatting Cleanup', () {
      test('should clean "ca" and "cirka" from portions and ingredients', () async {
        ...
        expect(result.recipe!.ingredients, contains('1 dl vatten'),
    ...
    group('Recept.se - Quality Validation', () {
      test('should accept recipe with complete metadata (high quality)', () async {
        ...
        // Assert - Recipe extracted with high quality
        expect(result.isSuccess, isTrue);
        expect(result.recipe!.title, equals('Grillad lax med citron och dill'));
        expect(result.recipe!.description, isNotEmpty);
        expect(result.recipe!.ingredients.length, greaterThanOrEqualTo(6));
    ...
    group('Recept.se - Formatting Cleanup', () {
      test('should clean "ca" and "cirka" from portions and ingredients', () async {
        ...
        expect(result.recipe!.ingredients, contains('800 g potatis'),
            reason: 'Should clean "ca " prefix from ingredient');
        expect(result.recipe!.ingredients, contains('2 dl mjölk'),
            reason: 'Should clean "cirka " prefix from ingredient');
