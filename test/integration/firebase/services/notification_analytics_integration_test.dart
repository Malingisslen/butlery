...
void main() {
  group('Notification Analytics - Firebase FieldValue Integration', () {
    ...
    test('should handle FieldValue.increment() for analytics counters',
        () async {
    ...
    test('should handle batch operations with FieldValue operations', () async {
      ...
      final statsRef =
          firestore.collection('notification_stats').doc('batch_stats');
      batch.set(statsRef, {
    ...
    test('should handle FieldValue.arrayUnion() for notification categories',
        () async {
      ...
      await firestore
          ...
          .update({
        'enabledCategories':
            FieldValue.arrayUnion(['friend_requests', 'menu_shares']),
        'lastUpdated': FieldValue.serverTimestamp(),
      });
    ...
    test('should handle FieldValue.arrayRemove() for notification categories',
        () async {
      ...
      await firestore
          ...
          .update({
        'enabledCategories':
            FieldValue.arrayRemove(['friend_requests', 'menu_shares']),
        'lastUpdated': FieldValue.serverTimestamp(),
      });
