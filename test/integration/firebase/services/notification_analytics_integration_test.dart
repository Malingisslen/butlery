...
void main() {
  group('Notification Analytics - Firebase FieldValue Integration', () {
    ...
    test('should handle FieldValue.serverTimestamp() for notification events', () async {
      // Act - Create notification event with server timestamp
      await firestore.collection('notification_analytics').doc('test_notif').set({
    ...
    test('should handle FieldValue.increment() for analytics counters', () async {
      // Setup - Create initial analytics document
      await firestore.collection('notification_stats').doc('daily_stats').set({
        'sentCount': 0,
        'deliveredCount': 0,
        'openedCount': 0,
      ...
      await firestore.collection('notification_stats').doc('daily_stats').update({
      ...
      // Assert
      final doc = await firestore.collection('notification_stats').doc('daily_stats').get();
      final data = doc.data()!;
    ...
    test('should handle batch operations with FieldValue operations', () async {
      // Act - Create batch with multiple FieldValue operations
      final batch = firestore.batch();
      
      // Add multiple notification events with server timestamps
      for (int i = 0; i < 3; i++) {
        final ref = firestore.collection('notification_analytics').doc('batch_notif_$i');
        batch.set(ref, {
      ...
      final statsRef = firestore.collection('notification_stats').doc('batch_stats');
      batch.set(statsRef, {
      ...
      final statsDoc = await firestore.collection('notification_stats').doc('batch_stats').get();
      final statsData = statsDoc.data()!;
      expect(statsData['totalSent'], equals(3));
      expect(statsData['lastUpdated'], isA<Timestamp>());
    });

    test('should handle FieldValue.arrayUnion() for notification categories', () async {
      // Setup - Create initial document  
      await firestore.collection('user_notification_prefs').doc(testUserId).set({
        'enabledCategories': ['recipe_comments'],
        'lastUpdated': FieldValue.serverTimestamp(),
      });
      
      // Act - Add new category using arrayUnion
      await firestore.collection('user_notification_prefs').doc(testUserId).update({
        'enabledCategories': FieldValue.arrayUnion(['friend_requests', 'menu_shares']),
        'lastUpdated': FieldValue.serverTimestamp(),
      });
      
      // Assert
      final doc = await firestore.collection('user_notification_prefs').doc(testUserId).get();
      final data = doc.data()!;
      
      final categories = List<String>.from(data['enabledCategories']);
      expect(categories, containsAll(['recipe_comments', 'friend_requests', 'menu_shares']));
      expect(categories.length, equals(3));
      expect(data['lastUpdated'], isA<Timestamp>());
    });

    test('should handle FieldValue.arrayRemove() for notification categories', () async {
      // Setup - Create document with multiple categories
      await firestore.collection('user_notification_prefs').doc(testUserId).set({
        'enabledCategories': ['recipe_comments', 'friend_requests', 'menu_shares', 'recipe_likes'],
        'lastUpdated': FieldValue.serverTimestamp(),
      });
      
      // Act - Remove categories using arrayRemove
      await firestore.collection('user_notification_prefs').doc(testUserId).update({
        'enabledCategories': FieldValue.arrayRemove(['friend_requests', 'menu_shares']),
        'lastUpdated': FieldValue.serverTimestamp(),
      });
    ...
    test('should handle complex query with timestamps and ordering', () async {
      ...
      for (int i = 0; i < 5; i++) {
        await firestore.collection('notification_analytics').doc('timed_notif_$i').set({
