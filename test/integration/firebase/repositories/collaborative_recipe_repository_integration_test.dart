/// Integration tests for Collaborative Recipe Repository with Firebase
/// 
/// Tests actual Firebase operations with emulator including real-time collaboration,
/// presence management, concurrent editing, and conflict resolution scenarios.
@Tags(['integration'])
library;
...
void main() {
  group('Collaborative Recipe Repository Firebase Integration', () {
    ...
    setUp(() async {
      ...
      testUser2 = await FirebaseTestSetup.createTestUser(
    ...
    group('Real-time Recipe Collaboration', () {
      ...
      test('should handle concurrent recipe updates', () async {
        ...
        futures.add(repository.updateRealtimeRecipe(
          realtimeRecipe.copyWith(
            recipe: baseRecipe.copyWith(ingredients: ['New Ingredient 1', 'New Ingredient 2']),
            editCount: 1,
          ),
        ));
      ...
      test('should stream recipe changes in real-time', () async {
        ...
        final subscription = repository
            ...
            .listen((recipe) => updates.add(recipe));
        ...
        expect(updates.last?.recipe.title, equals('Update 3'));
        expect(updates.last?.editCount, equals(3));
    ...
    group('Presence Management with Firebase', () {
      test('should manage multiple user presence with server timestamps', () async {
        // Arrange
        ...
        expect(presence1.data()?['lastSeen'], isA<Timestamp>());
      ...
      test('should stream active presence changes in real-time', () async {
        ...
        // Setup presence stream listener
        final subscription = repository
        ...
        // Assert
        expect(presenceUpdates.length, greaterThanOrEqualTo(4));
      ...
      test('should track participants as LiveEditor objects', () async {
        ...
        expect(participants.map((e) => e.userId), containsAll(['editor-1', 'editor-2']));
        expect(participants.map((e) => e.displayName), containsAll(['Editor One', 'Editor Two']));
      });
      
      test('should handle rapid heartbeat updates efficiently', () async {
        // Arrange
        const recipeId = 'heartbeat-recipe';
        await repository.updateUserPresence(recipeId, testUser.uid, 'Test User');
        ...
        for (int i = 0; i < 20; i++) {
          ...
          await Future.delayed(const Duration(milliseconds: 50)); // 50ms intervals
        }
        
        await Future.wait(heartbeatFutures);
        
        // Assert - User should still be active
        final isActive = await repository.isUserActivelyEditing(recipeId, testUser.uid);
    ...
    group('Cleanup and Maintenance with Firebase', () {
      test('should cleanup inactive editors with batch operations', () async {
        ...
        // Active editor
        batch.set(
          firestore.collection('realtime_recipes')
              .doc(recipeId)
              .collection('presence')
              .doc('active-editor'),
          {
    ...
    group('Complex Collaboration Scenarios', () {
      ...
      test('should handle recipe session lifecycle', () async {
        ...
        final activeEditors = await repository.getActiveEditors(recipeId);
        expect(activeEditors.where((e) => e['isCurrentlyActive'] == true), isEmpty);
      });
      
      test('should handle concurrent presence updates without conflicts', () async {
        // Arrange
    ...
    group('Performance and Scalability', () {
      ...
      test('should handle rapid recipe updates without data loss', () async {
        ...
        final finalIngredients = finalDoc.data()?['recipe']?['core']?['ingredients'] as List;
    ...
    group('Error Recovery and Edge Cases', () {
      test('should recover from network interruptions gracefully', () async {
        ...
        final isActive1 = await repository.isUserActivelyEditing(recipeId, testUser.uid);
        
        // Wait and query again
        await Future.delayed(const Duration(milliseconds: 500));
        final isActive2 = await repository.isUserActivelyEditing(recipeId, testUser.uid);
      ...
      test('should handle malformed data gracefully', () async {
        ...
        // Create presence with unusual data
        await firestore
            .collection('realtime_recipes')
            .doc(recipeId)
            .collection('presence')
        ...
        // Assert - Should return but with safe defaults
        expect(editors, hasLength(1));
        expect(editors.first['displayName'], equals('Unknown'));
        expect(editors.first['isActive'], isFalse);
      });
      ...
      test('should handle recipe deletion during active session', () async {
        ...
        await repository.createRealtimeRecipe(realtimeRecipe);
        await repository.updateUserPresence(recipeId, testUser.uid, 'Active User');
