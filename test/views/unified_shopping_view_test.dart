...
void main() {
  group('UnifiedShoppingView Integration Tests', () {
    ...
    group('Provider Setup Tests', () {
      testWidgets('should initialize UnifiedShoppingViewModel provider correctly', (tester) async {
        ...
        final shoppingViewModel = Provider.of<UnifiedShoppingViewModel>(context, listen: false);
        
        expect(shoppingViewModel, isNotNull);
        expect(shoppingViewModel.isInitialized, isTrue);
      });
      
      testWidgets('should render shopping interface with Swedish localization', (tester) async {
        // Arrange
        final providers = ProviderTestUtils.setupShoppingProviders();
        ...
        ViewTestHelpers.expectSwedishText(tester, 'Lägg till vara'); // Floating action button
        
        // Verify tooltips and action buttons are present (content validated in facade tests)
        expect(find.byType(FloatingActionButton), findsOneWidget);
        expect(find.byType(Consumer<UnifiedShoppingViewModel>), findsAtLeast(1));
      });
    });
    ...
    group('Facade Component Delegation Tests', () {
      testWidgets('should render all facade components correctly', (tester) async {
        // Arrange: Setup realistic shopping data
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel(
          lists: WidgetMockFactory.createTestShoppingLists(count: 2),
          isCollaborative: false,
        );
        
        final providers = [
          ChangeNotifierProvider<UnifiedShoppingViewModel>.value(value: testableViewModel),
        ];
        ...
        expect(find.byIcon(Icons.people_outline), findsOneWidget); // Share with friends
        ...
        expect(find.descendant(of: fab, matching: find.text('Lägg till vara')), findsOneWidget);
      ...
      testWidgets('should show online/offline sync status correctly', (tester) async {
        ...
        // Test offline state
        testableViewModel.setShoppingState(isLoading: true); // Simulate offline condition
        await tester.pump();
      ...
      testWidgets('should handle empty shopping list state correctly', (tester) async {
        ...
        expect(find.byType(FloatingActionButton), findsOneWidget); // Add button still available
        
        // Share buttons should be disabled when no items
        final shareButtons = tester.widgetList<IconButton>(find.byType(IconButton));
    ...
    group('Event Handler Integration Tests', () {
      testWidgets('should handle floating action button tap for item addition', (tester) async {
      ...
      testWidgets('should handle share button state based on items availability', (tester) async {
      ...
      testWidgets('should display correct sync status icon based on online state', (tester) async {
      ...
      testWidgets('should handle create list action correctly', (tester) async {
        ...
        final createButton = find.byIcon(Icons.add).first; // First add icon is create list
        ...
        expect(find.byIcon(Icons.add), findsAtLeast(1)); // Button remains available
      });
    });
    ...
    group('Lifecycle Management Tests', () {
      testWidgets('should initialize ViewModel correctly during widget creation', (tester) async {
        ...
        final context = tester.element(find.byType(UnifiedShoppingView));
        final viewModel = Provider.of<UnifiedShoppingViewModel>(context, listen: false);
        expect(viewModel, isNotNull);
        expect(viewModel, equals(testableViewModel));
      });
      
      testWidgets('should handle widget disposal without memory leaks', (tester) async {
      ...
      testWidgets('should handle Consumer state updates correctly', (tester) async {
    ...
    group('Swedish Localization Tests', () {
      testWidgets('should display all Swedish UI elements correctly', (tester) async {
      ...
      testWidgets('should handle Swedish shopping item names correctly', (tester) async {
        // Arrange: Create shopping list with Swedish item names
        final swedishShoppingList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
    ...
    group('Error Handling Tests', () {
      testWidgets('should handle empty provider gracefully', (tester) async {
        ...
        expect(find.byType(UnifiedShoppingView), findsOneWidget);
        expect(find.byType(FloatingActionButton), findsOneWidget); // Still available for adding
      });
      
      testWidgets('should handle rapid state changes without errors', (tester) async {
        // Arrange: Setup testable ViewModel
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel();
    ...
    group('Complete Shopping Workflow Tests', () {
      testWidgets('should complete full item addition workflow with success feedback', (tester) async {
        ...
        expect(find.byType(FloatingActionButton), findsOneWidget);
        expect(testableViewModel.isProperlyDisposed, isFalse); // ViewModel still active
      });
      
      testWidgets('should complete external sharing workflow with ShareService integration', (tester) async {
        // Arrange: Setup shopping ViewModel with items for sharing
        final shoppingList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel(
          lists: [shoppingList],
        );
        ...
        expect(testableViewModel.activeList!.items, isNotEmpty); // Items available for sharing
      });
      
      testWidgets('should complete list creation workflow with success confirmation', (tester) async {
      ...
      testWidgets('should complete bulk operations workflow with item state changes', (tester) async {
    ...
    group('Dialog Integration Workflow Tests', () {
      testWidgets('should handle shopping dialog workflows with proper state management', (tester) async {
        ...
        testableViewModel.setShoppingState(activeList: testableViewModel.activeList);
        await tester.pump();
        ...
        expect(find.byType(UnifiedShoppingView), findsOneWidget);
        expect(testableViewModel.lists, isNotEmpty); // Lists available for dialog operations
      });
      
      testWidgets('should handle sync status dialog with collaborative information', (tester) async {
        // Arrange: Setup collaborative shopping state
        final collaborativeList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel(
        ...
        expect(testableViewModel.lists, isNotEmpty); // Lists available for sync status
      });
      
      testWidgets('should handle confirmation dialogs for destructive operations', (tester) async {
        ...
        expect(testableViewModel.activeList, isNotNull); // Active list for operations
        expect(testableViewModel.lists, isNotEmpty); // Lists available for bulk operations
      });
    });
    ...
    group('External Service Integration Tests', () {
      testWidgets('should integrate with ShareService for external sharing workflows', (tester) async {
        // Arrange: Setup shareable shopping content
        final shareableList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel(
          lists: [shareableList],
        );
        testableViewModel.setShoppingState(
        ...
        final shareButtonWidget = tester.widget<IconButton>(externalShareButton);
        expect(shareButtonWidget.onPressed, isNotNull); // Button should be enabled
        
        // Assert: Verify ShareService integration readiness
        expect(testableViewModel.activeList, isNotNull); // Active list for sharing
        expect(testableViewModel.activeList!.items, isNotEmpty); // Items available for sharing
        ...
        final disabledShareButton = tester.widget<IconButton>(find.byIcon(Icons.share));
        expect(disabledShareButton.onPressed, isNull); // Button should be disabled when no activeList
      });
      
      testWidgets('should handle social sharing integration through friends system', (tester) async {
        // Arrange: Setup social sharing scenario
        final socialList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel(
          lists: [socialList],
        );
        testableViewModel.setShoppingState(
        ...
        expect(socialButtonWidget.onPressed, isNotNull); // Button should be enabled
        
        // Assert: Verify social integration readiness
        expect(testableViewModel.activeList, isNotNull); // List available for collaboration
        expect(testableViewModel.activeList!.items, isNotEmpty); // Content available for social sharing
      });
      
      testWidgets('should integrate with SnackBarUtils for user feedback workflows', (tester) async {
        ...
        expect(scaffold, findsOneWidget); // Scaffold available for SnackBar presentation
        
        // Assert: Verify feedback system integration readiness
        expect(find.byType(UnifiedShoppingView), findsOneWidget);
        expect(testableViewModel.isProperlyDisposed, isFalse); // ViewModel active for feedback
      });
    });
    ...
    group('Production Bug Detection and Edge Case Tests', () {
      testWidgets('should detect potential external sharing inconsistency bug', (tester) async {
        ...
        expect(shareButtonWidget.onPressed, isNull); // Should be null when no items
      ...
      testWidgets('should handle empty shopping list edge cases correctly', (tester) async {
        ...
        for (final button in shareButtons) {
          if (button.icon is Icon) {
            ...
            if (icon.icon == Icons.share || icon.icon == Icons.people_outline) {
              expect(button.onPressed, isNull, reason: 'Share buttons should be disabled when no items');
    ...
    group('User Feedback and Message Validation Tests', () {
      testWidgets('should handle Swedish localized feedback messages throughout workflows', (tester) async {
        // Arrange: Setup comprehensive Swedish localization test
        final swedishList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel(
          lists: [swedishList],
        );
      ...
      testWidgets('should validate success and error feedback message integration', (tester) async {
        ...
        expect(actionButtons, findsAtLeast(1)); // Action buttons for various feedback scenarios
        
        // Assert: Verify feedback system integration
        expect(find.byType(UnifiedShoppingView), findsOneWidget);
        expect(testableViewModel.isProperlyDisposed, isFalse); // ViewModel ready for feedback operations
      });
      
      testWidgets('should handle workflow error scenarios with proper user guidance', (tester) async {
        // Arrange: Setup error scenario testing
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel();
        // Set up error state for testing error workflows
        testableViewModel.setError('Nätverksfel - kunde inte synkronisera'); // Swedish error message
        ...
        expect(testableViewModel.hasError, isTrue);
        expect(testableViewModel.isOnline, isFalse); // Error should affect online status
        ...
        expect(find.byType(FloatingActionButton), findsOneWidget); // FAB still available for recovery
      });
    });
    ...
    group('Advanced State Management Tests', () {
      testWidgets('should handle provider state transitions efficiently', (tester) async {
        ...
        expect(find.byType(UnifiedShoppingView), findsOneWidget); // UI remains stable
        
        // Verify performance characteristics
        final transitionDuration = testableViewModel.getLastTransitionDuration();
        expect(transitionDuration, isNotNull); // Performance tracking active
      });
      
      testWidgets('should maintain consistent provider state across Consumer rebuilds', (tester) async {
        ...
        final context = tester.element(find.byType(UnifiedShoppingView));
        final viewModel = Provider.of<UnifiedShoppingViewModel>(context, listen: false);
        expect(viewModel, equals(testableViewModel)); // Same instance throughout
        expect(viewModel.lists, hasLength(3)); // Final state is correct
      });
      
      testWidgets('should optimize rendering performance with selective Consumer updates', (tester) async {
        ...
        expect(stopwatch.elapsedMilliseconds, lessThan(1000)); // Reasonable performance
        expect(find.byType(UnifiedShoppingView), findsOneWidget); // UI stable after updates
        expect(find.byType(Consumer<UnifiedShoppingViewModel>), findsAtLeast(1)); // Consumers active
      });
      
      testWidgets('should handle provider state synchronization with external services', (tester) async {
        ...
        final syncedList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
        testableViewModel.setShoppingState(
          isSyncing: false,
          lists: [syncedList],
    ...
    group('Performance Optimization Tests', () {
      testWidgets('should minimize unnecessary widget rebuilds during state changes', (tester) async {
        ...
        expect(rebuildCount, equals(initialRebuildCount)); // UnifiedShoppingView itself doesn't rebuild unnecessarily
        expect(find.byType(UnifiedShoppingView), findsOneWidget); // View remains stable
        
        // Consumers should handle selective updates
        expect(find.byType(Consumer<UnifiedShoppingViewModel>), findsAtLeast(1));
      });
      
      testWidgets('should handle large data sets efficiently without performance degradation', (tester) async {
        // Arrange: Setup large dataset scenario
        final largeShoppingLists = WidgetMockFactory.createTestShoppingLists(count: 10);
        final testableViewModel = TestableViewModelFactory.createShoppingViewModel(
        ...
        expect(stopwatch.elapsedMilliseconds, lessThan(2000)); // Reasonable render time
        expect(find.byType(UnifiedShoppingView), findsOneWidget);
        ...
        expect(switchStopwatch.elapsedMilliseconds, lessThan(500)); // Fast list switching
      });
      
      testWidgets('should optimize memory usage during prolonged shopping sessions', (tester) async {
        ...
        for (int i = 0; i < 10; i++) {
          ...
          final tempList = WidgetMockFactory.createTestShoppingLists(count: 1).first;
        ...
        expect(find.byType(UnifiedShoppingView), findsOneWidget); // View stable after prolonged use
        expect(testableViewModel.isProperlyDisposed, isFalse); // Still properly managing lifecycle
      ...
      testWidgets('should maintain responsive UI during intensive shopping operations', (tester) async {
    ...
    group('Advanced Lifecycle Management Tests', () {
      testWidgets('should handle proper initialization sequence with dependency resolution', (tester) async {
        ...
        expect(find.byType(UnifiedShoppingView), findsOneWidget);
        expect(testableViewModel.isProperlyDisposed, isFalse); // Properly initialized
      ...
      testWidgets('should ensure proper disposal sequence preventing memory leaks', (tester) async {
        ...
        expect(find.text('Different View'), findsOneWidget); // New content present
      ...
      testWidgets('should handle view recreation after disposal without state corruption', (tester) async {
        ...
        expect(testableViewModel.isProperlyDisposed, isFalse); // Properly recreated
      });
      
      testWidgets('should maintain provider context consistency during view state changes', (tester) async {
        ...
        final initialViewModel = Provider.of<UnifiedShoppingViewModel>(initialContext, listen: false);
        ...
        final finalViewModel = Provider.of<UnifiedShoppingViewModel>(finalContext, listen: false);
        
        expect(initialViewModel, equals(finalViewModel)); // Same ViewModel instance
        expect(finalViewModel, equals(testableViewModel)); // Matches test instance
