...
void main() {
  ...
  group('RecipeFormState - Ultrathink Enhanced Tests', () {
    ...
    final testImageUrls = ['https://example.com/image1.jpg', 'https://example.com/image2.png'];
    final testIngredients = ['500g köttfärs', '1 ägg', 'Ströbröd'];
    final testInstructions = ['Blanda köttfärsen', 'Forma kulor', 'Stek i panna'];
    ...
    group('Initialization and Default Values', () {
      test('should initialize with correct default values', () {
        ...
        expect(formState.instructions, equals([''])); // Default empty instruction
      ...
      test('should initialize FormFieldsManagers correctly', () {
        ...
        expect(formState.ingredientsManager.length, equals(1)); // One empty field
        expect(formState.instructionsManager.length, equals(1)); // One empty field
    ...
    group('Initialization with Initial Recipe', () {
      ...
      test('should handle recipe with empty collections correctly', () {
        ...
        // Assert
        expect(formState.ingredients, equals([''])); // Should default to empty field
        expect(formState.instructions, equals([''])); // Should default to empty field
      ...
      test('should handle template mode correctly', () {
        ...
        // Assert
        expect(formState.originalRecipe, isNull); // Should not set original when template
        expect(formState.isEditing, isFalse); // Template mode is not editing
        expect(formState.title, equals(testTitle)); // Data should still be loaded
        expect(formState.description, equals(testDescription));
      });
      ...
      test('should synchronize FormFieldsManagers with loaded data', () {
        ...
        expect(formState.ingredientsManager.length, equals(testIngredients.length));
        expect(formState.instructionsManager.length, equals(testInstructions.length));
    ...
    group('Basic Property Setters', () {
      ...
      test('should handle empty title', () {
        ...
        // Assert
        expect(formState.title, equals(''));
        expect(formState.isValid, isFalse); // Empty title should make form invalid
      });
    ...
    group('Image URL Management', () {
      ...
      test('should create defensive copy of image URLs', () {
        ...
        // Assert
        expect(formState.imageUrls, equals(testImageUrls)); // Should not be affected
      ...
      test('should respect maximum image limit when adding', () {
        ...
        expect(formState.imageUrls, isNot(contains('https://example.com/extra-image.jpg')));
      });
      ...
      test('should handle removing non-existent image URL', () {
        ...
        // Assert
        expect(formState.imageUrls, equals(testImageUrls)); // Should remain unchanged
      });
      ...
      test('should handle moving already-first image', () {
        ...
        expect(formState.imageUrls, equals(testImageUrls)); // Order should remain the same
      });
    });
    ...
    group('State Management Operations', () {
      ...
      test('should handle setting empty error', () {
        ...
        expect(formState.hasError, isTrue); // Empty string is still considered an error
      });
    });
    ...
    group('Form Validation', () {
      ...
      test('should handle multiple ingredients with some empty', () {
        ...
        formState.ingredientsManager.updateItems(['', 'Valid ingredient', '   ']);
        
        // Assert
        expect(formState.isValid, isTrue); // Should be valid because one ingredient is non-empty
      });
      ...
      test('should handle multiple instructions with some empty', () {
        ...
        formState.instructionsManager.updateItems(['', 'Valid instruction', '   ']);
        
        // Assert
        expect(formState.isValid, isTrue); // Should be valid because one instruction is non-empty
      });
    });
    ...
    group('Recipe Creation', () {
      ...
      test('should filter out empty ingredients', () {
        ...
        formState.ingredientsManager.updateItems(['', 'Valid ingredient', '   ', 'Another ingredient', '']);
        
        // Act
        final recipe = formState.createRecipe();
        
        // Assert
        expect(recipe.ingredients, equals(['Valid ingredient', 'Another ingredient']));
      ...
      test('should filter out empty instructions', () {
        ...
        formState.instructionsManager.updateItems(['', 'Valid instruction', '   ', 'Another instruction', '']);
        
        // Act
        final recipe = formState.createRecipe();
        
        // Assert
        expect(recipe.instructions, equals(['Valid instruction', 'Another instruction']));
      ...
      test('should filter out empty tags', () {
        ...
        formState.tagsManager.updateItems(['', 'Valid tag', '   ', 'Another tag', '']);
      ...
      test('should trim title and description in created recipe', () {
        ...
        expect(recipe.description, equals(testDescription)); // Should be trimmed
      });
      ...
      test('should set correct timestamps in created recipe', () {
        ...
        // Assert
        expect(recipe.createdAt.isAfter(beforeCreation.subtract(Duration(seconds: 1))), isTrue);
        expect(recipe.createdAt.isBefore(afterCreation.add(Duration(seconds: 1))), isTrue);
        expect(recipe.updatedAt.isAfter(beforeCreation.subtract(Duration(seconds: 1))), isTrue);
        expect(recipe.updatedAt.isBefore(afterCreation.add(Duration(seconds: 1))), isTrue);
      });
    ...
    group('FormFieldsManager Integration', () {
      test('should update ingredients through FormFieldsManager callbacks', () {
        ...
        final controllers = formState.ingredientsManager.getControllers(formState.ingredients);
      ...
      test('should update instructions through FormFieldsManager callbacks', () {
        ...
        // Get existing controllers from current state
        final controllers = formState.instructionsManager.getControllers(formState.instructions);
      ...
      test('should handle FormFieldsManager bounds checking', () {
        ...
        formState.ingredientsManager.updateValue(99, 'Out of bounds ingredient');
      ...
      test('should return defensive copies from FormFieldsManager getters', () {
        ...
        // Assert
        expect(formState.ingredients, equals(testIngredients)); // Should not be affected
      });
    ...
    group('Edge Cases and Error Handling', () {
      test('should handle extremely long field values', () {
        ...
        expect(() => formState.setDescription(longDescription), returnsNormally);
      ...
      test('should handle special characters in text fields', () {
        // Arrange
        final specialTitle = 'Title with émojis 🍕 and special chars: åäö ñáéíóú';
        final specialDescription = 'Description with symbols: @#\$%^&*()_+-=[]{}|;:,.<>?';
      ...
      test('should handle empty arrays in FormFieldsManagers', () {
        ...
        // Assert
        expect(formState.ingredients, equals([''])); // Should default to single empty field
        expect(formState.instructions, equals(['']));
        expect(formState.tags, equals(['']));
      });
      ...
      test('should handle invalid meal type gracefully', () {
        ...
        // Assert
        expect(formState.mealType, equals('Invalid Meal Type')); // Should accept any string
      });
      
      test('should handle large image URL lists within limit', () {
        // Arrange
        final manyUrls = List.generate(RecipeFormState.maxImages, (i) => 'https://example.com/image$i.jpg');
