...
void main() {
  ...
  Future<SharedShoppingListsViewModel> buildVm() async {
    final v =
        SharedShoppingListsViewModel(shoppingService: mockShoppingService);
  ...
  test(
    ...
    expect(vm.sharedLists, hasLength(2),
        reason: 'Both collaborative lists must be exposed after load.');
    expect(vm.sharedLists.map((l) => l.name),
  ...
  test(
    ...
    mockShoppingService = MockUnifiedShoppingService();
    mockShoppingService.setShoppingState(collaborativeLists: []);
    when(() => mockShoppingService.loadLists())
  ...
  test('refreshData overwrites stale list when new data is available',
      () async {
    ...
    expect(vm.sharedLists.map((l) => l.name), contains('Nytt'),
