...
void main() {
  ...
  group('RealtimeStreamManager - Ultrathink Enhanced Tests', () {
    ...
    group('Stream Operations - Start Watching', () {
      ...
      test('should not start watching with same menu ID if already streaming', () async {
        // Arrange
        await streamManager.startWatching(testMenuId);
        streamEvents.clear();
        ...
        verify(() => mockMenuService.watchRealtimeMenu(testMenuId)).called(1); // Only called once
      });
      
      test('should switch to new menu ID when already streaming different menu', () async {
        // Arrange
        await streamManager.startWatching(testMenuId);
        streamEvents.clear();
    ...
    group('Stream Operations - Pause and Resume', () {
      test('should pause streaming correctly', () async {
        ...
        // Assert
        expect(streamManager.isStreaming, isFalse);
        expect(streamManager.hasActiveSubscription, isTrue); // Subscription still active
        expect(streamManager.currentMenuId, equals(testMenuId));
      });
    ...
    group('Connection Management', () {
      ...
      test('should restart stream successfully', () async {
        ...
        expect(streamEvents, contains('stopped'));
        expect(streamEvents, contains('started'));
        verify(() => mockMenuService.watchRealtimeMenu(testMenuId)).called(2); // Original + restart
      });
    ...
    group('Stream Event Handling', () {
      ...
      test('should handle multiple rapid menu updates', () async {
        ...
        final menu1 = RealtimeMenu(
          ...
          data: RealtimeMenuData(menuTitle: 'Menu 1', createdForDate: DateTime.now(), menuSnapshot: {}),
        );
        final menu2 = RealtimeMenu(
          ...
          lastEditedBy: 'owner',
          lastEditedByDisplayName: 'Owner',
          data: RealtimeMenuData(menuTitle: 'Menu 2', createdForDate: DateTime.now(), menuSnapshot: {}),
        );
        final menu3 = RealtimeMenu(
          ...
          lastEditedBy: 'owner',
          lastEditedByDisplayName: 'Owner',
          data: RealtimeMenuData(menuTitle: 'Menu 3', createdForDate: DateTime.now(), menuSnapshot: {}),
        );
    ...
    group('Stream Statistics and Diagnostics', () {
      ...
      test('should provide accurate connection statistics - streaming', () async {
        // Arrange
        await streamManager.startWatching(testMenuId);
    ...
    group('Edge Cases and Error Handling', () {
      ...
      test('should handle subscription cancellation errors gracefully', () async {
        // Arrange
        await streamManager.startWatching(testMenuId);
