  ...
  group('UnifiedShoppingViewModel', () {
    ...
    setUp(() async {
      ...
      mockPermissionService.setPermissionState(
        ...
        defaultHasPermission: true,  // This will make permission checks return true
      );
      ...
      when(() => mockShoppingService.createPersonalList(any())).thenAnswer((_) async => 'list-123');
      ...
      when(() => mockShoppingService.setActiveList(any())).thenAnswer((_) async => true);
      when(() => mockShoppingService.renameList(any(), any())).thenAnswer((_) async => true);
      when(() => mockShoppingService.deleteList(any())).thenAnswer((_) async => true);
      ...
      when(() => mockShoppingService.toggleItemBought(any())).thenAnswer((_) async => true);
      when(() => mockShoppingService.removeItemFromActiveList(any())).thenAnswer((_) async => true);
      ...
      when(() => mockShoppingService.uncheckAllItems()).thenAnswer((_) async => true);
    ...
    group('Initialization', () {
      ...
      test('should create default list for new users', () async {
        ...
        // Assert
        verify(() => mockShoppingService.createPersonalList('Min Inköpslista')).called(1);
      });
    ...
    group('State Accessors', () {
      ...
      test('should calculate shopping statistics', () {
        // Arrange
        final item1 = ShoppingListFactory.buildItem(name: 'Mjölk', bought: true);
        final item2 = ShoppingListFactory.buildItem(name: 'Bröd', bought: false);
    ...
    group('Personal List Management', () {
      test('should create personal list successfully', () async {
        ...
        verify(() => mockShoppingService.createPersonalList('Veckans Inköp')).called(1);
      });
      ...
      test('should rename active list', () async {
        ...
        // Assert
        expect(result, isTrue);
        verify(() => mockShoppingService.renameList('active-list', 'Nytt Namn')).called(1);
      });
    ...
    group('Item Management', () {
      ...
      test('should toggle item bought status', () async {
        ...
        // Assert
        expect(result, isTrue);
        verify(() => mockShoppingService.toggleItemBought('item-123')).called(1);
      });
      ...
      test('should remove item from active list', () async {
        ...
        // Assert
        expect(result, isTrue);
        verify(() => mockShoppingService.removeItemFromActiveList('item-123')).called(1);
      });
    ...
    group('Export Functionality', () {
      ...
      test('should export list with categories', () {
        // Arrange
        final items = [
          ShoppingListFactory.buildItem(name: 'Mjölk', category: 'Mejeri', bought: true),
          ShoppingListFactory.buildItem(name: 'Bröd', category: 'Bageri', bought: false),
        ];
        final activeList = ShoppingListFactory.build(
          name: 'Test Lista',
          items: items,
    ...
    group('Backward Compatibility API', () {
      ...
      test('should create list using backward compatible method', () async {
        ...
        // Assert
        expect(result, isTrue);
        verify(() => mockShoppingService.createPersonalList('Test List')).called(1);
      });
      ...
      test('should rename list using backward compatible method', () async {
        ...
        // Assert
        expect(result, isTrue);
        verify(() => mockShoppingService.renameList('list-123', 'New Name')).called(1);
      });
      ...
      test('should export list using backward compatible method', () {
        // Arrange
        final activeList = ShoppingListFactory.build(name: 'Backward Compatible Export');
        mockShoppingService.setShoppingState(
          lists: [activeList],
          activeListId: activeList.id,
        );
    ...
    group('Debug Information', () {
      test('should provide debug information', () {
        // Arrange
        final personalList = ShoppingListFactory.build(type: ListType.personal);
        final collaborativeList = ShoppingListFactory.build(type: ListType.collaborative);
        mockShoppingService.setShoppingState(
