  ...
  group('RecipePermissionManager - Ultrathink Enhanced Tests', () {
    ...
    group('Initialization and Recipe ID Management', () {
      ...
      test('should set recipe ID successfully', () {
        ...
        expect(permissionManager.canShare, isTrue); // Should be true for existing recipe with owner permissions
      });
      ...
      test('should clear recipe ID when set to null', () {
        ...
        expect(permissionManager.canShare, isFalse); // Should be false for null recipe ID
      });
      ...
      test('should handle multiple recipe ID changes', () {
        ...
        expect(permissionManager.canEdit, isTrue); // Assuming authenticated user has edit permissions
        
        // Act & Assert - second recipe  
        permissionManager.setRecipeId(secondRecipeId);
        expect(permissionManager.canEdit, isTrue); // Should work for second recipe too
        
        // Act & Assert - clear recipe
        permissionManager.setRecipeId(null);
        expect(permissionManager.canEdit, isTrue); // Should be true for new recipe creation (null ID)
      });
    });
    ...
    group('UI State Permission Getters - New Recipe Creation', () {
      test('should allow all permissions for new recipe creation (null recipe ID)', () {
        // Arrange - ensure no recipe ID is set
        permissionManager.setRecipeId(null);
        
        // Act & Assert
        expect(permissionManager.canEdit, isTrue); // New recipe creation always allows editing
        expect(permissionManager.canView, isTrue); // New recipe creation always allows viewing
        expect(permissionManager.isOwner, isTrue); // User is owner of new recipe during creation
        
        // These should be false for null recipe ID
        expect(permissionManager.canShare, isFalse); // Cannot share non-existent recipe
        expect(permissionManager.canInvite, isFalse); // Cannot invite to non-existent recipe
        expect(permissionManager.canDelete, isFalse); // Cannot delete non-existent recipe
      });
    });
    ...
    group('UI State Permission Getters - Existing Recipe', () {
      ...
      test('should deny edit permission when not authorized for existing recipe', () {
        // Arrange - reconfigure with no edit permissions
        mockPermissionService.setPermissionState(
          currentUserId: testCurrentUserId,
          isAuthenticated: true,
      ...
      test('should deny view permission when not authorized for existing recipe', () {
      ...
      test('should deny invite permission when not authorized for existing recipe', () {
        // Arrange - reconfigure with no invitation permissions
        mockPermissionService.setPermissionState(
          currentUserId: testCurrentUserId,
          isAuthenticated: true,
    ...
    group('Permission Management Operations', () {
      test('should update user permission successfully when authorized', () {
        // Act
        final result = permissionManager.updateUserPermission(testRecipeId, testUserId, 'editor');
      ...
      test('should handle different permission types in update', () {
        ...
        for (final permission in permissions) {
          // Act
          final result = permissionManager.updateUserPermission(testRecipeId, testUserId, permission);
          
          // Assert
          expect(result, isTrue, reason: 'Should update permission to $permission');
        }
      });
      
      test('should handle null permission in update gracefully', () {
        // Act
        final result = permissionManager.updateUserPermission(testRecipeId, testUserId, null);
      ...
      test('should handle empty recipe ID in permission update', () {
        // Act
        final result = permissionManager.updateUserPermission('', testUserId, 'editor');
      ...
      test('should handle empty user ID in permission update', () {
        // Act
        final result = permissionManager.updateUserPermission(testRecipeId, '', 'editor');
      ...
      test('should share recipe with user successfully', () {
        // Act
        final result = permissionManager.shareRecipeWithUser(testRecipeId, testUserId, 'viewer');
      ...
      test('should handle different sharing permissions', () {
        ...
        for (final permission in permissions) {
          // Act
          final result = permissionManager.shareRecipeWithUser(testRecipeId, testUserId, permission);
          
          // Assert
          expect(result, isTrue, reason: 'Should share with $permission permission');
        }
      });
      
      test('should handle null values in recipe sharing', () {
        // Act & Assert
        expect(permissionManager.shareRecipeWithUser('', testUserId, 'viewer'), isTrue);
        expect(permissionManager.shareRecipeWithUser(testRecipeId, '', 'viewer'), isTrue);
        expect(permissionManager.shareRecipeWithUser(testRecipeId, testUserId, null), isTrue);
        expect(permissionManager.shareRecipeWithUser(testRecipeId, testUserId, ''), isTrue);
      });
    });
    
    group('Action and Field Validation', () {
      test('should allow all actions by default', () {
        final actions = ['create', 'edit', 'delete', 'share', 'invite', 'comment', 'rate'];
      ...
      test('should allow editing all fields by default', () {
        final fields = ['title', 'description', 'ingredients', 'instructions', 'tags', 'image'];
      ...
      test('should handle special characters in action names', () {
        final specialActions = ['action-with-dash', 'action_with_underscore', 'action.with.dots'];
        ...
        for (final action in specialActions) {
          ...
          // Assert
          expect(canPerform, isTrue, reason: 'Should handle special action: $action');
        }
      });
      
      test('should handle special characters in field names', () {
        final specialFields = ['field-with-dash', 'field_with_underscore', 'field.with.dots'];
        ...
        for (final field in specialFields) {
          ...
          // Assert
          expect(canEdit, isTrue, reason: 'Should handle special field: $field');
        }
      });
    });
    ...
    group('Listener Management', () {
      test('should add listener without throwing', () {
        ...
        expect(() => permissionManager.addListener(testListener), returnsNormally);
      });
      ...
      test('should remove listener without throwing', () {
        ...
        // Act & Assert - should not throw
        expect(() => permissionManager.removeListener(testListener), returnsNormally);
      });
      ...
      test('should handle removing non-existent listener', () {
        ...
        // Act & Assert - should not throw
        expect(() => permissionManager.removeListener(nonExistentListener), returnsNormally);
      });
    ...
    group('Lifecycle Methods', () {
      ...
      test('should handle operations after dispose', () {
        ...
        expect(() => permissionManager.canEditRecipe(testRecipeId), returnsNormally);
        expect(() => permissionManager.setRecipeId(testRecipeId), returnsNormally);
      });
    });
    ...
    group('Edge Cases and Error Handling', () {
      test('should handle extremely long recipe IDs', () {
        ...
        expect(() => permissionManager.setRecipeId(longRecipeId), returnsNormally);
        expect(() => permissionManager.canEditRecipe(longRecipeId), returnsNormally);
        expect(() => permissionManager.canInviteMembers(longRecipeId), returnsNormally);
      });
      ...
      test('should handle special characters in recipe IDs', () {
        ...
        for (final recipeId in specialIds) {
          // Act & Assert - should not crash
          expect(() => permissionManager.setRecipeId(recipeId), returnsNormally);
          expect(() => permissionManager.canEditRecipe(recipeId), returnsNormally);
        }
      });
      ...
      test('should handle permission service edge cases gracefully', () {
        ...
        for (final extremeId in extremeIds) {
          ...
          expect(() => permissionManager.canEditRecipe(extremeId), returnsNormally);
          expect(() => permissionManager.canInviteMembers(extremeId), returnsNormally);
          expect(() => permissionManager.canManageMembers(extremeId), returnsNormally);
        }
      });
      ...
      test('should maintain state consistency across operations', () {
        ...
        expect(canEdit1, equals(canEdit3)); // Same recipe ID should give same edit permission
        expect(canShare1, equals(canShare3)); // Same recipe ID should give same share permission
        expect(canEdit2, isTrue); // Null recipe ID should allow editing (new recipe)
    ...
    group('Integration Scenarios', () {
      test('should handle complete permission workflow for new recipe', () {
        ...
        expect(permissionManager.updateUserPermission(testRecipeId, testUserId, 'editor'), isTrue);
        expect(permissionManager.shareRecipeWithUser(testRecipeId, testUserId, 'viewer'), isTrue);
      });
      
      test('should handle complete permission workflow for existing recipe as owner', () {
        // Arrange
        permissionManager.setRecipeId(testRecipeId);
      ...
      test('should handle complete permission workflow for existing recipe as editor', () {
        ...
        expect(permissionManager.hasPermissions, isTrue);
        expect(permissionManager.canManageMembers(testRecipeId), isFalse); // Not owner
        expect(permissionManager.canEditRecipe(testRecipeId), isTrue);
        expect(permissionManager.canInviteMembers(testRecipeId), isFalse); // Not admin/owner
        expect(permissionManager.updateUserPermission(testRecipeId, testUserId, 'viewer'), isFalse); // Not owner
      });
      
      test('should handle complete permission workflow for existing recipe as viewer', () {
        ...
        expect(permissionManager.updateUserPermission(testRecipeId, testUserId, 'viewer'), isFalse);
      });
