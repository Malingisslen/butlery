...
void main() {
  ...
  group('startImport — guards', () {
    ...
    test('returns ImportFailed without invoking ImportManager when input empty',
        () async {
      ...
      verifyNever(() => mockImportManager.autoImport(any(),
          onProgress: any(named: 'onProgress')));
    });
    ...
    test('refuses to start a second import while one is in flight', () async {
      ...
      when(() => mockImportManager.autoImport(any(),
          onProgress: any(named: 'onProgress'))).thenAnswer((_) => hang.future);
      ...
      verify(() => mockImportManager.autoImport(any(),
  ...
  group('startImport — happy path', () {
    ...
    test('trims input, passes onProgress, returns ImportSucceeded(recipe)',
        () async {
      ...
      final captured = verify(() => mockImportManager.autoImport(captureAny(),
          onProgress: captureAny(named: 'onProgress'))).captured;
      expect(captured[0], 'https://example.com/recipe',
  ...
  group('startImport — assistance result', () {
    ...
    test('propagates extractedText/sourceUrl/ingredient hints to NeedsUserHelp',
        () async {
      when(() => mockImportManager.autoImport(any(),
              onProgress: any(named: 'onProgress')))
          .thenAnswer((_) async => ImportManagerResult.assistance(
    ...
    test('defaults missing assistance fields to empty list / empty string',
        () async {
      when(() => mockImportManager.autoImport(any(),
              onProgress: any(named: 'onProgress')))
          .thenAnswer((_) async => ImportManagerResult.assistance(
  ...
  group('startImport — error localization', () {
    ...
    cases.forEach((english, swedish) {
      test('"$english" → "$swedish"', () async {
        ...
        expect(viewModel.error, swedish,
  ...
  group('startImport — exception path', () {
    ...
    test('network exception persists pending URL via SharedPreferences',
        () async {
      when(() => mockImportManager.autoImport(any(),
              onProgress: any(named: 'onProgress')))
  ...
  group('handleAssistedRecipe', () {
    ...
    test('forwards to saveImportedRecipe and returns ImportSucceeded',
        () async {
  ...
  group('retryWithoutLlm', () {
    ...
    test('passes options: {skipLlm: true} to autoImport', () async {
      ...
      final captured = verify(() => mockImportManager.autoImport(any(),
          options: captureAny(named: 'options'),
          onProgress: any(named: 'onProgress'))).captured;
  ...
  group('phase-derived getters', () {
    ...
    test('progressMessage maps each phase to a non-empty Swedish string',
        () async {
      // idle
  ...
  group('pending import persistence', () {
    ...
    test('persisted URL hydrates input + hasPendingImport on construction',
        () async {
      SharedPreferences.setMockInitialValues(
  ...
  group('dispose safety', () {
    ...
    test('dispose during in-flight import does not crash on completion',
        () async {
      final completer = Completer<ImportManagerResult>();
      when(() => mockImportManager.autoImport(any(),
              onProgress: any(named: 'onProgress')))
