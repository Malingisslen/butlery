  ...
  group('AuthViewModel - Ultrathink Enhanced Tests', () {
    ...
    group('Initialization and Default State', () {
      test('should initialize with correct default state for all properties', () {
        ...
        // Assert
        expect(viewModel.isLoginMode, isTrue, reason: 'Should start in login mode');
        expect(viewModel.isPasswordVisible, isFalse, reason: 'Password should be hidden by default');
        expect(viewModel.isLoading, isFalse, reason: 'Should not be loading initially');
        expect(viewModel.errorMessage, isNull, reason: 'Should have no error message initially');
        expect(viewModel.isAuthenticated, isFalse, reason: 'Should not be authenticated initially');
      });
      
      test('should properly initialize AuthService listener connection', () async {
        // Arrange
        final testViewModel = AuthViewModel();
        ...
        // Assert
        expect(notificationCount, greaterThan(0), reason: 'Should be listening to AuthService changes');
    ...
    group('Authentication Mode Management', () {
      test('should toggle between login and register modes correctly', () {
        ...
        expect(notificationCount, equals(3)); // toggleAuthMode() calls clearError() which triggers 3 notifications
        
        // Act & Assert - toggle back to login
        viewModel.toggleAuthMode();
        expect(viewModel.isLoginMode, isTrue);
        expect(notificationCount, equals(6)); // 3 more notifications from second toggle
      });
    ...
    group('Sign In Functionality', () {
      ...
      test('should accept valid email formats including Swedish characters', () async {
        // Arrange - Use default successful behavior (shouldSucceed = true)
      ...
      test('should reject password shorter than 6 characters', () async {
        ...
        for (final password in shortPasswords) {
          ...
          expect(result, isFalse, reason: 'Should reject password length: ${password.length}');
          // Check appropriate error message based on length
          if (password.isEmpty) {
            expect(viewModel.errorMessage, equals('Lösenord kan inte vara tomt'));
          } else {
            expect(viewModel.errorMessage, equals('Lösenord måste vara minst 6 tecken'));
          }
        }
      });
      ...
      test('should reject email with leading/trailing spaces during validation', () async {
      ...
      test('should handle service failure during sign in', () async {
        ...
        mockAuthService.setAuthState(isAuthenticated: false, error: 'Service failure');
    ...
    group('Registration Functionality', () {
      test('should successfully register with valid credentials and display name', () async {
        // Arrange - Use default successful behavior (shouldSucceed = true)
      ...
      test('should validate all fields before registration', () async {
        ...
        expect(result, isFalse);
        expect(viewModel.errorMessage, equals('Visningsnamn kan inte vara tomt'));
      });
      ...
      test('should reject display name shorter than 2 characters', () async {
        ...
        for (final name in shortNames) {
          ...
          expect(result, isFalse, reason: 'Should reject display name: "$name"');
          // Check appropriate error message based on length
          if (name.isEmpty) {
            expect(viewModel.errorMessage, equals('Visningsnamn kan inte vara tomt'));
          } else {
            expect(viewModel.errorMessage, equals('Visningsnamn måste vara minst 2 tecken'));
          }
        }
      });
      ...
      test('should reject email with spaces during registration validation', () async {
        // Arrange - Production validates BEFORE trimming, so spaces cause rejection
    ...
    group('Password Reset Functionality', () {
      ...
      test('should reject email with spaces during password reset validation', () async {
        // Arrange - Production validates BEFORE trimming, so spaces cause rejection
    ...
    group('Lifecycle Management', () {
      ...
      test('should handle multiple dispose calls as Flutter ChangeNotifier behavior', () {
        // Arrange
        final testViewModel = AuthViewModel();
    ...
    group('Concurrent Operations', () {
      test('should handle rapid mode toggles correctly', () {
        ...
        expect(notificationCount, equals(30)); // 10 toggles × 3 notifications each (toggleAuthMode + clearError)
      });
    ...
    group('Swedish Localization Validation', () {
      test('should accept Swedish special characters in all text fields', () async {
        // Arrange - Use default successful behavior (shouldSucceed = true)
      ...
      test('should reject Swedish domain names per email validation standards', () async {
        ...
        for (final email in swedishDomains) {
          ...
          expect(result, isFalse, reason: 'Should reject email with non-ASCII domain: $email');
          expect(viewModel.errorMessage, equals('Ogiltig e-postadress'));
        }
      });
    });
