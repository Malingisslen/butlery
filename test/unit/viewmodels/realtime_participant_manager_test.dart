...
void main() {
  ...
  group('RealtimeParticipantManager - Ultrathink Enhanced Tests', () {
    ...
    late RealtimeMenuService mockMenuService; // Using interface type for centralized mock
    ...
    setUp(() {
      ...
      when(() => mockParticipantTracker.updateDisplayName(any(), any())).thenReturn(null);
      when(() => mockParticipantTracker.removeParticipant(any())).thenReturn(null);
    ...
    group('Add Participant Operations', () {
      ...
      test('should throw error when lacking permission to add participant', () async {
        ...
        expect(
          ...
          throwsA(isA<Exception>()
              .having((e) => e.toString(), 'message', contains('Ingen behörighet'))),
        );
      ...
      test('should rethrow service error when adding participant fails', () async {
        // Arrange
    ...
    group('Remove Participant Operations', () {
      ...
      test('should throw error when lacking permission to remove participant', () async {
        // Arrange - reconfigure with no invitation permissions
        mockPermissionService.setPermissionState(
          currentUserId: testCurrentUserId,
          isAuthenticated: true,
      ...
      test('should throw error when trying to remove self', () async {
        ...
        expect(
          ...
          throwsA(isA<Exception>()
              .having((e) => e.toString(), 'message', contains('Kan inte ta bort sig själv'))),
        );
      ...
      test('should rethrow service error when removing participant fails', () async {
        // Arrange
        final serviceError = Exception('Service error');
        when(() => mockMenuService.removeParticipant(
          resourceId: any(named: 'resourceId'),
    ...
    group('Update Participant Permission Operations', () {
      ...
      test('should throw error when lacking permission to update permissions', () async {
        // Arrange - reconfigure with no invitation permissions
        mockPermissionService.setPermissionState(
          currentUserId: testCurrentUserId,
          isAuthenticated: true,
      ...
      test('should throw error when trying to update own permissions', () async {
        // Act & Assert
        expect(
          () => participantManager.updateParticipantPermission(
            menuId: testMenuId,
          ...
          throwsA(isA<Exception>()
              .having((e) => e.toString(), 'message', contains('Kan inte ändra sina egna behörigheter'))),
        );
    ...
    group('Leave Menu Operations', () {
      ...
      test('should throw error when no current user ID available', () async {
        ...
        expect(
          ...
          throwsA(isA<Exception>()
              .having((e) => e.toString(), 'message', contains('Ingen användar-ID'))),
        );
    ...
    group('Participant Tracking', () {
      ...
      test('should get online participants from tracker', () {
        ...
        when(() => mockParticipantTracker.onlineParticipants).thenReturn(onlineUsers);
      ...
      test('should update participant display name in tracker', () {
        // Act
        participantManager.updateParticipantDisplayName(testParticipantUserId, testParticipantDisplayName);
      ...
      test('should remove participant from tracker', () {
        ...
        // Assert
        verify(() => mockParticipantTracker.removeParticipant(testParticipantUserId)).called(1);
      });
    });
    ...
    group('Participant Validation', () {
      ...
      test('should validate permission correctly', () {
        ...
        expect(participantManager.validatePermission(ResourcePermission.viewer), isTrue);
        expect(participantManager.validatePermission(ResourcePermission.editor), isTrue);
        expect(participantManager.validatePermission(ResourcePermission.admin), isTrue);
        expect(participantManager.validatePermission(ResourcePermission.owner), isTrue);
      ...
      test('should check if user is already a participant', () {
        ...
        final isParticipant = participantManager.isUserParticipant(testOwnerUserId, testMenu);
        expect(isParticipant, isTrue);
        
        // Test non-participant
        final isNotParticipant = participantManager.isUserParticipant('non_participant', testMenu);
      ...
      test('should check if more participants can be added', () {
        ...
        final canAddWithLimit = participantManager.canAddMoreParticipants(testMenu, maxParticipants: 2);
        expect(canAddWithLimit, isFalse); // Current menu has 3 participants, limit is 2
      });
    });
    ...
    group('Permission Analysis', () {
      test('should get participant permission level', () {
        // Test existing participant
        final permission = participantManager.getParticipantPermission(testOwnerUserId, testMenu);
        expect(permission, equals(ResourcePermission.owner));
        
        // Test non-existent participant
        final noPermission = participantManager.getParticipantPermission('non_existent', testMenu);
        expect(noPermission, isNull);
      });
      
      test('should get participants with specific permission', () {
        // Act
        final owners = participantManager.getParticipantsWithPermission(testMenu, ResourcePermission.owner);
        final editors = participantManager.getParticipantsWithPermission(testMenu, ResourcePermission.editor);
        final viewers = participantManager.getParticipantsWithPermission(testMenu, ResourcePermission.viewer);
      ...
      test('should group participants by permission level', () {
        // Act
        final grouped = participantManager.groupParticipantsByPermission(testMenu);
      ...
      test('should count participants by permission', () {
        // Act
        final counts = participantManager.countParticipantsByPermission(testMenu);
    ...
    group('Participant Statistics', () {
      test('should generate participant statistics', () {
        ...
        when(() => mockParticipantTracker.onlineParticipants).thenReturn(['user1']);
      ...
      test('should generate participation summary for single user', () {
        ...
        // Act
        final summary = participantManager.getParticipationSummary(singleUserMenu);
        
        // Assert
        expect(summary, equals('Bara du'));
      });
      
      test('should generate participation summary for multiple users all online', () {
        // Arrange
        when(() => mockParticipantTracker.onlineParticipants).thenReturn([testOwnerUserId, testCurrentUserId, testViewerUserId]);
      ...
      test('should generate participation summary for multiple users all offline', () {
        // Arrange
      ...
      test('should generate participation summary for mixed online/offline', () {
        // Arrange
        when(() => mockParticipantTracker.onlineParticipants).thenReturn([testCurrentUserId]);
    ...
    group('Bulk Operations', () {
      ...
      test('should filter out current user when removing multiple participants', () async {
        // Arrange
        final userIds = [testCurrentUserId, testParticipantUserId]; // Include current user
      ...
      test('should handle empty list when removing multiple participants', () async {
        // Act
        await participantManager.removeMultipleParticipants(
          menuId: testMenuId,
          userIds: [testCurrentUserId], // Only current user, should be filtered out
        );
    ...
    group('Error Handling and Edge Cases', () {
      ...
      test('should handle empty menu participants', () {
        ...
        // Act & Assert
        expect(participantManager.isUserParticipant(testCurrentUserId, emptyMenu), isFalse);
    ...
    group('Integration Scenarios', () {
      test('should handle complete participant management workflow', () async {
        ...
        participantManager.updateParticipantDisplayName(testParticipantUserId, 'Updated Name');
      ...
      test('should handle participant validation workflow', () {
        ...
        // Menu validation
        expect(participantManager.isUserParticipant(testOwnerUserId, testMenu), isTrue);
