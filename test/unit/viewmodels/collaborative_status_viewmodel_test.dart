...
void main() {
  ...
  void stubRecipeShared({bool shared = true}) {
    when(
      () => mockSocial.isRecipeSharedByUser(any(), any()),
    ).thenAnswer((_) async => shared);
    when(
      () => mockSocial.getRecipeParticipants(any()),
    ).thenAnswer((_) async => []);
  }
      ...
      expect(
        viewModel
            .getRecipeCollaborativeStatus(
              'r1',
              RecipeFactory.build(title: '(Delad) Recipe'),
            )
            .isCollaborative,
        isTrue,
      );
      ...
      for (final desc in [
        ...
        expect(
          viewModel
              .getRecipeCollaborativeStatus(
                'r_$desc',
                RecipeFactory.build(description: desc),
              )
              .isCollaborative,
          isTrue,
    ...
    test('should detect collaborative patterns in source URL', () {
      expect(
        viewModel
            .getRecipeCollaborativeStatus(
              'r1',
              RecipeFactory.build(sourceUrl: 'https://x?shared_recipe_id=1'),
            )
            .isCollaborative,
        isTrue,
      );
      expect(
        viewModel
            .getRecipeCollaborativeStatus(
              'r2',
              RecipeFactory.build(sourceUrl: 'https://x?collaborative=true'),
            )
            .isCollaborative,
        isTrue,
      );
      expect(
        viewModel
            .getRecipeCollaborativeStatus(
              'r3',
              RecipeFactory.build(sourceUrl: 'https://x/normal'),
            )
            .isCollaborative,
        isFalse,
      );
  ...
  group('Async Recipe Status Check', () {
    test('should cache status after async check', () async {
      ...
      when(() => mockSocial.getRecipeParticipants(any())).thenAnswer(
        (_) async => [
          UserProfile(
            uid: 'u1',
            displayName: 'User',
  ...
  group('Menu Status', () {
    test('should detect collaborative menu from recipe metadata', () {
      ...
      expect(
        viewModel.getMenuCollaborativeStatus('m1', data).isCollaborative,
        isTrue,
      );
    });
    ...
    test('should handle empty and null menu', () {
      expect(
        viewModel
            .getMenuCollaborativeStatus('m1', <String, List<Recipe>>{})
            .isCollaborative,
        isFalse,
      );
      expect(
        viewModel.getMenuCollaborativeStatus('m2', null).isCollaborative,
        isFalse,
      );
    });
    ...
    test('should perform async menu check', () async {
      when(
        () => mockSocial.isMenuSharedByUser(any(), any()),
      ).thenAnswer((_) async => true);
      when(
        () => mockSocial.getMenuParticipants(any()),
      ).thenAnswer((_) async => []);
      ...
      verify(
        () => mockSocial.isMenuSharedByUser('menu_1', 'test_user_id'),
      ).called(1);
    });
  ...
  group('Shopping List Status', () {
    test('should return non-collaborative for uncached list', () {
      expect(
        viewModel.getShoppingListCollaborativeStatus('list_1').isCollaborative,
        isFalse,
      );
    });
    ...
    test('should perform async shopping list check', () async {
      when(
        () => mockSocial.isShoppingListSharedByUser(any(), any()),
      ).thenAnswer((_) async => true);
      when(() => mockSocial.getShoppingListParticipants(any())).thenAnswer(
        (_) async => [
          UserProfile(
            uid: 'u1',
            displayName: 'Friend',
  ...
  group('Cache Management', () {
    ...
    test('should not check same content concurrently', () async {
      when(() => mockSocial.isRecipeSharedByUser(any(), any())).thenAnswer((
        _,
      ) async {
      ...
      verify(
        () => mockSocial.isRecipeSharedByUser('recipe_1', any()),
      ).called(1);
    });
  });
  ...
  group('Batch Operations', () {
    test('should batch check multiple content types', () async {
      ...
      when(
        () => mockSocial.getShoppingListParticipants(any()),
      ).thenAnswer((_) async => []);
      ...
      verify(
        () => mockSocial.isRecipeSharedByUser('recipe_2', any()),
      ).called(1);
      verify(() => mockSocial.isMenuSharedByUser('menu_1', any())).called(1);
      verify(
        () => mockSocial.isShoppingListSharedByUser('list_1', any()),
      ).called(1);
      verify(
        () => mockSocial.isShoppingListSharedByUser('list_2', any()),
      ).called(1);
    });
    ...
    test('should skip already cached items in batch', () async {
      ...
      when(
        () => mockSocial.getRecipeParticipants('recipe_1'),
      ).thenAnswer((_) async => []);
