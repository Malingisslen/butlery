...
void main() {
  ...
  setUp(() async {
    ...
    // Default stubs
    when(() => mockCategories.getCategoryById(testGroupId)).thenReturn(testGroup);
    ...
    when(() => mockManagement.getFriendById(any())).thenAnswer((invocation) {
      ...
      return testFriends.firstWhere((f) => f.uid == id, orElse: () => testFriends[0]);
    });
    when(() => mockInvitations.getSentInvitations()).thenReturn(testInvitations);
  ...
  group('Initialization', () {
    ...
    test('should load available friends excluding existing members', () {
      ...
      expect(viewModel.availableFriends.any((f) => f.uid == 'existing-member-1'), isFalse);
      expect(viewModel.availableFriends.any((f) => f.uid == 'friend-1'), isTrue);
    });
    
    test('should exclude friends with pending invitations', () {
      expect(viewModel.availableFriends.any((f) => f.uid == 'friend-with-pending'), isFalse);
    });
    
    test('should sort available friends alphabetically', () {
      final names = viewModel.availableFriends.map((f) => f.displayName).toList();
  ...
  group('Search Functionality', () {
    ...
    test('should filter friends by name', () {
      ...
      expect(viewModel.filteredFriends.first.displayName, equals('Anna Andersson'));
    });
    ...
    test('should filter friends by bio', () {
      ...
      expect(viewModel.filteredFriends.length, equals(1));
      expect(viewModel.filteredFriends.first.displayName, equals('Erik Eriksson'));
    });
    ...
    test('should clear search', () {
      ...
      expect(viewModel.filteredFriends.length, equals(viewModel.availableFriends.length));
    });
    ...
    test('should handle friends without bio in search', () {
      ...
      expect(viewModel.filteredFriends.length, equals(1));
      expect(viewModel.filteredFriends.first.displayName, equals('Maria Nilsson'));
    });
  });
  ...
  group('Invitation Sending', () {
    ...
    test('should handle invitation failures', () async {
      ...
      expect(viewModel.getInvitationStatusForUser('friend-1'), equals('failed'));
    });
    ...
    test('should reload available friends after sending', () async {
      ...
      expect(viewModel.availableFriends.any((f) => f.uid == 'friend-1'), isFalse); // Should be filtered out
    });
    ...
    test('should handle friends without email', () async {
      ...
      when(() => mockManagement.getAllFriends()).thenReturn([...testFriends, friendWithoutEmail]);
      when(() => mockManagement.getFriendById('no-email')).thenReturn(friendWithoutEmail);
      ...
      expect(result, isFalse);
      expect(viewModel.getInvitationStatusForUser('no-email'), equals('failed'));
    });
  });
  ...
  group('Invitation Status', () {
    test('should check existing invitations', () {
      final hasInvitation = viewModel.hasExistingInvitation('friend-with-pending');
    ...
    test('should get sent invitations for group', () {
      ...
      expect((invitations.first as GroupInvitation).groupId, equals(testGroupId));
    });
  ...
  group('Refresh Functionality', () {
    test('should refresh all data', () async {
      ...
      verify(() => mockCategories.getCategoryById(testGroupId)).called(greaterThan(1));
  ...
  group('Edge Cases', () {
    ...
    test('should handle all friends being members', () async {
      ...
      when(() => mockCategories.getCategoryById(testGroupId)).thenReturn(groupWithAllMembers);
    ...
    test('should handle service exceptions during initialization', () async {
      when(() => mockCategories.getCategoryById(any())).thenThrow(Exception('Database error'));
  ...
  group('Swedish Localization', () {
    ...
    test('should use Swedish invitation error messages', () async {
      ...
      expect(viewModel.invitationError, contains('Fel vid sändning av gruppinbjudningar'));
    });
  });
