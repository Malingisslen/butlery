...
void main() {
  ...
  group('RealtimeRecipeViewModel - Ultrathink Enhanced Tests', () {
    ...
    late UnifiedRecipeService
        mockRecipeService; // Using interface type for centralized mock
    ...
    setUp(() {
      ...
      when(() => mockRealtimeOps.connectionStream)
          .thenAnswer((_) => Stream.value(true));
      when(() => mockRealtimeOps.startRealtimeEditing(any()))
          .thenAnswer((_) async => true);
      when(() => mockRealtimeOps.stopRealtimeEditing(any()))
          .thenAnswer((_) async => true);
      ...
      when(() => mockRecipeService.isInRealtimeEditingSession(any()))
          .thenReturn(true);
      when(() => mockRecipeService.addIngredient(any(), any()))
          .thenAnswer((_) async => true);
      when(() => mockRecipeService.updateIngredient(any(), any(), any()))
          .thenAnswer((_) async => true);
      when(() => mockRecipeService.removeIngredient(any(), any()))
          .thenAnswer((_) async => true);
      when(() => mockRecipeService.addInstruction(any(), any()))
          .thenAnswer((_) async => true);
      when(() => mockRecipeService.updateInstruction(any(), any(), any()))
          .thenAnswer((_) async => true);
      when(() => mockRecipeService.removeInstruction(any(), any()))
          .thenAnswer((_) async => true);
    ...
    group('Realtime Session Management', () {
      test('should start realtime editing successfully', () async {
        ...
        verify(() => mockRealtimeOps.startRealtimeEditing(testRecipeId))
            .called(1);
      });

      test('should fail to start realtime editing with empty recipe ID',
          () async {
        // Act
      ...
      test('should fail to start realtime editing with whitespace recipe ID',
          () async {
        // Act
        final result = await viewModel.startRealtimeEditing('   ');
      ...
      test('should stop realtime editing successfully', () async {
        ...
        // Assert
        expect(result, isTrue);
        verify(() => mockRealtimeOps.stopRealtimeEditing(testRecipeId))
            .called(1);
      });

      test('should fail to stop realtime editing with empty recipe ID',
          () async {
        // Act
      ...
      test('should check if recipe is in realtime editing session', () {
        ...
        // Assert
        expect(result, isTrue);
        verify(() => mockRecipeService.isInRealtimeEditingSession(testRecipeId))
            .called(1);
      });
      ...
      test('should handle session check when not in session', () {
        // Arrange
        when(() => mockRecipeService.isInRealtimeEditingSession(testRecipeId))
            .thenReturn(false);
    ...
    group('Realtime Content Operations - Ingredients', () {
      test('should add ingredient in realtime successfully', () async {
        // Act
        final result =
            await viewModel.addIngredientRealtime(testRecipeId, testIngredient);

        // Assert
        expect(result, isTrue);
        verify(() =>
                mockRecipeService.addIngredient(testRecipeId, testIngredient))
            .called(1);
      });
      ...
      test('should fail to add ingredient with empty recipe ID', () async {
        // Act
        final result =
            await viewModel.addIngredientRealtime('', testIngredient);
      ...
      test('should fail to add ingredient when not in realtime session',
          () async {
        // Arrange
        when(() => mockRecipeService.isInRealtimeEditingSession(testRecipeId))
            .thenReturn(false);
      ...
      test('should update ingredient in realtime successfully', () async {
        ...
        final result = await viewModel.updateIngredientRealtime(
            testRecipeId, 0, testIngredient);

        // Assert
        expect(result, isTrue);
        verify(() => mockRecipeService.updateIngredient(
            testRecipeId, 0, testIngredient)).called(1);
      });

      test('should fail to update ingredient when not in realtime session',
          () async {
        // Arrange
        when(() => mockRecipeService.isInRealtimeEditingSession(testRecipeId))
            .thenReturn(false);
        ...
        expect(result, isFalse);
        verifyNever(
            () => mockRecipeService.updateIngredient(any(), any(), any()));
      });

      test('should remove ingredient in realtime successfully', () async {
        // Act
        final result =
            await viewModel.removeIngredientRealtime(testRecipeId, 0);

        // Assert
        expect(result, isTrue);
        verify(() => mockRecipeService.removeIngredient(testRecipeId, 0))
            .called(1);
      });

      test('should fail to remove ingredient when not in realtime session',
          () async {
        // Arrange
        when(() => mockRecipeService.isInRealtimeEditingSession(testRecipeId))
            .thenReturn(false);
    ...
    group('Realtime Content Operations - Instructions', () {
      test('should add instruction in realtime successfully', () async {
        ...
        final result = await viewModel.addInstructionRealtime(
            testRecipeId, testInstruction);

        // Assert
        expect(result, isTrue);
        verify(() =>
                mockRecipeService.addInstruction(testRecipeId, testInstruction))
            .called(1);
      });

      test('should fail to add instruction when not in realtime session',
          () async {
        // Arrange
        when(() => mockRecipeService.isInRealtimeEditingSession(testRecipeId))
            .thenReturn(false);
      ...
      test('should update instruction in realtime successfully', () async {
        ...
        final result = await viewModel.updateInstructionRealtime(
            testRecipeId, 0, testInstruction);

        // Assert
        expect(result, isTrue);
        verify(() => mockRecipeService.updateInstruction(
            testRecipeId, 0, testInstruction)).called(1);
      });

      test('should fail to update instruction with empty instruction',
          () async {
        // Act
        final result =
            await viewModel.updateInstructionRealtime(testRecipeId, 0, '');

        // Assert
        expect(result, isFalse);
        verifyNever(
            () => mockRecipeService.updateInstruction(any(), any(), any()));
      });

      test('should remove instruction in realtime successfully', () async {
        // Act
        final result =
            await viewModel.removeInstructionRealtime(testRecipeId, 0);

        // Assert
        expect(result, isTrue);
        verify(() => mockRecipeService.removeInstruction(testRecipeId, 0))
            .called(1);
      });
    });
    ...
    group('Active Editor Tracking', () {
      ...
      test('should return empty list for empty recipe ID in async call',
          () async {
        // Act
      ...
      test('should get active editors synchronously', () {
        ...
        when(() => mockRealtimeOps.getActiveEditors(testRecipeId))
            .thenReturn(activeEditors);
      ...
      test('should check if user is actively editing', () {
        // Arrange
        when(() => mockRealtimeOps.getActiveEditors(testRecipeId))
            .thenReturn(['user_1', testUserId]);

        // Act
        final isEditing =
            viewModel.isUserActivelyEditing(testRecipeId, testUserId);
      ...
      test(
          'should return empty list stream for invalid recipe ID when watching editors',
          () {
        // Act
    ...
    group('Session State Management', () {
      test('should get active realtime sessions correctly', () {
        ...
        when(() => mockRecipeService.isInRealtimeEditingSession(testRecipeId))
            .thenReturn(true);
        when(() => mockRecipeService.isInRealtimeEditingSession('recipe_456'))
            .thenReturn(false);
    ...
    group('Conflict Resolution', () {
      ...
      test('should fail conflict resolution with invalid resolution type',
          () async {
        // Act & Assert
        await expectLater(
    ...
    group('Realtime Analytics', () {
      test('should get realtime stats correctly', () {
        ...
        when(() => mockRealtimeOps.getActiveEditors(testRecipeId))
            .thenReturn(['user_1', 'user_2']);
        when(() => mockRealtimeOps.getActiveEditors('recipe_456'))
            .thenReturn([]);
        when(() => mockRealtimeOps.getActiveEditors(testRecipeId))
    ...
    group('Edge Cases and Error Handling', () {
      ...
      test('should handle null presence data in active editors async',
          () async {
        // Arrange
        when(() => mockRealtimeOps.getRecipePresence(testRecipeId))
            .thenAnswer((_) async => [
