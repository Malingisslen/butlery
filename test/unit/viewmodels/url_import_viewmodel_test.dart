  ...
  group('UrlImportViewModel', () {
      ...
      when(() => mockTextStrategy.import(any(), options: any(named: 'options')))
          .thenAnswer((_) async => ImportResult.success(
            RecipeFactory.build(
              ...
              ingredients: ['3 ägg', '3 dl mjölk', '2 dl vetemjöl', '1 tsk salt', '2 msk smält smör'],
              instructions: ['Vispa ihop', 'Tillsätt mjöl', 'Rör i smör', 'Låt svälla', 'Stek i panna'],
              timeMinutes: 20,
              portions: 4,
            ),
          ));
    ...
    group('Recipe Site Recognition', () {
      ...
      test('should recognize international recipe sites', () {
        ...
        for (final site in sites) {
          ...
          expect(viewModel.isKnownRecipeSite(), isTrue, reason: 'Should recognize $site');
        }
      });
    ...
    group('URL Suggestions', () {
      test('should suggest for known recipe site', () {
        ...
        // Assert
        expect(suggestions, contains('✅ URL från känd receptsida - bör fungera bra'));
      });
      ...
      test('should warn for unknown site', () {
        ...
        // Assert
        expect(suggestions, contains('ℹ️ Okänd sida - innehållsextraktion kan variera'));
      });
      ...
      test('should warn about long URLs', () {
        ...
        // Assert
        expect(suggestions, contains('⚠️ Mycket lång URL - försök kopiera huvudreceptsidans URL'));
      });
      ...
      test('should identify social media links', () {
        ...
        for (final url in socialUrls) {
          ...
          final suggestions = viewModel.getUrlSuggestions();
          expect(suggestions, contains('📱 Social media-länk - innehållsextraktion kan vara begränsad'));
        }
      });

      test('should provide optimal suggestion for perfect URL', () {
        // Arrange
        viewModel.updateUrl('https://www.ica.se/pannkakor');  // URL without 'recept' keyword
    ...
    group('Content Fetching', () {
      ...
      test('should handle empty response', () async {
        ...
        expect(viewModel.error, anyOf(
          contains('Inget innehåll hittades'),
          contains('för kort')
        ));
      });
    ...
    group('Fetch and Parse Workflow', () {
      ...
      test('should parse after successful fetch', () async {
        ...
        // Assert
        expect(viewModel.hasParsedRecipe, isTrue);
        verify(() => mockTextStrategy.import(any(), options: any(named: 'options'))).called(1);
      });
    });
    ...
    group('Import and Save', () {
      ...
      test('should track loading state during import', () async {
        ...
        expect(viewModel.isLoading, isFalse); // Should be false after completion
      });
    });
    ...
    group('Content Analysis', () {
      ...
      test('should detect missing ingredients', () async {
        ...
        viewModel.updateUrl('https://www.example.com/recipe');
        when(() => mockHttpClient.get(any(), headers: any(named: 'headers')))
            .thenAnswer((_) async => TestHttpResponse(
        ...
        // Assert
        expect(analysis['issues'], contains('Ingen ingredienssektion hittades'));
      });
      ...
      test('should identify portion information', () async {
        ...
        // Assert
        expect(analysis['positives'], contains('Innehåller portionsinformation'));
      });
      ...
      test('should score content appropriately', () async {
        ...
        for (final testCase in testCases) {
          viewModel.updateUrl('https://www.example.com/recipe');
          when(() => mockHttpClient.get(any(), headers: any(named: 'headers')))
              .thenAnswer((_) async => TestHttpResponse(testCase['content'] as String, 200));
          await viewModel.fetchFromUrl();
          
          final analysis = viewModel.analyzeExtractedContent();
          expect(analysis['quality'], equals(testCase['expectedQuality']));
          if (testCase['minScore'] != null) {
            expect(analysis['score'], greaterThanOrEqualTo(testCase['minScore'] as int));
          }
          if (testCase['maxScore'] != null) {
            expect(analysis['score'], lessThanOrEqualTo(testCase['maxScore'] as int));
          }
        }
      });
    });
