  ...
  ModelTestBase.testModelGroup('ContentReaction', () {
    group('Construction', () {
      ...
      test('should handle all reaction types', () {
        ...
        expect(
            reactions.any((r) => r.reactionType == ReactionType.like), isTrue);
        expect(
            reactions.any((r) => r.reactionType == ReactionType.love), isTrue);
        expect(reactions.any((r) => r.reactionType == ReactionType.helpful),
            isTrue);
        expect(reactions.any((r) => r.reactionType == ReactionType.delicious),
            isTrue);
        expect(
            reactions.any((r) => r.reactionType == ReactionType.easy), isTrue);
        expect(reactions.any((r) => r.reactionType == ReactionType.creative),
            isTrue);
      });
    });
    ...
    group('Factory Methods', () {
      ...
      test(
          'should generate unique composite IDs for same user different content',
          () {
      ...
      test('should generate same composite ID for same user same content', () {
        ...
        expect(
            reaction1.id,
            equals(reaction2
                .id)); // Same ID ensures one reaction per user per content
      });
    ...
    group('Display Properties', () {
      ...
      test('should get display text with emoji', () {
        ...
        final helpfulReaction =
            reaction.copyWith(reactionType: ReactionType.helpful);
    ...
    group('Edge Cases', () {
      ...
      test('should handle unknown reaction type key', () {
        ...
        expect(reaction.reactionType,
            equals(ReactionType.like)); // Falls back to like
      });
    });
  });
}
