  ...
  ModelTestBase.testModelGroup('FriendRequest', () {
    group('Construction', () {
      ...
      test('should use current time when sentAt not provided', () {
        ...
        expect(request.sentAt.isAfter(beforeCreation.subtract(Duration(seconds: 1))), isTrue);
        expect(request.sentAt.isBefore(afterCreation.add(Duration(seconds: 1))), isTrue);
      });
    });
    ...
    group('Status Operations', () {
      ...
      test('should accept request with updated status and timestamp', () {
        ...
        expect(accepted.respondedAt!.isAfter(beforeAccept.subtract(Duration(seconds: 1))), isTrue);
        expect(accepted.respondedAt!.isBefore(afterAccept.add(Duration(seconds: 1))), isTrue);
      ...
      test('should reject request with updated status and timestamp', () {
        ...
        expect(rejected.respondedAt!.isAfter(beforeReject.subtract(Duration(seconds: 1))), isTrue);
        expect(rejected.respondedAt!.isBefore(afterReject.add(Duration(seconds: 1))), isTrue);
      });
      ...
      test('should cancel request with updated status and timestamp', () {
        ...
        expect(cancelled.respondedAt!.isAfter(beforeCancel.subtract(Duration(seconds: 1))), isTrue);
        expect(cancelled.respondedAt!.isBefore(afterCancel.add(Duration(seconds: 1))), isTrue);
      });
    ...
    group('Expiration Detection', () {
      ...
      test('should not be expired when sent exactly 7 days ago', () {
        ...
        expect(request.isExpired, isFalse); // Not expired until > 7 complete days
      });
    });
    ...
    group('Firestore Serialization', () {
      test('should serialize to Firestore format', () {
        ...
        expect(firestore.containsKey('id'), isFalse); // ID is document ID, not in data
