  ...
  group('TextFormatting', () {
    ...
    group('Normalize Text', () {
      ...
      test('should normalize whitespace to single spaces', () {
        expect(TextFormatting.normalizeText('  multiple   spaces  '), equals('multiple spaces'));
        expect(TextFormatting.normalizeText('tabs\t\there'), equals('tabs here'));
        expect(TextFormatting.normalizeText('newlines\n\nhere'), equals('newlines here'));
        expect(TextFormatting.normalizeText('\r\nwindows\r\nline\r\nbreaks\r\n'), equals('windows line breaks'));
      });
      
      test('should handle mixed Unicode and whitespace', () {
        expect(TextFormatting.normalizeText('𝐀𝐁𝐂   text'), equals('ABC text'));
        expect(TextFormatting.normalizeText('  𝐚𝐛𝐜  mixed  '), equals('abc mixed'));
        expect(TextFormatting.normalizeText('𝟏𝟐𝟑\t\nnumbers'), equals('123 numbers'));
      });
      ...
      test('should preserve standard characters', () {
        ...
        expect(TextFormatting.normalizeText('special!@#'), equals('special!@#'));
      });
    ...
    group('Is Portion Or Time Line', () {
      ...
      test('should detect time indicators', () {
        ...
        expect(TextFormatting.isPortionOrTimeLine('Tillagning: 20 minuter'), isTrue);
      });
      ...
      test('should reject non-matching text', () {
        ...
        expect(TextFormatting.isPortionOrTimeLine('portioner utan nummer'), isFalse);
        expect(TextFormatting.isPortionOrTimeLine('minuter utan nummer'), isFalse);
        expect(TextFormatting.isPortionOrTimeLine(''), isFalse);
      });
      
      test('should handle embedded patterns', () {
        expect(TextFormatting.isPortionOrTimeLine('Receptet ger 4 portioner'), isTrue);
        expect(TextFormatting.isPortionOrTimeLine('Baka i 25 minuter i ugn'), isTrue);
    ...
    group('Edge Cases and Special Values', () {
      test('should handle very large numbers', () {
        ...
        expect(TextFormatting.parseSwedishNumber('999999,99'), equals(999999.99));
        expect(TextFormatting.toSwedishHalfFraction(999999.5), equals('999999 ½'));
      });
      ...
      test('should handle special float values', () {
        ...
        expect(TextFormatting.toSwedishHalfFraction(double.infinity), equals('∞'));
        expect(TextFormatting.toSwedishHalfFraction(double.negativeInfinity), equals('-∞'));
        expect(TextFormatting.toSwedishHalfFraction(double.nan), equals('NaN'));
      });
      
      test('should handle Unicode edge cases in normalizeText', () {
        // Empty Unicode range
        expect(TextFormatting.normalizeText('\u{1D3FF}'), equals('\u{1D3FF}')); // Just before range
        expect(TextFormatting.normalizeText('\u{1D800}'), equals('\u{1D800}')); // Just after range
        
        // Mixed content
        expect(TextFormatting.normalizeText('Normal 𝐀𝐁𝐂 text'), equals('Normal ABC text'));
      });
    });
  });
}