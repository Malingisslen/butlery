...
void main() {
  ...
  test('addDeliveryEvents writes one doc per event to notification_delivery',
      () async {
  ...
  test(
      'delivery lifecycle: addDeliveryEvents then updateDelivery hit the SAME '
  ...
  test('getDeliveriesForUser filters by targetUserId and sentAt window',
      () async {
  ...
  test('getEngagementsForUser filters by userId and timestamp window',
      () async {
    final since = DateTime(2026, 1, 1);
    await firestore
        .collection(FirestoreCollections.notificationEngagement)
    ...
    await firestore
        ...
        .add({
      'userId': userId,
      'timestamp': Timestamp.fromDate(DateTime(2025, 11, 1)), // before window
    });
