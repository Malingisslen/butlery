  ...
  group('RecipeDiscoveryService', () {
    ...
    group('Collaborative Recipe Discovery', () {
      test('should get collaborative recipes where user is member', () async {
        ...
        expect(recipes.map((r) => r.id), isNot(contains('collab_3'))); // User not a member
        expect(recipes.map((r) => r.id), isNot(contains('personal_1'))); // Not collaborative
      });
    ...
    group('Shared Recipe Discovery', () {
      test('should get recipes shared with current user', () async {
        ...
        // Assert
        expect(recipes.length, equals(1)); // Only collab_2 where user is member but not owner
        expect(recipes[0].id, equals('collab_2'));
      });
    ...
    group('Recommendation Engine Tests', () {
      test('should get trending recipes based on activity', () async {
        ...
        expect(recipes.first.id, equals('trending_1')); // Higher score due to more members and recent activity
      });
      ...
      test('should recommend recipes based on user history', () async {
        ...
        // Assert
        expect(recipes.where((r) => r.tags?.contains('italian') ?? false).length, greaterThanOrEqualTo(2));
      });
    ...
    group('Advanced Filtering Tests', () {
      ...
      test('should handle complex search queries with Swedish characters', () async {
        // Arrange
    ...
    group('Pagination and Performance Tests', () {
      test('should handle pagination with limit', () async {
        // Arrange
        final manyRecipes = List.generate(50, (i) => 
          RecipeBuilder()
            .withId('recipe_$i')
            .withTitle('Recipe $i')
            .asCollaborative()
          ...
          RecipeBuilder()
            ...
            .withSocialData(
              RecipeSocialData(
                ...
                memberPermissions: i % 2 == 0 ? {} : {'user_123': ResourcePermission.viewer},
              ),
            )
            .build()
        );
      ...
      test('should sort results by relevance in search', () async {
        ...
        // Assert
        expect(results.first.id, equals('exact_title')); // Title match should be first
      });
