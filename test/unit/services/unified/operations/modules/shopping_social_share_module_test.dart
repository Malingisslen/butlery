...
void main() {
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          contains('Namnlös'),
          reason:
              'BUT-1109: Swedish localized fallback should be used '
              '(default app locale)',
        );
        expect(
          title,
  ...
  group('Convenience wrappers', () {
    ...
    test(
      'sendCollaborationInvite is a pass-through to shareWithFriends',
      () async {
        await _seedPersonalList(firestore, listId: 'l1');
  ...
  group('shareWithGroups', () {
    ...
    test(
      'single group with 3 members → 1 shared doc + 3 received docs',
      () async {
        await _seedPersonalList(firestore, listId: 'l1');
        await _seedGroup(
          firestore,
    ...
    test(
      'overlapping members across groups → no duplicate received docs',
      () async {
        await _seedPersonalList(firestore, listId: 'l1');
        await _seedGroup(
          firestore,
  ...
  group('getShoppingListsSharedWithMe', () {
    ...
    /// shared doc.
    test(
      'returns merged inbox entries with status from received pointer',
      () async {
    ...
    test('BUT-1108: shared doc without me in sharedWithUserIds is filtered out '
      ...
      expect(
        ...
        isEmpty,
        reason:
            'BUT-1108: defense-in-depth — pointer alone must not '
  ...
  group('getShoppingListsSharedByMe', () {
    ...
    test(
      'returns shared docs with sharedWithCount matching recipients',
      () async {
        await firestore
            .collection(FirestoreCollections.sharedContent)
            .doc('s1')
  ...
  group('markSharedShoppingListAsViewed', () {
    test('unauthenticated → false', () async {
      final ok = await _unauthModule(
        firestore,
      ).markSharedShoppingListAsViewed('s1');
      expect(ok, isFalse);
    });
  ...
  group('getShoppingListSharingStats', () {
    test('unauthenticated → empty map', () async {
      final stats = await _unauthModule(
        firestore,
      ).getShoppingListSharingStats('any');
      expect(stats, isEmpty);
    });

    test(
      'zero shares → sharedWith=0, totalShared=0, lastShared=null',
      () async {
