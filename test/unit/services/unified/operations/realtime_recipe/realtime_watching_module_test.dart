...
void main() {
  group('RealtimeWatchingModule', () {
    ...
    setUp(() async {
      ...
      watchingModule = RealtimeWatchingModule(mockParentService, mockRealtimeSyncService);
    });
    ...
    group('Single Recipe Watching', () {
      test('should watch recipe with realtime service', () async {
        // Arrange
        when(() => mockRealtimeSyncService.watchResource<RealtimeRecipe>('recipe_1'))
      ...
      test('should fall back to polling when realtime service unavailable', () async {
        // Arrange
    ...
    group('Multiple Recipe Watching', () {
      test('should watch multiple recipes simultaneously', () async {
        ...
        when(() => mockRealtimeSyncService.watchResource<RealtimeRecipe>('recipe_1'))
            .thenAnswer((_) => Stream.value(testRealtimeRecipe));
        when(() => mockRealtimeSyncService.watchResource<RealtimeRecipe>('recipe_2'))
            .thenAnswer((_) => Stream.value(RealtimeRecipeBuilder().withId('recipe_2').build()));
        
        // Act
        final stream = watchingModule.watchMultipleRecipes(['recipe_1', 'recipe_2']);
      ...
      test('should watch multiple recipes individually with error handling', () async {
        // Arrange
        when(() => mockRealtimeSyncService.watchResource<RealtimeRecipe>('recipe_1'))
            .thenAnswer((_) => Stream.value(testRealtimeRecipe));
        when(() => mockRealtimeSyncService.watchResource<RealtimeRecipe>('recipe_2'))
        ...
        // Act
        final stream = watchingModule.watchMultipleRecipesIndividually(['recipe_1', 'recipe_2']);
    ...
    group('Connection Monitoring', () {
      ...
      test('should wait for connection with timeout', () async {
        ...
        when(() => mockRealtimeSyncService.connectionStream)
            .thenAnswer((_) => connectionController.stream);
        mockRealtimeSyncService.setServiceState(isConnected: false);
    ...
    group('Utility Methods', () {
      ...
      test('should report limited capabilities without realtime service', () {
        ...
        expect(capabilities['canWatchSingle'], isTrue); // Still available with polling
        expect(capabilities['canWatchMultiple'], isTrue); // Still available with polling
      });
    });
  });
}
...
class MockRealtimeSyncService extends Mock implements RealtimeSyncService {
  ...
  // Removed watchResource override - let mocktail handle it
}