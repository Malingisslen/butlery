  ...
  group('RealtimeRecipeOperations - Comprehensive Behavior Testing', () {
    ...
    group('Real-time Watching with Stream Behavior', () {
      ...
      test('should validate empty recipe ID and throw ArgumentError', () {
        ...
        expect(
          ...
          throwsA(
            isA<ArgumentError>()
                .having((e) => e.message, 'message', contains('cannot be null or empty')),
          ),
        );
      });
      
      test('should watch recipe with retry and recover from connection failures', () async {
        // Arrange
        var attemptCount = 0;
        ...
        final recipes = await stream.take(1).timeout(
          Duration(seconds: 2),
          onTimeout: (sink) => throw TimeoutException('Retry did not succeed'),
        ).toList();
      ...
      test('should watch multiple recipes and combine streams correctly', () async {
        ...
        when(() => mockRealtimeService.watchResource<RealtimeRecipe>('collab-456'))
            .thenAnswer((_) => controller2.stream);
        
        // Act
        final stream = operations.watchMultipleRecipes(['recipe-123', 'collab-456']);
        ...
        expect(lastEmitted.any((r) => r.core.title == 'Updated Meatballs'), isTrue);
      ...
      test('should watch multiple recipes individually with error isolation', () async {
        ...
        when(() => mockRealtimeService.watchResource<RealtimeRecipe>('recipe-123'))
            .thenAnswer((_) => controller1.stream);
        when(() => mockRealtimeService.watchResource<RealtimeRecipe>('error-recipe'))
            .thenAnswer((_) => Stream.error(Exception('Recipe not found')));
        
        // Act
        final stream = operations.watchMultipleRecipesIndividually(['recipe-123', 'error-recipe']);
    ...
    group('Module Delegation Tests', () {
      ...
      test('should validate edit changes', () {
        // Act
        final isValid = operations.validateEditChanges('recipe-123', {'title': 'New Title'});
    ...
    group('Error Handling and Edge Cases', () {
      test('should handle null realtime service gracefully', () {
        ...
        expect(() => opsWithoutRealtime.watchRecipe('recipe-123'), returnsNormally);
      });
