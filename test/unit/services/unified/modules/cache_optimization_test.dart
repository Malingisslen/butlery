...
void main() {
  group('CacheOptimization', () {
    ...
    group('Cleanup Invalid Permissions', () {
      ...
      test('should remove collaborative recipes where user is not member', () async {
        // Arrange
      ...
      test('should handle corrupted entries during permission cleanup', () async {
        // Arrange
        mockCacheHelper.setCacheState(cache: {
    ...
    group('Cleanup Corrupted Entries', () {
      test('should remove corrupted entries', () async {
        ...
        // Act
        final removedCount = await CacheOptimization.cleanupCorruptedEntries(mockCacheHelper);
    ...
    group('Cache Health Assessment', () {
      test('should assess cache health correctly', () async {
        ...
        expect(recommendations.any((r) => r.toString().contains('corrupted')), isTrue);
        expect(recommendations.any((r) => r.toString().contains('permission')), isTrue);
      });
      ...
      test('should recommend optimization for large cache', () async {
        ...
        final recommendations = health['recommendations'] as List;
        expect(recommendations.any((r) => r.toString().contains('large')), isTrue);
      });
    ...
    group('Advanced Cache Strategies', () {
      test('should implement LRU cache eviction strategy', () async {
        ...
        expect(cache.containsKey('recipe_2'), isTrue); // Kept (recently accessed)
        expect(cache.containsKey('recipe_4'), isTrue); // Kept (recently accessed)
      });
      ...
      test('should implement TTL cache with time-based expiry', () async {
        ...
        expiryTimes['expired_recipe'] = now.subtract(const Duration(minutes: 1));
    ...
    group('Performance Optimization', () {
      ...
      test('should optimize memory usage with size limits', () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          final estimatedSize = recipeData.toString().length * 2; // Rough estimate
    ...
    group('Swedish Content Cache Support', () {
      test('should cache Swedish recipes with special characters', () async {
        ...
        final description = cached['description'] ?? cached['core']?['description'] ?? '';
        final ingredients = cached['ingredients'] as List? ?? cached['core']?['ingredients'] as List? ?? [];
