  ...
  group('FirebaseSyncManager', () {
    ...
    setUp(() async {
      ...
      when(() => mockRecipeRepository.subscribeToUserRecipes(
        ...
        return streamController.stream.listen(
          ...
          onError: (error) {
            final errorCallback =
                invocation.namedArguments[#onError] as Function(dynamic)?;
    ...
    group('Collaborative Recipes Sync', () {
      test('should handle collaborative recipe additions', () async {
        ...
        final verifyDoc = await fakeFirestore
            .collection('realtime_recipes')
            .doc('collab_1')
            .get();
        ...
        final participants =
            recipeData['participants'] as Map<String, dynamic>?;
      ...
      test('should only sync recipes where user is participant', () async {
        ...
        final userRecipe = RealtimeRecipe(
          ...
          recipe:
              RecipeBuilder().withId('user_recipe').asCollaborative().build(),
        ...
        final otherRecipe = RealtimeRecipe(
          ...
          ownerDisplayName: 'User 456',
          recipe:
              RecipeBuilder().withId('other_recipe').asCollaborative().build(),
          participants: {
            'user_456': ResourcePermission.owner,
        ...
        final verifyUserDoc = await fakeFirestore
            .collection('realtime_recipes')
            .doc('user_recipe')
            .get();
        final verifyOtherDoc = await fakeFirestore
            .collection('realtime_recipes')
            .doc('other_recipe')
            .get();
        ...
        final userParticipants =
            userRecipeData['participants'] as Map<String, dynamic>?;
        final otherParticipants =
            otherRecipeData['participants'] as Map<String, dynamic>?;
    ...
    group('Sync Status and Diagnostics', () {
      test('should get sync status information', () {
        ...
        expect(status['activeSubscriptions'],
      ...
      test('should check individual sync types', () {
        ...
        expect(FirebaseSyncManager.isCollaborativeSyncActive(subscriptions),
            isFalse);
      ...
      test('should get active subscription names', () {
        ...
        // Act
        final active =
            FirebaseSyncManager.getActiveSubscriptions(subscriptions);

        // Assert
        expect(
            active, containsAll(['personal_recipes', 'collaborative_recipes']));
      });
    });
