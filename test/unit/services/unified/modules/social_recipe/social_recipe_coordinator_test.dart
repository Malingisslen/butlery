  ...
  group('SocialRecipeCoordinator', () {
      ...
      test('should create collaborative recipe with initial members', () async {
        ...
        if (recipe.socialData?.memberPermissions != null) {
          expect(
              ...
              isTrue);
          expect(recipe.socialData!.memberPermissions![currentUserId],
              equals(ResourcePermission.admin));
          expect(recipe.socialData!.memberPermissions!['user-2'],
              equals(ResourcePermission.editor));
          expect(recipe.socialData!.memberPermissions!['user-3'],
    ...
    group('Membership Management', () {
      test('should add member to recipe with permission', () async {
        ...
        final recipe = testRecipesMap[recipeId];
        if (recipe?.socialData?.memberPermissions != null) {
          expect(recipe!.socialData!.memberPermissions!.containsKey(newUserId),
              isTrue);
          expect(recipe.socialData!.memberPermissions![newUserId],
              equals(permission));
        }
      });
      ...
      test('should remove member from recipe', () async {
        ...
        if (recipe?.socialData?.memberPermissions != null) {
          expect(
              recipe!.socialData!.memberPermissions!.containsKey(userToRemove),
              isFalse);
        }
      });
      ...
      test('should update member permission', () async {
        ...
        final recipe = testRecipesMap[recipeId];
        if (recipe?.socialData?.memberPermissions != null) {
          expect(recipe!.socialData!.memberPermissions![userId],
              equals(newPermission));
        }
      });
        ...
        if (recipe?.socialData?.memberPermissions != null) {
          expect(
              recipe!.socialData!.memberPermissions!
                  .containsKey('share-user-1'),
              isTrue);
          expect(
              recipe.socialData!.memberPermissions!.containsKey('share-user-2'),
              isTrue);
          expect(recipe.socialData!.memberPermissions!['share-user-1'],
              equals(permission));
          expect(recipe.socialData!.memberPermissions!['share-user-2'],
              equals(permission));
        }
      });
    ...
    group('Permission Validation', () {
      test('should check if user can edit recipe', () async {
        ...
        // Act - Check owner
        final ownerCanEdit =
            await coordinator.canEditRecipe(recipeId, currentUserId);

        // Act - Check editor
        final editorCanEdit =
            await coordinator.canEditRecipe(recipeId, 'user-2');

        // Act - Check viewer
        final viewerCanEdit =
            await coordinator.canEditRecipe(recipeId, 'user-3');
      ...
      test('should check if user can manage recipe members', () async {
        ...
        final ownerCanManage =
            await coordinator.canManageRecipeMembers(recipeId, currentUserId);

        // Act - Check editor
        final editorCanManage =
            await coordinator.canManageRecipeMembers(recipeId, 'user-2');

        // Act - Check viewer
        final viewerCanManage =
            await coordinator.canManageRecipeMembers(recipeId, 'user-3');
      ...
      test('should get user permission for recipe', () async {
        ...
        // Act
        final permission =
            await coordinator.getUserPermissionForRecipe(recipeId, 'user-2');
    ...
    group('Query Operations', () {
      test('should get collaborative recipes for user', () async {
        ...
        final collaborativeRecipes =
            await coordinator.getCollaborativeRecipesForUser();
      ...
      test('should get recipes shared by user', () async {
        // Act
        final sharedByUser =
            await coordinator.getRecipesSharedByUser(currentUserId);
      ...
      test('should get recipes with specific permission', () async {
        ...
        // Act
        final recipesWithPermission =
            await coordinator.getRecipesWithPermission(permission);
      ...
      test('should get most active collaborators', () async {
        ...
        final activeCollaborators =
            await coordinator.getMostActiveCollaborators(limit: 5);
    ...
    group('Notification Coordination', () {
      ...
      test('should send notification when recipe shared', () async {
        ...
        // Act
        final result = await coordinator.shareRecipeWithUsers(
            recipeId, userIds, ResourcePermission.viewer);
