...
void main() {
  group('IntelligentCacheManager', () {
    ...
    setUp(() async {
      ...
      mockRecipeService = TestServiceLocator.get<UnifiedRecipeService>() as MockUnifiedRecipeService;
      mockFriendsService = TestServiceLocator.get<UnifiedFriendsService>() as MockUnifiedFriendsService;
    ...
    group('UserBehaviorPattern', () {
      ...
      test('should serialize and deserialize to JSON', () {
        ...
        expect(restored.mealTypePreferences, equals(original.mealTypePreferences));
      ...
      test('should calculate recency scores correctly', () {
        ...
        // Assert
        expect(likelyRecipes.first, equals('recent')); // Recent has higher score
      });
    ...
    group('CacheEntry', () {
      test('should track access statistics', () {
        ...
        expect(entry.lastAccessed.isAfter(initialAccessTime) || 
               entry.lastAccessed.isAtSameMomentAs(initialAccessTime), isTrue);
      });
      ...
      test('should calculate eviction score based on access and age', () {
        ...
        // Assert
        expect(frequentScore > oldScore, isTrue); // Frequent access = higher score = less likely to evict
      });
    });
    ...
    group('Memory Management', () {
      test('should estimate recipe size correctly', () {
        ...
        final largeRecipe = RecipeFactory.build(
          ...
          instructions: List.generate(10, (i) => 'Detailed step $i with instructions'),
        );
    ...
    group('Cache Eviction Strategies', () {
      ...
      test('should prioritize frequently accessed items during eviction', () async {
        // Arrange
        ...
        final manyRecipes = List.generate(20, (i) => RecipeFactory.build(
          id: 'new_$i',
          title: 'New Recipe $i',
        ));
        mockRecipeService.setRecipeState(
    ...
    group('Predictive Loading', () {
      ...
      test('should predict recipes based on time of day', () async {
        ...
        final mealType = hour < 10 ? 'Frukost' : hour < 15 ? 'Lunch' : 'Middag';
    ...
    group('Concurrent Access', () {
      test('should handle concurrent cache reads safely', () async {
        ...
        final futures = List.generate(10, (i) => 
          manager.getCachedRecipe('concurrent_test')
        );
      ...
      test('should handle concurrent cache writes safely', () async {
        ...
        final futures = recipes.map((r) => 
          manager.getCachedRecipe(r.id)
        ).toList();
      ...
      test('should handle concurrent eviction safely', () async {
        ...
        final recipes = List.generate(50, (i) => RecipeFactory.build(
          id: 'evict_$i',
        ));
        mockRecipeService.setRecipeState(recipes: recipes);
    ...
    group('Memory Pressure Handling', () {
      test('should reduce cache size under memory pressure', () async {
        ...
        final recipes = List.generate(100, (i) => RecipeFactory.build(
          id: 'memory_$i',
          title: 'Recipe with very long title to consume more memory $i',
      ...
      test('should prioritize critical items during memory pressure', () async {
        ...
        final manyRecipes = List.generate(50, (i) => RecipeFactory.build(
          id: 'other_$i',
          title: 'Other Recipe $i',
          description: 'Very long description to consume memory ' * 100,
        ));
        mockRecipeService.setRecipeState(recipes: [favoriteRecipe, ...manyRecipes]);
    ...
    group('Edge Cases', () {
      ...
      test('should handle cache overflow gracefully', () async {
        ...
        final hugeRecipes = List.generate(1000, (i) => RecipeFactory.build(
          id: 'huge_$i',
      ...
      test('should handle corrupted cache data', () async {
        // Arrange
        mockBehaviorCache.setCacheState(cache: {
          'behavior_pattern': {'corrupted': 'invalid_data'}, // Invalid pattern data
        });
