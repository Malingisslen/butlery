  ...
  group('PerformanceMonitoringService', () {
    ...
    group('Metric Recording', () {
      ...
      test('should maintain queue size limit', () {
        ...
        // Assert
        final report = service.generateReport();
        expect(report.metrics[MetricType.custom]!.length, lessThanOrEqualTo(1000));
      });
    });
    ...
    group('Timer Management', () {
      test('should start and stop timers correctly', () async {
        ...
        expect(timerMetric.value, greaterThanOrEqualTo(45.0)); // Allow some variance
        expect(timerMetric.value, lessThan(200.0)); // Allow more variance for CI
      });
    ...
    group('Network Request Tracking', () {
      ...
      test('should generate warning for slow requests', () {
        ...
        expect(report.warnings.any((w) => w.contains('Slow network request')), isTrue);
      });
    });
    ...
    group('Memory Usage Tracking', () {
      ...
      test('should generate warning for high memory usage', () {
        ...
        final report = service.generateReport();
        expect(report.warnings, isNotEmpty);
        expect(report.warnings.any((w) => w.contains('High memory usage')), isTrue);
      });
    });
    ...
    group('User Interaction Tracking', () {
      ...
      test('should generate warning for slow interactions', () {
        ...
        final report = service.generateReport();
        expect(report.warnings, isNotEmpty);
        expect(report.warnings.any((w) => w.contains('Slow interaction response')), isTrue);
      });
    });
    ...
    group('Report Generation', () {
      test('should generate comprehensive report', () {
        // Arrange
        service.recordNetworkRequest('api/test', const Duration(milliseconds: 100));
