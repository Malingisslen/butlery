  ...
  group('Comments Service', () {
    ...
    group('Add Comments', () {
      ...
      test('should throw when adding empty comment', () async {
        ...
        )).thenAnswer((_) async => throw SecurityViolationException('Comment cannot be empty'));
      ...
      test('should throw when user tries to add comment for another user', () async {
        ...
        when(() => mockRepository.addComment(
          ...
          content: content,
        )).thenAnswer((_) async => throw PermissionDeniedException('Cannot comment as another user'));
    ...
    group('Get Comments', () {
      test('should get top-level comments for recipe', () async {
        ...
        final expectedComments = <RecipeComment>[...List.generate(5, (i) => RecipeComment(
          id: 'comment_$i',
          recipeId: recipeId,
          authorId: 'user_$i',
      ...
      test('should get replies for a comment', () async {
        ...
        final expectedReplies = <RecipeComment>[...List.generate(3, (i) => RecipeComment(
          id: 'reply_$i',
          recipeId: 'recipe_123',
          authorId: 'user_$i',
          authorDisplayName: 'User $i',
    ...
    group('Update Comments', () {
      ...
      test('should throw when updating with empty content', () async {
        ...
        // Act & Assert
        expect(
          () async => await mockRepository.updateComment(commentId, emptyContent),
          throwsA(isA<SecurityViolationException>()),
        );
      });
    });
    ...
    group('Delete Comments', () {
      ...
      test('should throw when trying to delete another users comment', () async {
        ...
        when(() => mockRepository.deleteComment(commentId))
            .thenAnswer((_) async => throw PermissionDeniedException('Cannot delete another users comment'));
    ...
    group('Comment Threading', () {
      ...
      test('should get comment thread using replies', () async {
        ...
        expect(level1Replies.every((c) => c.parentCommentId == rootCommentId), isTrue);
      ...
      test('should handle thread visibility using replies', () async {
        ...
        when(() => mockRepository.getReplies(parentCommentId))
            .thenAnswer((_) async => isCollapsed ? collapsedReplies : expandedReplies);
      ...
      test('should sort comments by like count', () async {
        ...
        // Act
        final sortedComments = await mockRepository.getCommentsForRecipe(recipeId);
    ...
    group('Moderation', () {
      ...
      test('should prevent adding comments with forbidden words', () async {
        ...
        when(() => mockRepository.addComment(
          ...
          userId: userId,
          content: badContent,
        )).thenAnswer((_) async => throw SecurityViolationException('Content contains forbidden words'));
      ...
      test('should handle user with multiple deleted comments', () async {
        ...
        final deletedCommentIds = ['violation_1', 'violation_2', 'violation_3', 'violation_4', 'violation_5'];
        ...
        )).thenAnswer((_) async => throw PermissionDeniedException('User is banned from commenting'));
    ...
    group('Swedish Language Support', () {
      test('should handle Swedish comments with special characters', () async {
        ...
        const recipeId = 'recipe_123';
        const userId = 'test_user_123';
        const swedishContent = 'Jättegott recept! Köttbullarna blev perfekta med lingonsylt och gräddsås.';
