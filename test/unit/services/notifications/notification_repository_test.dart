  ...
  group('NotificationRepository', () {
    ...
    group('Preference Management', () {
      ...
      test('should update preferences in Firestore and cache', () async {
        ...
        expect(cachedPrefs.categorySettings[NotificationCategory.friends], isFalse);
      ...
      test('should save preferences locally for offline access', () async {
        ...
        final savedJson = prefs.getString('notification_preferences_$testUserId');
    ...
    group('Notification Type Checking', () {
      test('should check if user wants to receive specific notification', () async {
        // Arrange
        await fakeFirestore
            .collection('notification_preferences')
            .doc(testUserId)
        ...
        expect(shouldReceiveRecipes, isTrue); // Default when not explicitly disabled
      });
      
      test('should default to true for immediate notifications on error', () async {
    ...
    group('Device Token Management', () {
      ...
      test('should batch update device information', () async {
        ...
        // Assert
        final device1 = await fakeFirestore
            .collection(collection)
            .doc('device-1')
            .get();
        final device2 = await fakeFirestore
            .collection(collection)
            .doc('device-2')
            .get();
      ...
      test('should cleanup old devices', () async {
        ...
        // Assert
        final oldDevice = await fakeFirestore
            .collection(collection)
            .doc('old-device')
            .get();
        ...
        expect(recentDevice.data()?['isActive'], isTrue); // Should remain active
      });
      ...
      test('should update device last seen timestamp', () async {
        ...
        // Assert
        final device = await fakeFirestore
            .collection(collection)
            .doc(deviceId)
            .get();
      ...
      test('should deactivate all user devices', () async {
        ...
        await fakeFirestore.collection(collection).doc('other-user-device').set({
          'userId': 'other-user',
          'isActive': true,
        });
        ...
        expect(otherDevice.data()?['isActive'], isTrue); // Should not be affected
      });
    });
...
class NotificationBatch {
  ...
  factory NotificationBatch.fromMap(String id, Map<String, dynamic> map) {
    return NotificationBatch(
      ...
      userId: map['userId'] ?? '',
      notifications: List<Map<String, dynamic>>.from(map['notifications'] ?? []),
      count: map['count'] ?? 0,
    );
  }
}