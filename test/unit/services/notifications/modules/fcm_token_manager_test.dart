...
void main() {
  ...
  group('FCMTokenManager', () {
    ...
    setUp(() async {
      ...
      when(() => mockRepository.getPreferences()).thenAnswer((_) async => 
      ...
      when(() => mockRepository.cleanupOldDevices(any(), any(), olderThan: any(named: 'olderThan')))
          .thenAnswer((_) async {});
      ...
      when(() => mockMessaging.getToken()).thenAnswer((_) async => 'test-fcm-token-001');
      
      // Use empty stream initially to avoid immediate token refresh
      when(() => mockMessaging.onTokenRefresh).thenAnswer((_) => Stream.empty());
    ...
    group('Token Lifecycle', () {
      test('should initialize and register initial token', () async {
        ...
        verify(() => mockRepository.saveTokenToFirestore(any(), any(), any())).called(greaterThanOrEqualTo(1));
      });
      
      test('should handle token refresh from FCM', () async {
        // Arrange
        final refreshController = StreamController<String>();
        when(() => mockMessaging.onTokenRefresh).thenAnswer((_) => refreshController.stream);
        ...
        // Assert
        verify(() => mockRepository.saveTokenToFirestore(any(), any(), any())).called(greaterThanOrEqualTo(2));
      ...
      test('should force refresh token when requested', () async {
        // Arrange
        await tokenManager.initialize();
        when(() => mockMessaging.getToken()).thenAnswer((_) async => 'new-token-456');
        
        // Act
        await tokenManager.refreshToken();
        
        // Assert
        verify(() => mockMessaging.getToken()).called(2); // Once in init, once in refresh
        verify(() => mockRepository.saveTokenToFirestore(any(), any(), any())).called(greaterThanOrEqualTo(2));
      });
      ...
      test('should cleanup on logout', () async {
        ...
        // Assert
        verify(() => mockMessaging.unsubscribeFromTopic('user_test-user-123')).called(1);
      ...
      test('should handle null token from Firebase', () async {
        ...
        verifyNever(() => mockRepository.saveTokenToFirestore(any(), any(), any()));
      });
    });
    
    group('Topic Subscriptions', () {
      test('should subscribe to user topic when updating preferences', () async {
        ...
        // Assert
        verify(() => mockMessaging.subscribeToTopic('user_test-user-123')).called(1);
      });
      ...
      test('should update topic subscriptions based on preferences', () async {
        ...
        // Assert
        verify(() => mockMessaging.subscribeToTopic('system_updates')).called(1);
        verify(() => mockMessaging.subscribeToTopic('social_digest')).called(1);
        verify(() => mockMessaging.subscribeToTopic('recipe_recommendations')).called(1);
        verify(() => mockMessaging.subscribeToTopic('friend_activity')).called(1);
      });
      
      test('should unsubscribe from topics when preferences disabled', () async {
        // Arrange
        await tokenManager.initialize();
        ...
        // Assert
        verify(() => mockMessaging.unsubscribeFromTopic('system_updates')).called(1);
        verify(() => mockMessaging.unsubscribeFromTopic('social_digest')).called(1);
        verify(() => mockMessaging.unsubscribeFromTopic('recipe_recommendations')).called(1);
        verify(() => mockMessaging.unsubscribeFromTopic('friend_activity')).called(1);
      });
    ...
    group('Error Handling', () {
      ...
      test('should handle token fetch failure', () async {
        // Arrange
        when(() => mockMessaging.getToken()).thenAnswer((_) async => throw Exception('Network error'));
      ...
      test('should handle topic subscription failure gracefully', () async {
        ...
        when(() => mockMessaging.subscribeToTopic(any()))
            .thenAnswer((_) async => throw Exception('Topic subscription failed'));
    ...
    group('Device Management', () {
      test('should update device info on token registration', () async {
        ...
        // Assert
        verify(() => mockRepository.updateDeviceInfo(any(), any(), any())).called(1);
      });
