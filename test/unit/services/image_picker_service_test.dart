...
void main() {
  ...
  group('ImagePickerService Tests', () {
    ...
    group('pickImage', () {
      test('should successfully pick image from camera with granted permission',
          () async {
        // Arrange
        ...
        verify(() => mockPermissionProvider.checkPermission(Permission.camera))
            .called(1);
      ...
      test('should request camera permission when denied', () async {
        ...
        verify(() => mockPermissionProvider.checkPermission(Permission.camera))
            .called(1);
        verify(() =>
                mockPermissionProvider.requestPermission(Permission.camera))
            .called(1);
      });
      ...
      test('should handle gallery permission with limited access', () async {
        ...
        // Assert
        expect(result, isNotNull);
        verify(() => mockPermissionProvider.checkPermission(Permission.photos))
            .called(1);
      });

      test(
          'should fallback to storage permission when photos permission is permanently denied',
          () async {
        // Arrange
        final mockXFile = MockXFile();
        ...
        verify(() => mockPermissionProvider.checkPermission(Permission.photos))
            .called(1);
        verify(() => mockPermissionProvider.checkPermission(Permission.storage))
            .called(1);
      });
      ...
      test('should return null when image file validation fails', () async {
        ...
        when(() => mockXFile.path).thenReturn('/path/to/image.jpg');
        when(() => mockImageValidator.isValidImageFile(any()))
            .thenReturn(false);
    ...
    group('debugPermissions', () {
      test('should check all permission statuses', () async {
        ...
        when(() => mockPermissionProvider.checkPermission(Permission.storage))
            .thenAnswer((_) async => PermissionStatus.denied);
        when(() =>
                mockPermissionProvider.checkPermission(Permission.mediaLibrary))
            .thenAnswer((_) async => PermissionStatus.permanentlyDenied);
