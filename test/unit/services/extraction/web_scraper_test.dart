  ...
  group('WebScraper', () {
    ...
    group('Platform Detection Integration', () {
      test('should handle Instagram URLs', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(instagramUrl, platform);
        
        // Assert
        expect(result.success, isTrue);
        expect(result.metadata['platform'], equals('instagram'));
        verify(() => mockWebScraper.performExtraction(instagramUrl, platform)).called(1);
      });
      ...
      test('should handle Facebook URLs', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(facebookUrl, platform);
        
        // Assert
        expect(result.success, isTrue);
        expect(result.metadata['platform'], equals('facebook'));
        verify(() => mockWebScraper.performExtraction(facebookUrl, platform)).called(1);
      });
      ...
      test('should handle TikTok URLs', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(tiktokUrl, platform);
        
        // Assert
        expect(result.success, isTrue);
        expect(result.metadata['platform'], equals('tiktok'));
        verify(() => mockWebScraper.performExtraction(tiktokUrl, platform)).called(1);
      });
      ...
      test('should handle generic/unknown platform URLs', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(genericUrl, platform);
        
        // Assert
        expect(result.success, isTrue);
        expect(result.metadata['platform'], equals('unknown'));
        verify(() => mockWebScraper.performExtraction(genericUrl, platform)).called(1);
      });
    });
    ...
    group('URL Validation', () {
      test('should handle empty URLs', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(emptyUrl, platform);
        
        // Assert
        expect(result.success, isFalse);
        expect(result.error, contains('Invalid URL'));
        verify(() => mockWebScraper.performExtraction(emptyUrl, platform)).called(1);
      });
      ...
      test('should handle invalid URLs', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(invalidUrl, platform);
        
        // Assert
        expect(result.success, isFalse);
        expect(result.error, contains('Invalid URL format'));
        verify(() => mockWebScraper.performExtraction(invalidUrl, platform)).called(1);
      });
      
      test('should handle URLs with special characters', () async {
        // Arrange
        const specialUrl = 'https://example.com/recipe?id=123&type=dessert#ingredients';
        const platform = SourcePlatform.unknown;
        final expectedResult = ExtractionResult(
          success: true,
        ...
        // Act
        final result = await mockWebScraper.performExtraction(specialUrl, platform);
        
        // Assert
        expect(result.success, isTrue);
        verify(() => mockWebScraper.performExtraction(specialUrl, platform)).called(1);
      });
      ...
      test('should handle URLs with unicode characters', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(unicodeUrl, platform);
        
        // Assert
        expect(result.success, isTrue);
        verify(() => mockWebScraper.performExtraction(unicodeUrl, platform)).called(1);
      });
      ...
      test('should handle very long URLs', () async {
        ...
        // Act
        final result = await mockWebScraper.performExtraction(longUrl, platform);
        
        // Assert
        expect(result.success, isFalse);
        expect(result.error, contains('URL too long'));
        verify(() => mockWebScraper.performExtraction(longUrl, platform)).called(1);
      });
    });
    ...
    group('Timeout Behavior', () {
      test('should handle extraction timeout', () async {
        ...
        mockWebScraper.setTimeoutBehavior(
          shouldTimeout: true,
    ...
    group('Concurrent Operations', () {
      test('should handle multiple concurrent extractions', () async {
        ...
        for (int i = 0; i < urls.length; i++) {
          when(() => mockWebScraper.performExtraction(urls[i], SourcePlatform.unknown))
              .thenAnswer((_) async => ExtractionResult(
                success: true,
      ...
      test('should handle concurrent extractions with mixed results', () async {
        ...
        for (final testCase in testCases) {
          ...
          when(() => mockWebScraper.performExtraction(url, SourcePlatform.unknown))
              .thenAnswer((_) async => ExtractionResult(
                success: success,
        ...
        // Act
        final futures = testCases.map((tc) =>
    ...
    group('JavaScript Selector Strategies', () {
      test('should extract content using platform-specific selectors', () async {
        // Arrange
        ...
        final expectedResult = ExtractionResult(
          success: true,
          extractedText: 'Instagram post content extracted with specific selectors',
          metadata: {
    ...
    group('Swedish Content Support', () {
      test('should properly extract Swedish characters', () async {
        ...
        final expectedResult = ExtractionResult(
          success: true,
          extractedText: 'Köttbullar med lingon och gräddsås. Använd färsk timjan.',
    ...
    group('Error Recovery', () {
      test('should recover from WebView initialization errors', () async {
        ...
        when(() => mockWebScraper.performExtraction(url, platform))
            .thenAnswer((_) async => ExtractionResult(
              success: false,
              error: 'WebView initialization failed - recovered with empty result',
            ));
    ...
    group('Resource Cleanup', () {
      ...
      test('should handle disposal during extraction', () async {
        ...
        // Act
        final extractionFuture = mockWebScraper.performExtraction(url, platform);
