...
// Using centralized MockAccountDeletionRepository from production_mocks.dart
...
void main() {
  group('AccountDeletionService - Unit Tests', () {
    ...
    setUp(() async {
      ...
      // Configure mock user using setUserState
    ...
    tearDown(() async {
      mockRepository.resetDeletionState();
    ...
    group('Error Handling', () {
      ...
      test('should handle offline service failure', () async {
        // Arrange
        when(() => mockOfflineService.clearUserData(any())).thenAnswer(
            (_) async => throw Exception('Offline storage unavailable'));
        ...
        expect(
            result['errors']
                .any((e) => e.toString().contains('Offline storage')),
            isTrue);
      });
    ...
    group('Business Logic', () {
      test('should execute deletions in correct order', () async {
        ...
        // Act
        final result =
            await coordinator.deleteUserAccount(reason: 'Test order');
      ...
      test('should log correct analytics data', () async {
        ...
        // Act
        final result =
            await coordinator.deleteUserAccount(reason: 'Test analytics');
      ...
      test('should mark success only when all deletions succeed', () async {
        ...
        // Act
        final result1 =
            await coordinator.deleteUserAccount(reason: 'All success');
        ...
        // Act
        final result2 =
            await coordinator.deleteUserAccount(reason: 'One failure');
      ...
      test('should provide detailed error information', () async {
        ...
        mockRepository.setDeletionResult('recipes', false);
        when(() => mockOfflineService.clearUserData(any())).thenAnswer(
            (_) async => throw Exception('Storage error: disk full'));

        // Act
        final result =
            await coordinator.deleteUserAccount(reason: 'Test errors');

        // Assert
        expect(result['success'], isFalse);
        expect(result['failedCollections'],
            containsAll(['recipes', 'offline_cache']));
        expect(result['errors'], isNotEmpty);
        expect(
            result['errors'].any((e) => e.toString().contains('Storage error')),
            isTrue);
      });
    });
