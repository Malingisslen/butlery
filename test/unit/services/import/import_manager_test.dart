...
void main() {
  group('ImportManager', () {
    ...
    setUp(() {
      ...
      when(() => mockPersonalOperations.addUnifiedRecipe(any())).thenAnswer(
          (_) async => RecipeOperationResult.success('Recipe added'));
    });
    ...
    group('Auto Import', () {
      ...
      test('should try preferred strategy first if provided', () async {
        ...
        verify(() => mockStrategy.import(input, options: any(named: 'options')))
            .called(1);
      });
      ...
      test('should fallback to other strategies if preferred fails', () async {
        ...
        when(() => mockStrategy.import(input, options: any(named: 'options')))
            .thenAnswer((_) async =>
                import_strategy.ImportResult.failure('Strategy failed'));
      ...
      test('should return failure when no strategy can handle input', () async {
        ...
        expect(
            result.errorMessage, contains('No import strategy could handle'));
      ...
      test('should pass options to strategy', () async {
        ...
        when(() => mockStrategy.import(input, options: options)).thenAnswer(
            (_) async => import_strategy.ImportResult.success(testRecipe));
    ...
    group('Strategy Management', () {
      test('should get compatible strategies for input', () {
        ...
        const archiveInput =
            'archive:recipe-123'; // Or whatever format archive expects
        const textInput = '''
        ...
        // Act
        importManager.getCompatibleStrategies(
            archiveInput); // Test it runs without error
      ...
      test('should validate input with specific strategy', () {
        ...
        // Act
        final validResult =
            importManager.validateInput(validInput, strategy: textStrategy);
        final invalidResult =
            importManager.validateInput(invalidInput, strategy: textStrategy);
    ...
    group('Direct Recipe Save', () {
      test('should save imported recipe directly', () async {
        ...
        verify(() => mockPersonalOperations.addUnifiedRecipe(testRecipe))
            .called(1);
      });
    ...
    group('Edge Cases', () {
      ...
      test('should handle strategy returning null recipe', () async {
        ...
        when(() => mockStrategy.import(input, options: any(named: 'options')))
            .thenAnswer(
                (_) async => import_strategy.ImportResult.success(null));
      ...
      test('should handle PersonalRecipeOperations save failure', () async {
        ...
        when(() => mockPersonalOperations.addUnifiedRecipe(testRecipe))
            .thenAnswer(
                (_) async => RecipeOperationResult.failure('Database error'));
