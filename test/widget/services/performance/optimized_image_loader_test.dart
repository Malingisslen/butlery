...
void main() {
  group('ImageOptimizationParams', () {
    ...
    test('should handle URL with existing query parameters', () {
      ...
      const originalUrl = 'https://firebasestorage.googleapis.com/image.jpg?token=abc';
  ...
  group('ImageMemoryCacheManager', () {
    ...
    test('should calculate hit rate correctly', () {
      ...
      final hitRate = (totalHits / (totalHits + totalMisses) * 100).toStringAsFixed(1);
  ...
  group('OptimizedImageLoader Widget', () {
    ...
    setUp(() {
      ...
      when(() => mockConfig.effectiveBorderRadius).thenReturn(BorderRadius.circular(8));
    ...
    }, skip: true); // CachedNetworkImage cannot handle infinity for cache dimensions
  });
  ...
  group('Widget State Management', () {
    ...
    testWidgets('should show default error widget when no custom error provided', 
        (WidgetTester tester) async {
      // Arrange & Act
      await tester.pumpWidget(
        TestApp(
  ...
  group('Image Optimization Features', () {
    ...
    testWidgets('should apply BoxFit correctly', 
        (WidgetTester tester) async {
      // Arrange
      final mockConfig = MockImageConfig();
      when(() => mockConfig.borderRadius).thenReturn(null);
