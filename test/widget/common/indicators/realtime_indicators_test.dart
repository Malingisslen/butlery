...
void main() {
  group('RealtimeIndicators.editIndicator', () {
    testWidgets('constructs an EditIndicatorWidget with forwarded props',
        (tester) async {
      await tester.pumpWidget(_wrap(RealtimeIndicators.editIndicator(
        editorName: 'Anna',
        editorId: 'u-1',
      ...
      await tester.pump();
      final w =
          tester.widget<EditIndicatorWidget>(find.byType(EditIndicatorWidget));
  ...
  group('RealtimeIndicators.participantList', () {
    testWidgets('constructs a ParticipantListWidget with forwarded props',
        (tester) async {
      await tester.pumpWidget(_wrap(RealtimeIndicators.participantList(
        activities: const [],
        currentUserId: 'me',
      )));
      final w = tester
    ...
    testWidgets('onRemoveParticipant + onChangePermission callbacks forwarded',
        (tester) async {
  ...
  group('RealtimeIndicators.realtimeStatus', () {
    testWidgets('constructs a RealtimeStatusWidget with forwarded props',
        (tester) async {
      await tester.pumpWidget(_wrap(RealtimeIndicators.realtimeStatus(
        isOnline: true,
        statusDescription: 'Online',
        statusEmoji: '🟢',
      )));
      final w = tester
          .widget<RealtimeStatusWidget>(find.byType(RealtimeStatusWidget));
  ...
  group('RealtimeIndicators.realtimeStatusBanner', () {
    testWidgets('constructs a RealtimeStatusBanner with forwarded props',
        (tester) async {
      void onRetry() {}
      await tester.pumpWidget(_wrap(RealtimeIndicators.realtimeStatusBanner(
        isOnline: false,
      ...
      final w = tester
          .widget<RealtimeStatusBanner>(find.byType(RealtimeStatusBanner));
