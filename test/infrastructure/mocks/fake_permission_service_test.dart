...
void main() {
  group('FakePermissionService.setPermissionState auth derivation', () {
    test(
      'currentUser without explicit isAuthenticated derives authenticated',
      () {
    ...
    test(
      'explicit isAuthenticated:false is honored despite currentUser set',
      () {
        final svc = FakePermissionService();
        final user = UserBuilder().withId('u1').withName('Alice').build();
        ...
        expect(
          ...
          isFalse,
          reason:
              'explicit isAuthenticated:false must override the '
    ...
    test(
      'explicit isAuthenticated:true with currentUser stays authenticated',
      () {
        final svc = FakePermissionService();
        final user = UserBuilder().withId('u1').withName('Alice').build();
