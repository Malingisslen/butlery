...
class TestDataIsolator {
  ...
  static void initializeTest(String testName) {
    ...
    debugPrint('Initialized test isolation for: $testName with prefix: $prefix');
  }
  ...
  static String isolateCollection(String baseCollection) {
    if (_currentTestName == null) {
      debugPrint('Warning: No test isolation active, using base collection name');
...
extension FirestoreIsolationExtensions on FirebaseFirestore {
  ...
  DocumentReference<Map<String, dynamic>> isolatedDoc(String collectionPath, String docId) {
...
class IsolatedTestData {
  ...
  factory IsolatedTestData.current() {
    ...
    if (testName == null || prefix == null) {
      throw StateError('No test isolation active. Call TestDataIsolator.initializeTest first.');
    }
