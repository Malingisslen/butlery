    ...
    data.forEach((key, value) {
      if (value is Timestamp) {
        ...
        converted[key] = value.map((item) {
          ...
          if (item is Map<String, dynamic>) return convertTimestampsToDateTime(item);
          return item;
        }).toList();
      } else {
        converted[key] = value;
    ...
    data.forEach((key, value) {
      if (value is DateTime) {
        ...
        converted[key] = value.map((item) {
          ...
          if (item is Map<String, dynamic>) return convertDateTimeToTimestamps(item);
          return item;
        }).toList();
      } else {
        converted[key] = value;
...
extension DateTimeTestExtensions on DateTime {
  ...
  bool isCloseTo(dynamic other, {Duration tolerance = const Duration(seconds: 1)}) {
    return TimestampTestHelper.areTimestampsEqual(this, other, tolerance: tolerance);
  }
}
...
class TimestampMatcher extends Matcher {
  ...
  const TimestampMatcher(this.expected, {
