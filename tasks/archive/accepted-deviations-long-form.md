# Accepted deviations — long-form entries (archive)

Not auto-loaded. Verbatim copy of `.claude/rules/accepted-deviations.md` as it read on 2026-09-19,
before each entry was cut to its current verdict (Malin's approval, context-diet). The full
rationale per entry is also in `docs/architecture/ACCEPTED_DEVIATIONS.md`.
...
- **SUPERSEDES the BUT-2062 entry's claims about the ratings WRITER, which BUT-2057 falsified
  ...
  Retired verbatim (fragment; the original wraps): "`recipeOwnerId` on RATINGS is a decided strip although `FirebaseRatingsRepository` never"
  Retired verbatim: "`firebase_ratings_repository.dart` is already 536 against a row saying 507"
  The `ACCEPTED_LARGE_FILES` row is updated in this commit to the measured
  count. The layer argument
  that count supported is unaffected: the projection stays in the manager.
  The two mirrors word this fact DIFFERENTLY, so a grep for one misses the other. The sibling
  supersession is in `docs/architecture/ACCEPTED_DEVIATIONS.md` and quotes that file's wording.
...
- **SUPERSEDES the BUT-2062 entry's residual about the two create limbs (BUT-2079,
  ...
  Retired verbatim (fragment; the original wraps): "And neither create limb carries `keys().hasOnly`, so a hand-rolled client can store a field of its own and have its OWN content dropped; the `data_minimisation` sentence says so, and tightening the limbs is its own ticket."
  The two mirrors word this fact DIFFERENTLY; the sibling supersession in
  `docs/architecture/ACCEPTED_DEVIATIONS.md` quotes that file's wording. BUT-2079, 2026-09-11
