...
Example: 2026-09-17/18 — ingen mening shippade. En tredje granskare mätte att även den strukna versionen beskrev fel sak: listkopians `listData.items` exporteras redan, och det som saknas är en separat undersamling utan skrivare. Malin valde att återställa strängen till sina bytes före ändringen hellre än att låta en fjärde formulering prövas. Symbolen rättad i tre filer.
...
### Ett larms EGEN kropp kan inte dateras av `createdAt` (2026-09-17)
Date: 2026-09-17
Trigger: Malin frågade om en köad fix för `main-health-alert.yml` fortfarande var aktuell: sortera de hämtade körningarna på skapelsedatum själv, bortse från en körning vars kod inte ligger bland de senaste incheckningarna på main, och skriv VARFÖR med det falska larmet 2026-08-28 som exempel. Båda halvorna saknades fortfarande i filen. Det svåra var att DATERA det falska larmet: larmet kör `issues.update({body})` på varje röd körning, så kroppen jag läste är inte nödvändigtvis den som skrevs vid `created_at`. Att `updated_at == closed_at` (2026-08-28T18:34:37Z) och att enda kommentaren är autostängningen bevisade att den röda kroppen aldrig skrevs om, vilket är det som gav rätt att datera citatet till 16:22:11Z. Först DÅ gick premissen att mäta: vid det klockslaget var Build Validations nyaste avslutade main-körning 2026-08-27T15:18:24Z och GRÖN, med två gröna körningar bakom sig, medan larmet namngav en körning skapad 2026-08-08 (attempt 1, aldrig omkörd). Svaret var alltså inte nyast-först.
Rule: Ett API-svars ORDNING är ett påstående, inte en garanti — sortera själv när en `find()` avgör om något larmar. Och en artefakt som SKRIVS OM av den automatik som skapade den kan inte dateras av sin skapelsetid: leta efter det som bevisar att den inte rörts (`updated_at`, kommentarslistan) innan du bygger ett resonemang på när innehållet påstods. Den dyraste raden i den här ändringen var min egen motivering till fixens andra halva — jag skrev att sortering ENSAMT inte hade räddat larmet, vilket mätningen ovan motbevisade; en motivering är ett påstående om den väg man INTE öppnat.
Example: 2026-09-17 — `main-health-alert.yml` sorterar nu per gate på `created_at` själv och filtrerar bort körningar vars `head_sha` inte finns bland mains senaste 100 incheckningar (den falska körningens commit låg 184 incheckningar bak, så båda skydden fångar just det larmet var för sig). Ärende #244 var redan autostängt 2026-08-28T18:34:37Z och main är grönt idag.

### En röd regelsvit kan vara emulatorns TILLSTÅND, inte en regel (2026-09-19)
Date: 2026-09-19
Trigger: BUT-2105 beskrev två röda ALLOW-fall i `audit-logs-rules.test.ts` och gissade på regeldrift (åldersspärren, ADR-0020:s stämpelkrav). En hel körning gav fem röda sviter, alla på ALLOW-CREATE, vilket läste som en gemensam skärpning. Mätningen visade motsatsen: en emulator som legat igång sedan tidigare körningar hade kvar dokument på sviternas EGNA fasta id:n, så varje `set()` utvärderades som en update och nekades av `update: if false` eller en oföränderlighetsspärr. Fem sviter, en orsak, noll regelfel. Den femte röda var en helt annan sak: storage-emulatorn kördes inte (ECONNREFUSED 9199), och runnern startar den inte lokalt.
Rule: Innan en röd regelsvit tolkas som regeldrift: fråga vad som redan LIGGER i emulatorn på testets doc-id. Ett fel som bara träffar ALLOW-CREATE-fall är fingeravtrycket för kvarliggande data, inte för en skärpt regel — CI ser det aldrig, eftersom varje jobb startar en ny emulator. Och omvänt, i samma familj: en DENY som utvärderas som update är grön av fel skäl, vilket `clearFirestore()` i `setup()` INTE lagar när kollisionen sker inom samma körning.
Example: 2026-09-19 — `env.clearFirestore()` i `setup()` för audit-logs, menus, realtime-menus och acquisition; grönt två gånger i rad på samma emulator, mutationsprövat per create-regel. Restpunkterna filade som BUT-2119 (fler sviter utan rensning, och acquisitions egna create-DENY-fall som landar på update-grenen) och BUT-2118 (appens måltidsröst skriver ett slumpat id där regeln kräver uid).

### En kommentarsrättelse mitt i en grindrunda ogiltigförklarar varje granskares täckning (2026-09-20)
Date: 2026-09-20
Trigger: BUT-1988/1987/1986 tog SEX granskningsrundor, och noll av dem handlade om en kodbugg efter den första. Varje runda gick till: en granskare hittar en mening som min egen ändring gjort falsk (tjänstens borttagna metoder namngivna i `withPresence`-docblocket, `_executeWrite`:s uppräkning av vem som passerar `guarded: false`, `_publishInFlight`-fältets "carried ONLY by…", widgetgruppens "BOTH branches of the gate are driven", en proveniensats i ett nytt test) → jag stryker den → strykningen ändrar filens bytes → grindens täckning är nyckelad på bytes, så varje granskare som läst filen måste läsa om → nästa granskare hittar nästa mening. Kedjan är inte slumpmässig: en ändring som TAR BORT kod gör systematiskt varje mening som räknade upp den koden falsk, och de meningarna ligger utspridda i filer olika granskare äger.
Rule: När en ändring tar bort en metod, ett fält eller en gren: grepa efter varje mening som RÄKNAR UPP det som togs bort, i hela diffens filer, INNAN granskarna skickas ut — "ONLY", "the two methods above", "the writes that…", "BOTH branches" är sökorden. Stryk allihop i samma redigering. Gör man det efteråt, en mening per runda, betalar man en full omläsning per mening — och varje omläsning är en ny chans att hitta nästa. Motsatt fälla finns också: en granskare som säger "detta blockerar inte, fixa gärna" kostar lika mycket som ett blockerande fynd om man tar det efter att täckningen registrerats. Samla dem, eller skjut dem till en biljett.
Example: 2026-09-20 — fem satser strukna över sex rundor i `weekly_menu_plan_viewmodel.dart`, `weekly_menu_plan_service.dart` och `calendar_weekly_menu_widget_test.dart`. Ett grep på "ONLY|the two methods|BOTH branches" i de tre filerna före första utskicket hade hittat fyra av fem.

### Ett muteringsprov över HELA testfilen krediterar rött till fel påstående (2026-09-20)
Date: 2026-09-20
Trigger: BUT-1899 lade till ett test som binder `direct_`-prefixet på myntstället till `LogSanitizer`. Jag muterade prefixet, körde `flutter test <fil>`, såg rött och skrev "muteringsprovat" i både kommentar, commit-meddelande och granskarbrief. Kodgranskaren räknade ut att mitt test måste vara grönt under fog-mutationen; körd med `--plain-name` på det ENDA testet var det grönt. Röden kom från två äldre påståenden i samma fil som redan spikade id-strängen som literal. Skyddet jag trodde jag hade byggt fanns inte, och hade shippat bakom tre meningar som påstod motsatsen. Andra halvan samma runda: jag skrev att en ändring av SKILJETECKNET alltid fångas av den generella uid-regeln, mätt på EN substitution (`-`). Helhetsgranskaren körde fyra skiljetecken; tas tecknet bort helt blir id:t en 56-teckens körning som regeln inte matchar, och båda uid:na går ut råa. Kodgranskaren hade godkänt meningen som "mätt".
Rule: Ett muteringsprov MÅSTE köras skopat till det enda testet (`--plain-name`) och rapporteras med den fallande radens utskrift — en körning över hela filen gör varje rött till bevis för vilket test man råkar tänka på, och det är exakt den formen som tillverkar ett falskt "detta är pinnat". Och ett prov på EN variant av en ändring bevisar inte en klass av ändringar: skriver man "varje ändring av X" måste varje gren av X ha körts, annars är kvantifikatorn ett nytt omätt påstående. En granskare som HÅLLER MED om ett sådant påstående är inte en mätning — den som körde kommandot vinner, oavsett ordning.
Example: 2026-09-20 — `conversation_mutation_module_test.dart` fick en tredje kontroll (`expect(inFreeText, contains(masked))`) som jämför de två maskerarna mot varandra i stället för att bara leta läckta uid; skopat prov rött på rad 90 med båda strängarna utskrivna. Den överdrivna satsen ströks — först försökte jag snäva in den, vilket är en ny formulering och alltså ett nytt påstående, och backade.

### "Är detta aktuellt?" mäts mot ärendets EGEN historik, inte mot koden (2026-09-20)
Date: 2026-09-20
Trigger: Malin klistrade in ett BUT-2094-uppdrag ("du valde alternativ B, bygg sektionen") och frågade om det var aktuellt. Jag mätte koden — noll rader i produktion, ingen skrivare kvar, vägen inte längre klientläsbar — rekommenderade motsatsen, fick klartecken och började redigera avvikelseregistren. Först vid grep efter `BUT-2094` i repot såg jag `1c69969b2` från 2026-09-18: frågan var redan ställd i en full panel 2026-09-17, redan besvarad (ingen sektion), redan bordlagd till BUT-1747 som FÖRLANSERINGSGRIND, och den `data_minimisation`-mening jag höll på att rekommendera bort hade redan strukits av tre granskare av exakt samma skäl jag just "upptäckt". Min post beskrev ett nytt beslut som inte fanns och skulle ha daterat om ett befintligt. Backade båda filerna.
Rule: En "är detta aktuellt?"-fråga har TVÅ mätningar, och kodmätningen är den andra. Den första är ärendets egen historik: `git log --all --grep=<ID>` plus en grep på ID:t genom repot, FÖRE första rekommendationen. Ett beslutsärende kan ha besvarats utan att stängas, och ett svar som ligger i ett annat ärendes ändring syns inte i statusfältet. Den dyraste formen är när kodmätningen är RIKTIG och leder till rätt slutsats — då känns rekommendationen bevisad, och det som saknas är bara att någon redan kommit dit. Att skriva en daterad beslutspost för ett beslut som redan är daterat är dessutom precis den nya prosa som kodstilens supersederingsregel finns för att hindra.
Example: 2026-09-20 — BUT-2094 stängd som besvarad med en kommentar som citerar `1c69969b2`, och BUT-1747:s beskrivning utökad med den andra luckan den bar utan att nämna. Noll kodändringar, noll nya rader i avvikelseregistren.

### En granskares VERDIKTRAD registreras separat från dess läsningar (2026-09-20)
Date: 2026-09-20
Trigger: Sprinten 2026-09-20 runda 6. Två granskare läste varje stagead fil, rapporterade fullständigt och avslutade med prosa EFTER sin `REVIEW-VERDICT:`-rad. Commit-grinden vägrade: "read by a code-reviewer run that never recorded its verdict line". Läsningarna låg i liggaren, alla fynd var åtgärdade, koden var klar — och ingenting av det spelade roll, för raden som stänger en granskning registreras separat från läsningarna. Kostnaden var två extra väckningar av samma körning, och varje väckning är en ny chans att filen hinner ändras under omläsningen. Samma runda visade den dyrare tvillingen: en rättelse på ETT ord i en fil som fyra granskare läst kostade tre omläsningar, för liggaren är nyckelad på bytes.
Rule: Be varje granskare skicka verdiktraden som ETT EGET meddelande, efter rapporten — och verifiera efteråt med `grep '"t":"verdict"' .claude/state/review-ledger.jsonl | tail`, aldrig genom att läsa granskarens rapport. Skriv INTE varför det fungerar: registreraren (`workflow-guards/scripts/review-verdict.mjs`) läser `last_assistant_message` och tar SISTA `REVIEW-VERDICT:`-träffen var som helst i fältet, så "inget efter raden" är inte det som avgör. Det mätta är utfallet: rapporter som slutade på raden registrerades inte, ett meddelande som bara var raden registrerades direkt. En rapport som SÄGER "pass" och en liggare som registrerat "pass" är två olika saker, och det är bara den andra grinden läser. Väck samma körning i stället för att starta en ny granskning: läsningarna står kvar, och en ny körning betalar hela omläsningen igen.
Example: 2026-09-20 — 097526861 och 9a1216413 stoppades en gång var på en saknad verdiktrad, från granskare som redan passerat. Samma dag, BUT-2130: FEM rapporter som slutade på raden registrerade noll; ett meddelande som bara var raden registrerades på sekunden.

### Ett upprepat UTFALL är inte en mekanism — öppna verktyget innan du skriver ned varför (2026-09-20)
Date: 2026-09-20
Trigger: Fem granskningsrapporter i rad slutade på sin `REVIEW-VERDICT:`-rad och ingen registrerades i liggaren; ett meddelande som bara var raden registrerades direkt. Jag skrev ned slutsatsen som en mekanism — "raden registreras bara när den är HELA det sista meddelandet" — i sprintplanen, i en lärdom, i dess rubrik och i den alltid laddade digest-raden, och briefade tre granskare på den. Den var fel. `review-verdict.mjs` läser `last_assistant_message` och tar SISTA `REVIEW-VERDICT:`-träffen var som helst i fältet; senare samma runda registrerades flera fullständiga rapporter som slutade på raden. Fem observationer kändes som en mätning för att de var samstämmiga, och de fem misslyckandena är fortfarande oförklarade.
Rule: Ett utfall du sett N gånger bevisar utfallet, aldrig orsaken. Innan en FÖRKLARING skrivs in någonstans som överlever turen — en lärdom, en digest-rad, en kodkommentar, en granskarbrief — öppna det som avgör: hooken, skriptet, regelfilen. Går det inte att öppna, skriv utfallet och säg att orsaken är okänd; "praktiken som fungerat" är ett ärligt påstående, "mekanismen är X" är det inte. Den dyraste formen är när praktiken FAKTISKT fungerar: då bekräftar varje lyckad körning vanan och ingenting testar förklaringen. Och rättelsen är inte klar förrän den nått varje kopia — rubriken ovanför den rättade meningen bar kvar den motbevisade lydelsen och motsade sin egen regel två rader ned.
Example: 2026-09-20 — bceac531b. Struken i `tasks/lessons.md` (både rubrik och Rule-rad), i `.claude/rules/lessons-digest.md` och i sprintplanens avvikelselogg, i samma commit som den kod rundan handlade om. Helhetsgranskaren blockerade det första försöket: planen påstod att rättelsen landat medan lärdomsfilerna låg ostagade, så HEAD hade fortfarande den motbevisade raden i den alltid laddade digesten.

### En strykning måste mäta BÅDA satserna — annars överlever den falska (2026-09-20)
Date: 2026-09-20
Trigger: BUT-1890. Fyra guldfall fick noten "nuts are a snack aisle, which is the answer the shopping dialog's old map gave before it was retired. The BUT-1666 case this replaces asserted only that 'nöt' as a suffix is not meat — that still holds, via _beefPattern." En claim-lint-grind flaggade meningen. Jag strök den andra satsen, som var SANN och mätbar, och lät den första stå. Granskaren mätte då den överlevande: den gamla kartans obundna `te` rider inuti "nötter" och dess dryckhink låg före snacks, så kartan svarade `drinks` — aldrig `snacks`. Den kvarvarande satsen var det enda i meningen som var falskt, och min egen första mätning i samma session hade redan skrivit ut `ROW|nötter|old=drinks`. Jag hade läst det och ändå strukit fel halva. Det var tredje rundan samma mening var fel.
Rule: När en grind eller granskare flaggar en mening med TVÅ påståenden, avgör inte utifrån vilket som lät mest som en siffra. Mät varje sats för sig innan du väljer vilken som ska bort, och leta i sessionens egen utdata först — mätningen som avgör är ofta redan utskriven. En strykning är bara säker när det som blir kvar också är mätt; "en deletion bär inget nytt påstående" gäller det raderade, inte det överlevande. Har samma mening varit fel två gånger, radera hela påståendet i stället för att välja en halva: ett beslut som är Malins behöver ingen motivering i koden.
Example: 2026-09-20 — 1121bdf97. Noten slutade som "BUT-1890: nuts are a snack aisle." Beslutet står; ingen förklaring bakom det.

### En fixtur som svaras av en TIDIGARE regel krediterar fel vakt (2026-09-20)
Date: 2026-09-20
Trigger: BUT-1890. `ingredient_categorizer_vocabulary_test.dart` hade en grupp vars uttalade jobb var att binda varje ordgräns. Fyra av dess fixturer band ingenting: `Salladsskål` och `Fryst spenat` svarades av grönsaksregeln långt före den gräns de krediterade; `Mjöl`/`Vetemjöl` av torrvaruregeln långt före dryckregeln, så `öl`-gränsen kunde tas bort helt utan att ett enda prov rödnade; `Cidervinäger` av skafferiregeln, så `vin`-gränsens AVVISANDE halva var opinnad. Samma form träffade själva blockerarens fix: `Entrecôte` svaras av sitt eget köttnyckelord, så lånebokstavsklassen — hela rättelsen — var raderbar-grön. Dessutom påstod motorns skafferikommentar två ordningsegenskaper ("efter canned så `sojabönor` förblir konserv, efter frukt så `honungsmelon` förblir melon") som ingen fixtur kunde märka om de bröts.
Rule: I en motor där första träff vinner är en fixturs dödsmängd en egenskap hos REGELKEDJAN, inte hos ordet. Innan ett prov krediteras en gräns, spåra vilken regel som faktiskt svarar på just den strängen — den måste NÅ regeln den påstås vakta. Fixturen ska vara ett ord som ingen tidigare regel gör anspråk på (`Bröllopstårta` för `öl`, `Côte de boeuf` för lånebokstäverna, `Vinbär` för `vin`), och varje ordningspåstående i en kommentar behöver en fixtur som byter svar när blocket flyttas. Att flytta ett block är en giltig mutant, inte bara att radera ett nyckelord.
Example: 2026-09-20 — 1121bdf97. Sju fixturer bytta eller tillagda; varje bindning mutationsprövad med filen återställd och md5-verifierad efteråt.

### Backticks i en heredoc-kropp är kommandosubstitution för commit-grinden (2026-09-20)
Date: 2026-09-20
Trigger: `system_events`-retentionen. Commiten skrevs med `-m "$(cat <<'EOF' … EOF)"` och ett meddelande som citerar kod i backticks, så som repots commit-meddelanden alltid gör. `require-review-before-commit` vägrade två gånger och citerade tillbaka en rad ur SJÄLVA MEDDELANDET som "this command runs" — andra gången raden "samlingen i `firestore.indexes.json`, inget städjobb, …". Den första vägran var korrekt (jag hade verkligen ett stagningssteg i samma anrop); den andra fick mig att tro att ordet "commit" i brödtexten var triggern. Det var det inte. Samma sak hände igen när den här lärdomen skulle skrivas med en heredoc — texten utlöste grinden den beskriver.
Rule: Grinden kör `shellSegments()` över HELA råkommandot, och en heredoc-kropp är råtext som vilken annan. Backticks i kroppen parsas som kommandosubstitution, blir ett eget segment, och `readOnlyStep()` säger nej — alltså ett muterande steg före commiten. Ordet "commit" i brödtexten gör ingenting utöver att passera billighetskontrollen högst upp i hooken. Skriv meddelandet till en fil i scratchpad och kör commit-kommandot med `-F <fil>` som ensamt anrop; det är den enda formen där meddelandets innehåll aldrig kan parsas. Gäller `$(…)`, `&&` och `;` i kroppen lika mycket som backticks — och gäller varje heredoc som råkar innehålla ordet, inte bara riktiga commits.
Example: 2026-09-20 — cdc464465. Två vägran, sedan `-F` från scratchpad: igenom på första försöket, elva filer, alla lefthook-grindar gröna. Mekanismen läst i `workflow-guards/scripts/_common.mjs:904-919` (`stepBeforeCommit` → `shellSegments` → `readOnlyStep`), inte gissad ur utfallet.

### En rättelse är en mätning och ärver ingen auktoritet av att komma sist (2026-09-21)
Date: 2026-09-21
Trigger: BUT-2033. Vaktfilens rubrik påstod att två `lib/`-filer bär ett `/*` som räddas av att inget `*/` följer. Tre granskare bekräftade den oberoende, en av dem med orden "`*/` search returns −1 in both". claim-lint blockerade commiten för radnummer i prosa, och när jag härledde ersättningen använde jag en förekomstkontroll: för varje fil med `/*`, finns `*/` någonstans? Den gav fyra andra filer, och `http_content_fetcher.dart` saknades. Jag drog slutsatsen att meningen var fel i antal, urval och mekanism, sa det till Malin, och strök den. Det var min mätning som var fel: förekomst är inte ordning, och i Accept-huvudets `*/*;q=0.8` ligger stängaren på offset 0 och öppnaren på offset 1 — stängaren står FÖRE öppnaren och kan inte stänga den. `testing-specialist` vägrade medge, mätte om med offsetjämförelse och fick fem filer inklusive den jag avfärdat. Jag mätte om själv och bekräftade: stängare finns i filen, men inte efter öppnaren, och noll träffar för strippningens egen regex.
Rule: En rättelse anländer med auktoriteten av att komma sist, och ingen mäter om den — allra minst när den kullkastar något flera granskare bekräftat. Kör den rättande mätningen innan du medger, och innan du berättar för Malin att någon annan hade fel. Kontrollera särskilt att det nya kommandot ställer SAMMA fråga som påståendet: varje förekomstkontroll är ordningsblind, så ett påstående om ett avgränsarPAR (blockkommentarer, citattecken, taggar, parenteser) kräver en offsetjämförelse — sök stängaren FRÅN öppnarens position, aldrig i hela strängen. Att flera granskare säger samma sak är inte heller korroborering när de ärvt samma brief; enighet mäter briefen, inte koden.
Example: 2026-09-21 — cf4aee358. Meningen förblev struken, men av lintens skäl (inga radnummer i prosa) och inte av den falska mekanismen, och siffran "fyra" skrevs varken i commit-meddelandet, ärendet eller den här posten annat än som det fel den var. Den sanna mängden är fem; den står här och inte i artefakten, eftersom en daterad fillista i en docstring ruttnar oavsett om dagens version stämmer.
