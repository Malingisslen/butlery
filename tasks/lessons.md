...
- **Rule**: When a change adds or edits anything under `lib/widgets/`, run `flutter test test/architecture/architecture_test.dart` **locally before pushing** — `dart analyze` does NOT enforce the project's architecture guards (raw-spinner ban, layering rules, file-size/accepted-large list, color/theme rules). Analyze-clean is necessary, not sufficient. For reusable widgets: use `LoadingIndicator` (not `CircularProgressIndicator`) and the `StateWidget` factories for loading/empty/error (see `lib/widgets/CLAUDE.md`). For agents building widgets, add "run the architecture test" to the verify step.
- **Example**: Swapped the raw spinner → `LoadingIndicator(size: 24, strokeWidth: 2)` (matching `cook_snap_gallery`'s image placeholder); arch test +16 -1 → all-green. Running the arch test in the same pre-push pass as analyze would've made this a zero-cost inline fix instead of a red-CI round-trip.
